...
```{r Model}
M2 <- glmmTMB(total_cover_prop_no1 ~ poly(Elevation,2) + DomSpeciesSuperSimp + poly(YSR,2) + DomSpeciesSuperSimp:YSR + (1|Site), data = salt_comm_rest_H, family = "beta_family")
...
-   determine which species have at least 5 values and subset for these species
...
#  Aster, Bolboschoenus, Phragmites, Pneumatophores, Sarcocornia, Sporobolus and Triglochin have >5 values, but exclude Pneumatophores as they only occur at 1 site
...
```{r Model_building}
M <- glmmTMB(Stems ~ DomSpeciesSuperSimp + Species*(Elevation + YSR) +  offset(log(Quadrat_area)) + rr(Species+0|Combo, d = 2) + (1|Site), family = "poisson", data = stem_dens_unique_H_res)

resid <- residuals(simulateResiduals(M), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M, re.form = NA), resid) # has a trend, not great but ok
plotQQunif(M) # ok

M5 <- glmmTMB(Stems ~ DomSpeciesSuperSimp + Species*(Elevation + YSR)+ offset(log(Quadrat_area)) + rr(Species+0|Combo, d = 2) + (1|Site), family = "nbinom2", data = stem_dens_unique_H_res)
...
```{r Model_testing}
M <- glmmTMB(Stems ~ DomSpeciesSuperSimp + Species*Elevation + YSR + offset(log(Quadrat_area)) + rr(Species+0|Combo, d = 2) + (1|Site), family = "nbinom2", data = stem_dens_unique_H_res)

M0 <- glmmTMB(Stems ~ DomSpeciesSuperSimp + Species*Elevation + offset(log(Quadrat_area)) + rr(Species+0|Combo, d = 2) + (1|Site), family = "nbinom2", data = stem_dens_unique_H_res)
...
-   Create a stems dataset (we'll use this for diameter and height) with the six species that have enough stem data
...
```{r Model_building}
M <- glmmTMB(Stem_dia_mm ~ DomSpeciesSuperSimp + Species*(Elevation + sp_prop + YSR) + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_dia)

plot(predict(M, re.form = NA), residuals(M)) # ok but clumped
plotQQunif(M) # ok but not great

M6 <- glmmTMB(sqrt(Stem_dia_mm) ~ DomSpeciesSuperSimp + Elevation + Species*(sp_prop + YSR) + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_dia)
...
```{r Model_testing}
M <- glmmTMB(sqrt(Stem_dia_mm) ~ DomSpeciesSuperSimp + Elevation + Species*sp_prop + YSR + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_dia)

M0 <- glmmTMB(sqrt(Stem_dia_mm) ~ DomSpeciesSuperSimp + Elevation + Species*sp_prop  + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_dia)
...
-   Use log-transformed data as raw data are right-skewed and come up with model errors (see M below)
...
M7 <- glmmTMB(log(Stem_ht_mm) ~ DomSpeciesSuperSimp + Species*(Elevation + sp_prop + YSR) +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht)
...
```{r Model_building}
M8 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species + DomSpeciesSuperSimp:Species + Elevation:Species + Species:(reprod_type * YSR) + YSR + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)
...
M8 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species + DomSpeciesSuperSimp:Species + Elevation:Species + Species:(reprod_type * YSR) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)
M1 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species + DomSpeciesSuperSimp:Species + Elevation:Species + Species:(reprod_type + YSR) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)
M <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species + DomSpeciesSuperSimp:Species + Elevation:Species + Species:reprod_type + YSR + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)
M0 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species + DomSpeciesSuperSimp:Species + Elevation:Species + Species:reprod_type + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)
...
ggplot(data = reprod.s_sf, aes(y = Reproduction, x = YSR, group = YSR, color = reprod_type, fill = reprod_type)) +   geom_boxplot() +   
  geom_jitter() +   
  facet_wrap(~Species)
```
...
-   **Need to decide whether either of these models is ok and if so which is better. Then write up the aspects of the chosen model**

-   **??** Use log(1-x) transformed data as the original data were highly left skewed
...
```{r Model_create}
M <- glmmTMB(total_cover_prop_offset ~ Elevation +  DomSpeciesSuperSimp + Estuary + rest_status + (1|Site), family = "beta_family", data = salt_comm)
...
M10 <- glmmTMB(log(1-total_cover_prop_offset) ~ Elevation +  DomSpeciesSuperSimp + Estuary + rest_status + (1|Site), data = salt_comm)
...
joint_tests(M)
summary(M)
emmeans(M, pairwise ~ rest_status, type = "response")
emmeans(M, pairwise ~ Estuary, type = "response")
emmeans(M, pairwise ~ DomSpeciesSuperSimp, type = "response")
```
...
```{r Prep/explore}
salt_less <- salt %>% 
  subset(Species %in% c("Sporobolus", "Sarcocornia", "Pneumatophores", "Triglochin")) %>% 
...
```{r Prep/explore}
salt_less <- salt %>% 
  subset(Species %in% c("Sporobolus", "Sarcocornia", "Pneumatophores", "Triglochin")) %>% subset(!is.na(Stem_ht_mm))
...
```{r Model_create}
M15 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species + Species:Elevation + sp_prop + reprod_type * rest_status + Species:(sp_prop + reprod_type * rest_status) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long)
...
```{r Model_test}
M15 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species*(sp_prop + reprod_type *  rest_status) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long)

M1 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species*(reprod_type + sp_prop + rest_status) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long)

M <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species*(reprod_type + sp_prop) + rest_status + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long)

M0 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species*(reprod_type + sp_prop) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long)

anova(M15,M1,M,M0)

M15 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species + Species:Elevation + sp_prop + reprod_type * rest_status + Species:(sp_prop + reprod_type * rest_status) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long)
M1 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species + Species:Elevation + sp_prop + reprod_type * rest_status + Species:(sp_prop + reprod_type + rest_status) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long)
M <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species + Species:Elevation + sp_prop + reprod_type * rest_status + Species:(sp_prop + reprod_type) + rest_status + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long)
M0 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species + Species:Elevation + sp_prop + reprod_type * rest_status + Species:(sp_prop + reprod_type) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long)
