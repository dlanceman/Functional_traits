...
This code is for analysis of saltmarsh functional traits data, analysing for associations between functional traits and time since restoration and between restoring and natural sites.
...
-   using setwd() to set the working directory where the data are stored
    ...
    -   In this working directory, you should have the project file, and the subfolders "Data", "Script" and "Output"
...
Run this function on all data for community-level variables, and on specific species individually for species- and stem-level variables so we can see if any measured observations from a species seem unlikely for that particular species.
...
cleveland_plot(spor$Stem_dia_mm, "Stem diameter (mm)") # right skewed. There are some very large values that need investigating - plant net doesn't state the regular stem diameter, but >4mm seems unrealistic based on field experience, so exclude the two values >4mm. Also, the 0.1mm measurements are likely too low, but should be OK and no reason to remove, so retain
...
salt[c(862,947),20] <- NA # remove sporobolus measurements with unrealistic diameters > 4mm
```
...
```{r Subset}
salt_H_rest <- salt %>% subset(Estuary == "Hunter") %>% subset(rest_status == 1)
...
# Hunter only PCA
salt_comm_comp_H_p <- salt_comm_comp_H %>% 
  mutate(rest_status = case_when(
    rest_status == 1 ~ "Restored",
    rest_status == 0 ~ "Natural"
  ))
spdata <- salt_comm_comp_H_p[,c(1,2,3,8,10,12)]
pca_res <- prcomp(spdata, scale. = TRUE)
autoplot(pca_res, data = salt_comm_comp_H, color = "YSR", size = 2,
         loadings = TRUE, loadings.colour = 'black',
         loadings.label = TRUE, loadings.label.size = 6, loadings.label.color = 'black',
         loadings.label.hjust = 0, loadings.label.vjust = -0.5) +
  theme_classic() +
...
#ggsave("C:/Users/z5204897/OneDrive - UNSW/Desktop/PhD/Data_analysis/Project/Functional_traits/Output/Saltmarsh_comm_comp.jpg", width = 2400, height = 1500, units = "px")
```
...
# Proportion of each Dominant species
salt_comm_rest_H %>% group_by(DomSpeciesSuperSimp) %>% summarise(n = n()) %>% ungroup()
```
...
-   not sure geom_smooth works for emmip - it's making the same line but not sure we can trust the confidence intervals. Want some way of making smoothed CIs though (didn't use this for paper)
...
-   The data are count data, so use either a poisson or negative binomial model.
    ...
    -   negative binomial looks slightly better, so use that
...
-   Make sure plots are for total stems, not stems without offset (as below - either stems with offset or total stems)
...
-   Use log transformation, to be consistent with the restoration status stem diameter model
...
plot(predict(M6, re.form = NA), residuals(M6)) # clumped but ok
plotQQunif(M6) # not great but acceptable as normality is not that important here

# check collinearity
M0 <- glmmTMB(log(Stem_dia_mm) ~ DomSpeciesSuperSimp + Species + Elevation + sp_prop + YSR +  rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_dia)
...
-   scale Elevation and sp_prop, as the model does not run properly (null convergence) without scaling these predictors.
...
M6 <- glmmTMB(log(Stem_dia_mm) ~ Species + DomSpeciesSuperSimp + Species:scale(Elevation) + Species:scale(sp_prop) + Species:YSR + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_dia)

M0 <- glmmTMB(log(Stem_dia_mm) ~ Species + DomSpeciesSuperSimp + Species:scale(Elevation) + Species:scale(sp_prop)  + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_dia)
...
-   Use square-root transformed data to be consistent with the stem height restoration status model

-   We are interested in how stem heights change for each species considering all other factors, so create interactions with as many factors as possible
...
-   anova to test importance of YSR \* Species \* reprod_type and YSR by comparing with two null models (M, M0)
...
-   try both a non-zero-inflated and a zero-inflated poisson model and negative binomial model, as it's count data
...
-   The data are count data, so try poisson or negative binomial models.

    -   negative binomial looks better, so use that
...
Note, the negative binomial QQ plot has a small reverse fan shape. This suggests lower variability with higher values. The main impact of this is lack of power and since it doesn't inflate type 1 error rate, we considered it to be fine.
...
-   continuous data, so gaussian model

-   log-transform the dependent variable as that meant residuals looked better
