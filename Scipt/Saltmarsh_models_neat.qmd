...
#ggsave("C:/Users/z5204897/OneDrive - UNSW/Desktop/PhD/Data_analysis/Project/Functional_traits/Output/Saltmarsh_elevation_doc.jpg", width = 2400, height = 1500, units = "px")
```
...
M1.1 <- glmmTMB(cover_prop_no1 ~ Species:YSR + (1|Site/Combo) + rr(Species+0|Combo, d = 1), data = comm_comp_long, family = beta_family(link = "logit"), ziformula = ~1)

# check assumptions
resid <- residuals(simulateResiduals(M1.1), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M1.1,re.form = NA), resid)   # great
plotQQunif(M1.1) 
# residuals looked better with elevation but still fine without
```
...
M0.1 <- glmmTMB(cover_prop_no1 ~ (1|Site/Combo) + rr(Species+0|Combo, d = 1), data = comm_comp_long, family = beta_family(link = "logit"), ziformula = ~1)
...
autoplot(pca_res, data = salt_comm_comp, color = "YSR", size = 2,
         ...
         loadings.label = TRUE, loadings.label.size = 5, loadings.label.color = 'black', loadings.label.vjust = 1.2, loadings.label.hjust = 0.2) +
  theme_classic() +
...
# change DomSpeciesSuperSimp into a factor variable with the DomSpecies in the correct order
salt_comm_rest_H$DomSpeciesSuperSimp <- factor(salt_comm_rest_H$DomSpeciesSuperSimp, levels = c("Sporobolus", "Sarcocornia", "Other")) 
salt_comm_nat$DomSpeciesSuperSimp <- factor(salt_comm_nat$DomSpeciesSuperSimp, levels = c("Sporobolus", "Sarcocornia", "Other")) 
```
...
Try a model without elevation.

```{r Model_no_elevation}
...
scatter.smooth(predict(M2.1,re.form = NA), resid) # looks great
plotQQunif(M2.1)

# residuals look identical to model with elevation, good

M1 <- glmmTMB(total_cover_prop_no1 ~ DomSpeciesSuperSimp + poly(YSR,2) + (1|Site), data = salt_comm_rest_H, family = "beta_family")

M <- glmmTMB(total_cover_prop_no1 ~ DomSpeciesSuperSimp + YSR + (1|Site), data = salt_comm_rest_H, family = "beta_family")

M0 <- glmmTMB(total_cover_prop_no1 ~ DomSpeciesSuperSimp + (1|Site), data = salt_comm_rest_H, family = "beta_family")

anova(M2.1,M1, M0, M)

em <- emmeans(M2.1, ~ YSR, at = list(YSR = c(6,11,12,16,30,33)), type = "response")

em

plot(em)

em1 <- emmeans(M2.1, ~ YSR | DomSpeciesSuperSimp, at = list(YSR = c(6,11,12,16,30,33)), type = "response")
...
S_Cov_rest2 <- ggplot(data = salt_comm_rest_H, aes(x = YSR, y = total_cover_prop * 100, group = DomSpeciesSuperSimp, color = DomSpeciesSuperSimp)) +
  ...
  geom_line(data = data.frame(em), aes(y = response * 100), group = 1, colour = "black", linewidth = 0.5) +
...
ggarrange(S_Cov_rest2, S_Cov_nat2, ncol =2,nrow=1,widths = c(2,1.5)) 

ggsave("C:/Users/z5204897/OneDrive - UNSW/Desktop/PhD/Data_analysis/Project/Functional_traits/Output/Saltmarsh_cover_doc.jpg", width = 2400, height = 1400, units = "px")
```
...
```{r Model_no_elevation}
M3.1 <- glmmTMB(Burrows_0.5x0.5 ~ DomSpeciesSuperSimp + poly(YSR,2) +  (1|Site), data = salt_comm_rest_H_burrows, family = "nbinom2")
...
M <- glmmTMB(Burrows_0.5x0.5 ~ DomSpeciesSuperSimp + YSR +  (1|Site), data = salt_comm_rest_H_burrows, family = "nbinom2")
M0 <- glmmTMB(Burrows_0.5x0.5 ~ DomSpeciesSuperSimp +  (1|Site), data = salt_comm_rest_H_burrows, family = "nbinom2")
...
em <- emmeans(M3.1, ~ YSR, at = list(YSR = c(6,11,12,16,30,33)), type = "response")
...
ggsave("C:/Users/z5204897/OneDrive - UNSW/Desktop/PhD/Data_analysis/Project/Functional_traits/Output/Saltmarsh_burrows.jpg", width = 4000, height = 2500, units = "px")
...
S_Bur_rest2 <- ggplot(data = salt_comm_rest_H, aes(x = YSR, y = Burrows_0.5x0.5)) +
  geom_jitter(alpha = 0.5) +
  geom_point(data = data.frame(em), aes(y = response), group = NA, colour = "black", shape = 7, size = 3)+
  geom_line(data = data.frame(em), aes(y = response), group = 1, colour = "black", linewidth = 0.5) +
  theme_classic() +
  theme(legend.position = "none",
        axis.text = element_text(size = 12),
        axis.title = element_text(size = 16)) +
...
S_Bur_nat2 <- ggplot(salt_comm_nat, aes(x=Site, y=Burrows_0.5x0.5, fill=Estuary, group = Site)) +
    geom_boxplot(width=0.4) +
  geom_jitter(width = 0.1, height = 0, alpha = 0.5)  +
    scale_fill_viridis(discrete = TRUE, begin = 0.5, end = 0.8, option = "A") +
    theme_classic() +
...
ggarrange(S_Bur_rest2,S_Bur_nat2,ncol =2,nrow=1,widths = c(3,1))

ggsave("C:/Users/z5204897/OneDrive - UNSW/Desktop/PhD/Data_analysis/Project/Functional_traits/Output/Saltmarsh_burrows_line.jpg", width = 2400, height = 1500, units = "px")
```
...
Mx <- glmmTMB(Stems ~ Species*(DomSpeciesSuperSimp + Elevation) + offset(log(Quadrat_area)) + rr(Species+0|Combo, d = 2) + (1|Site), family = "nbinom2", data = stem_dens_unique_H_res)
...
# check whether to include DomSpecies in brackets
M7 <- glmmTMB(sqrt(Stem_ht_mm) ~ DomSpeciesSuperSimp + Species*(Elevation + sp_prop + YSR) +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht)
M7.1 <- glmmTMB(sqrt(Stem_ht_mm) ~ Species*(DomSpeciesSuperSimp + Elevation + sp_prop + YSR) +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht)
anova(M7,M7.1) # include in brackets

plot(predict(M7.1, re.form = NA), residuals(M7)) 
plotQQunif(M7.1)
...
```{r Model_testing}
M <- glmmTMB(log(Stem_ht_mm) ~  Species*(DomSpeciesSuperSimp +Elevation + sp_prop) + YSR +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht)
M0 <- glmmTMB(log(Stem_ht_mm) ~ Species*(DomSpeciesSuperSimp + Elevation + sp_prop) +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht)

anova(M7.1, M, M0)
summary(M7.1)

em <- emtrends(M7.1, var = "YSR", specs = ~Species, type = "response")

em

plot(em)

emmeans(M7.1, ~ YSR | Species, at = list(YSR = c(6,11,12,16,30,33)), type = "response")
...
```{r Plot}
emmip(M7.1, ~ YSR | Species, at = list(YSR = c(6:33)), type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Stem height (mm)") +   
...
```{r Model_building}
M8 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species*(sp_prop + reprod_type * YSR) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)
...
M0 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Species + sp_prop + Elevation +   reprod_type + YSR  + (1|Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)
...
```{r Model_testing}
M1 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species*(sp_prop + reprod_type + YSR) + (1|Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)

M <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species*(sp_prop + reprod_type) + YSR + (1|Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)

M0 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species*(sp_prop + reprod_type)  + (1|Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)
...
# just sarcocornia flowering
emmip(M8, ~ YSR , at = list(YSR = c(6:33), Species = "Sarcocornia", reprod_type = "Flowers"), type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Reproduction likelihood") +   
  geom_point(data = reprod_proportions %>% subset(Species == "Sarcocornia") %>% subset(reprod_type == "Flowers"), aes(x = YSR, y = avg, group = reprod_type)) 
```
...
```{r Elevation_model}
M8.5 <- glmmTMB(Elevation ~ DomSpeciesSuperSimp + rest_status + Estuary + (1|Site), family = "gaussian", data = salt_comm)
...
M0 <- glmmTMB(Elevation ~ DomSpeciesSuperSimp + Estuary + (1|Site), family = "gaussian", data = salt_comm)

anova(M8.5,M0)

emmeans(M8.5, ~ rest_status, type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Elevation (m)")
...
# check collinearity on a model without interaction terms
M0 <- glmmTMB(Sp_cover_prop ~ Estuary + Species + Elevation + rest_status + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = beta_family(link = "logit"), ziformula = ~ 1, data = salt_comm_comp_long_less)
check_collinearity(M0) # all low
```
...
```{r Model_create}
M <- glmmTMB(Stems ~ Estuary + Species*(DomSpeciesSuperSimp + Elevation + rest_status) + (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "poisson", data = stem_dens_unique_less)

resid <- residuals(simulateResiduals(M), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M, re.form = NA), resid)
plotQQunif(M)

M0 <- glmmTMB(Stems ~ Estuary +  Species*(DomSpeciesSuperSimp +Elevation + rest_status) + (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "nbinom2", data = stem_dens_unique_less)
...
M12 <- glmmTMB(sqrt(Stems) ~ Estuary + Species * (DomSpeciesSuperSimp + Elevation +  rest_status) +  (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "gaussian", data = stem_dens_unique_less)
...
# should DomSpecies be inside or outside of the brackets?
M12 <- glmmTMB(sqrt(Stems) ~ Estuary + Species * (DomSpeciesSuperSimp + Elevation +  rest_status) +  (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "gaussian", data = stem_dens_unique_less)
M12.1 <- glmmTMB(sqrt(Stems) ~ Estuary + DomSpeciesSuperSimp + Species * (Elevation +  rest_status) +  (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "gaussian", data = stem_dens_unique_less)
anova(M12,M12.1) # inside is better.
...
```{r Model_test}
M <- glmmTMB(sqrt(Stems) ~ Estuary + Species * (DomSpeciesSuperSimp + Elevation) +  rest_status +  (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "gaussian", data = stem_dens_unique_less)

M0 <- glmmTMB(sqrt(Stems) ~ Estuary + Species * (DomSpeciesSuperSimp + Elevation) +   (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "gaussian", data = stem_dens_unique_less)
...
-   try untransformed, log-transformed and sqrt-transformed dependent variable

    -   sqrt is best, assumptions don't look great for any model but hopefully good enough
...
M13.1 <- glmmTMB((Stem_dia_mm) ~ DomSpeciesSuperSimp + Estuary + Species*(Elevation + sp_prop + rest_status) +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)

resid <- residuals(simulateResiduals(M13.1), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M13.1, re.form = NA), resid) 
plotQQunif(M13.1) 
scatter.smooth(predict(M13.1, re.form = NA), abs(resid))

M13.2 <- glmmTMB(sqrt(Stem_dia_mm) ~ DomSpeciesSuperSimp + Estuary + Species*(Elevation + sp_prop + rest_status) +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)

resid <- residuals(simulateResiduals(M13.2), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M13.2, re.form = NA), resid) 
plotQQunif(M13.2) 
scatter.smooth(predict(M13.2, re.form = NA), abs(resid))

# test which model is best
anova(M13.1,M13.2,M13) #M13.2

# also test whether putting DomSpecies inside the Species bracket improves the model
M13.2 <- glmmTMB(sqrt(Stem_dia_mm) ~ DomSpeciesSuperSimp + Estuary + Species*(Elevation + sp_prop + rest_status) +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
M13.3 <- glmmTMB(sqrt(Stem_dia_mm) ~ Estuary + Species*(DomSpeciesSuperSimp + Elevation + sp_prop + rest_status) +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
anova(M13.2,M13.3) # yes, improves

resid <- residuals(simulateResiduals(M13.3), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M13.3, re.form = NA), resid) 
plotQQunif(M13.3) 
scatter.smooth(predict(M13.3, re.form = NA), abs(resid))

# check collinearity on a model without interaction terms
M0 <- glmmTMB(sqrt(Stem_dia_mm) ~ Estuary + Species + DomSpeciesSuperSimp + Elevation + sp_prop + rest_status +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
check_collinearity(M0) # all low
```
...
```{r Model_test}
M <- glmmTMB(sqrt(Stem_dia_mm) ~ Estuary + Species*(DomSpeciesSuperSimp + Elevation + sp_prop)  + rest_status +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)

M0 <- glmmTMB(sqrt(Stem_dia_mm) ~ Estuary + Species*(DomSpeciesSuperSimp + Elevation + sp_prop) +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)

anova(M13.3,M,M0)

joint_tests(M13.3)
emmeans(M13.3, pairwise ~ rest_status | Species, type = "response")
emmeans(M13.3, ~ rest_status, type = "response")
summary(M13.3)
```
...
-   Use square-root or log-transformed stem height, as when you try to run the below model with non-transformed data, it comes up with model convergence issues.
...
M14.1 <- glmmTMB(log(Stem_ht_mm) ~ Estuary + DomSpeciesSuperSimp + Species*(Elevation + sp_prop + rest_status) +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)

resid <- residuals(simulateResiduals(M14.1), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M14.1, re.form = NA), resid) # fine but some outliers
plotQQunif(M14.1)

# test differences between sqrt and log transformed
anova(M14,M14.1)

# also check whether DomSpecies in bracket improves the model
M14.2 <- glmmTMB(log(Stem_ht_mm) ~ Estuary + Species*(DomSpeciesSuperSimp + Elevation + sp_prop + rest_status) +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
anova(M14.1,M14.2) # yes, improves

resid <- residuals(simulateResiduals(M14.2), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M14.2, re.form = NA), resid) # fine but some outliers
plotQQunif(M14.2)

M0 <- glmmTMB(log(Stem_ht_mm) ~ Estuary + Species + Elevation + sp_prop + rest_status +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
...
```{r Model_test}
M <- glmmTMB(log(Stem_ht_mm) ~ Estuary + Species*(DomSpeciesSuperSimp + Elevation + sp_prop) + rest_status +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)

M0 <- glmmTMB(log(Stem_ht_mm) ~ Estuary + Species*(DomSpeciesSuperSimp + Elevation + sp_prop) +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
...
emmeans(M14.2, ~ rest_status, type = "response")
emmeans(M14.2, ~ rest_status | Species, type = "response")
```

Finally, plot the data.

```{r Plot}
ggplot(salt_less, aes(x=as.factor(rest_status), y=Stem_ht_mm, fill = as.factor(rest_status))) +
    geom_violin() +
  geom_jitter()  +
    scale_fill_viridis(discrete = TRUE, begin = 0.5, end = 0.8, option = "A") +
    theme_classic() +
...
-   as we expect each species to have different responses, include interactions between Species and Elevation, species proportion and restoration status
    ...
    -   doesn't run with elevation \* species interaction

    -   doesn't run without DomSpecies \* Species interaction so include that
...
```{r Model_create}
M15 <- glmmTMB(Reproduction ~ Elevation + Species*(DomSpeciesSuperSimp + sp_prop + reprod_type * rest_status) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long)

resid <- residuals(simulateResiduals(M15), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M15, re.form = NA), resid) # fine
plotQQunif(M15) 

# check collinearity on model without interactions
M0 <- glmmTMB(Reproduction ~ Elevation + Species + DomSpeciesSuperSimp + sp_prop + reprod_type + rest_status + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long)
check_collinearity(M0)
```
...
```{r Model_test}
M1 <- glmmTMB(Reproduction ~ Elevation + Species*(DomSpeciesSuperSimp + sp_prop + reprod_type + rest_status) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long)

M <- glmmTMB(Reproduction ~ Elevation + Species*(DomSpeciesSuperSimp + sp_prop + reprod_type) + rest_status + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long)

M0 <- glmmTMB(Reproduction ~ Elevation + Species*(DomSpeciesSuperSimp + sp_prop + reprod_type) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long)
...
ggsave("C:/Users/z5204897/OneDrive - UNSW/Desktop/PhD/Data_analysis/Project/Functional_traits/Output/Saltmarsh_reproduction.jpg", width = 2000, height = 1250, units = "px")


emmeans(M8, ~ YSR, at = list(YSR = c(6,11,12,16,30,33), Species = "Sporobolus", reprod_type = "Seeds"), type = "response")

S_Reprod_rest2 <- ggplot(data = reprod_all_long, aes(x = YSR, y = Burrows_0.5x0.5)) +
  geom_jitter(alpha = 0.5) +
  geom_point(data = data.frame(em), aes(y = response), group = NA, colour = "black", shape = 7, size = 3)+
  geom_line(data = data.frame(em), aes(y = response), group = 1, colour = "black", linewidth = 0.5) +
  theme_classic() +
...
For species-specific traits, are the above models actually asking our desired questions around whether each species is experiencing trait changes over time, or do these models simply test for an overall change and differences between species' trends? For model robustness, check this using single-species models.

## 4.1 Stem density YSR

-   Need to run the Stem density YSR code in 2d first to prepare the dataset we are using for this

### Sporobolus

```{r Prep/exlore}
#PREP
stem_dens_rest_spor <- stem_dens_unique_H_res %>% subset(Species == "Sporobolus")

#EXPLORE
hist(stem_dens_rest_spor$total_stems) # right skewed

plot(stem_dens_rest_spor$total_stems ~ stem_dens_rest_spor$YSR) # not sure if it's non-linear
ggplot(data = stem_dens_rest_spor, aes(x = YSR, y = total_stems)) +
  geom_point() +
  geom_smooth(method = "lm") +
  geom_smooth() # could be polynomial

# without accounting for offset
ggplot(data = stem_dens_rest_spor, aes(y = Stems, x = YSR)) +
  geom_point() 

plot(stem_dens_rest_spor$total_stems ~ stem_dens_rest_spor$Elevation) # no evidence it's non linear
...
```{r Model_building}
M16 <- glmmTMB(Stems ~ DomSpeciesSuperSimp + sp_prop + Elevation + YSR + offset(log(Quadrat_area)) + (1|Site), family = "nbinom2", data = stem_dens_rest_spor)

resid <- residuals(simulateResiduals(M16), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M16, re.form = NA), resid) # has a trend
scatter.smooth(predict(M16, re.form = NA), abs(resid))
plotQQunif(M16) 

# test if polynomial is better
M16.0 <- glmmTMB(Stems ~ DomSpeciesSuperSimp + sp_prop + Elevation + poly(YSR,2) + offset(log(Quadrat_area)) + (1|Site), family = "nbinom2", data = stem_dens_rest_spor)
anova(M16,M16.0) # no better, stick with linear

M16.1 <- glmmTMB(Stems ~ DomSpeciesSuperSimp + sp_prop +  Elevation + YSR + offset(log(Quadrat_area)) + (1|Site), family = "poisson", data = stem_dens_rest_spor)

resid <- residuals(simulateResiduals(M16.1), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M16.1, re.form = NA), resid) # worse than above
scatter.smooth(predict(M16.1, re.form = NA), abs(resid))
plotQQunif(M16.1) # terrible

#M16 should be fine, the apparent trend is mostly based on a couple outlier points, and the majority of the points seem evenly spread
...
```{r Model_testing}
M16 <- glmmTMB(Stems ~ DomSpeciesSuperSimp + sp_prop + Elevation + YSR + offset(log(Quadrat_area)) + (1|Site), family = "nbinom2", data = stem_dens_rest_spor)
M0 <- glmmTMB(Stems ~ DomSpeciesSuperSimp + sp_prop + Elevation + offset(log(Quadrat_area)) + (1|Site), family = "nbinom2", data = stem_dens_rest_spor)
...
emmeans(M16, ~ YSR, at = list(YSR = c(6,11,12,16,30,33)), type = "response", offset = log(10000))
```

## 4.2 Stem diameter YSR

Again, run 2e prep code first to create the dataframe needed for this.

### Sporobolus

First, explore and prep.

```{r Explore/prep}
stem_dia_rest_spor <- salt_H_rest_stems_dia %>% subset(Species == "Sporobolus")

#EXPLORE
hist(stem_dia_rest_spor$Stem_dia_mm) # right skewed

plot(stem_dia_rest_spor$Stem_dia_mm ~ stem_dia_rest_spor$YSR) # doesn't look non-linear

plot(stem_dia_rest_spor$Stem_dia_mm ~ stem_dia_rest_spor$Elevation) # no evidence it's non linear
```

Next, create a model.

```{r Model_building}
M17 <- glmmTMB((Stem_dia_mm) ~ DomSpeciesSuperSimp + Elevation + sp_prop + YSR + (1|Site/Combo), family = "gaussian", data = stem_dia_rest_spor)

resid <- residuals(simulateResiduals(M17), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M17, re.form = NA), resid) # has a trend
plotQQunif(M17) 

M17.1 <- glmmTMB(sqrt(Stem_dia_mm) ~ DomSpeciesSuperSimp + Elevation + sp_prop + YSR + (1|Site/Combo), family = "gaussian", data = stem_dia_rest_spor)

resid <- residuals(simulateResiduals(M17.1), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M17.1, re.form = NA), resid) # has a trend
plotQQunif(M17.1) 

M17.2 <- glmmTMB(log(Stem_dia_mm) ~ DomSpeciesSuperSimp + Elevation + sp_prop + YSR + (1|Site/Combo), family = "gaussian", data = stem_dia_rest_spor)

resid <- residuals(simulateResiduals(M17.1), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M17.1, re.form = NA), resid) # has a trend
plotQQunif(M17.1) 

# no qqplots are great but residuals plots look fine and this is what matters. compare the models using anova

anova(M17,M17.1,M17.2)
# M17.1 is best
```
...
```{r}
M17.1 <- glmmTMB(sqrt(Stem_dia_mm) ~ DomSpeciesSuperSimp + Elevation + sp_prop + YSR + (1|Site/Combo), family = "gaussian", data = stem_dia_rest_spor)
M0 <- glmmTMB(sqrt(Stem_dia_mm) ~ DomSpeciesSuperSimp + Elevation + sp_prop + (1|Site/Combo), family = "gaussian", data = stem_dia_rest_spor)
...
emmeans(M17.1, ~ YSR, at = list(YSR = c(6,11,12,16,30,33)), type = "response", offset = log(10000))
```

## 4.3 Reproduction YSR

### Sporobolus seed relative quantity

```{r Prep/explore}
# create an appropriate dataframe to use
spor_comm <- salt %>% subset(Species == "Sporobolus") %>% distinct(Combo, .keep_all = TRUE)
# make seeds into an ordinal variable
spor_comm$Seeds <- factor(spor_comm$Seeds, order = TRUE, levels = c("No", "Very few", "Few", "Some", "Lots"))

#EXPLORE
# can't look for skew and linearity as it's a categorical variable. But can explore potential trends
plot(spor_comm$Seeds ~ spor_comm$YSR)

plot(spor_comm$Seeds ~ spor_comm$Elevation) 
```
