...
-   relationship between YSR and Elevation (numeric predictors)
    -   model this relationship and test the model
        -   **this is not meeting assumptions, log and sqrt transformations don't help**
...
M <- glmmTMB(Elevation ~ DomSpeciesSuperSimp + YSR + (1|Site), family = "gaussian", data = salt_comm_rest_H)
...
```{r Model_building}
M <- glmmTMB(Stems ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:YSR  +  offset(log(Quadrat_area)) + rr(Species+0|Combo, d = 2) + (1|Site), family = "poisson", data = stem_dens_unique_H_res)

resid <- residuals(simulateResiduals(M), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M, re.form = NA), resid) # has a trend, not great but ok
plotQQunif(M) # ok

M5 <- glmmTMB(Stems ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:YSR + offset(log(Quadrat_area)) + rr(Species+0|Combo, d = 2) + (1|Site), family = "nbinom2", data = stem_dens_unique_H_res)
...
M0 <- glmmTMB(Stems ~ Species + DomSpeciesSuperSimp + Species:Elevation + offset(log(Quadrat_area)) + rr(Species+0|Combo, d = 2) + (1|Site), family = "nbinom2", data = stem_dens_unique_H_res)
...
M8.5 <- glmmTMB(Elevation ~ Estuary + rest_status + (1|Site), family = "gaussian", data = salt_comm)
...
M0 <- glmmTMB(Elevation ~ Estuary + (1|Site), family = "gaussian", data = salt_comm)
...
```{r Model_create}
M <- glmmTMB(Stems ~ Estuary + Species + DomSpeciesSuperSimp + Species:Elevation + Species:rest_status + (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "poisson", data = stem_dens_unique_less)

resid <- residuals(simulateResiduals(M), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M, re.form = NA), resid)
plotQQunif(M)

M0 <- glmmTMB(Stems ~ Estuary + Species + DomSpeciesSuperSimp + Species:Elevation + Species:rest_status + (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "nbinom2", data = stem_dens_unique_less)
...
M12 <- glmmTMB(sqrt(Stems) ~ Estuary + Species + DomSpeciesSuperSimp + Species:Elevation + Species:rest_status +  (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "gaussian", data = stem_dens_unique_less)
...
Mx <- glmmTMB(sqrt(Stems) ~ Estuary + Species + Elevation +  rest_status +  (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "gaussian", data = stem_dens_unique_less)
...
M <- glmmTMB(sqrt(Stems) ~ Estuary + Species + DomSpeciesSuperSimp + Species:Elevation +  (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "gaussian", data = stem_dens_unique_less)
