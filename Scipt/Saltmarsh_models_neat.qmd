...
S_Bur_rest <- ggplot(salt_comm_rest, aes(x=YSR, y=(Burrows_0.5x0.5+0.01), group = YSR, fill = Estuary)) +
  geom_hline(yintercept = mean(salt_comm_rest$Burrows_0.5x0.5, na.rm = TRUE), linetype = "dashed", linewidth = 1) +
  annotate(
    "rect",
    xmin = -Inf, xmax = Inf,  # Extend the rectangle across the entire x-axis
    ymin = 1, 
    ymax = 11,  # Set the y-range for the rectangle as the 95% confidence interval bounds
    alpha = 0.4,  # Set the transparency level
    fill = "grey"  # Set the fill color
  ) +
    geom_boxplot(width = 2) +
...
S_Bur_nat <- ggplot(salt_comm_nat, aes(x=Site, y=Burrows_0.5x0.5, fill=Estuary, group = Site)) +
  ...
  annotate(
    ...
    xmin = -Inf, xmax = Inf,  # Extend the rectangle across the entire x-axis
    ymin = 2, 
    ymax = 23,  # Set the y-range for the rectangle as the 95% confidence interval bounds
    alpha = 0.4,  # Set the transparency level
    fill = "grey"  # Set the fill color
  ) +
    geom_boxplot(width=0.4) +
...
plot(predict(M7.0), residuals(M7.0)) # clumped but ok
...
-   try both a non-zero-inflated and a zero-inflated poisson model and negative binomial model
...
M1 <- glmmTMB(Burrows_0.5x0.5 ~ Estuary + DomSpeciesSuperSimp + Elevation + rest_status + (1|Site), data = salt_comm_burrows, family = "poisson", ziformula =~1)
...
M11 <- glmmTMB(Burrows_0.5x0.5 ~ Estuary + DomSpeciesSuperSimp + Elevation + rest_status + (1|Site), data = salt_comm_burrows, family = "nbinom2")
...
```{r Model_test}
M0 <- glmmTMB(Burrows_0.5x0.5 ~ Estuary + DomSpeciesSuperSimp + Elevation + (1|Site), data = salt_comm_burrows, family = "nbinom2")
...
```{r Plot}
ggplot(salt_comm, aes(x=Estuary, y=Burrows_0.5x0.5, fill = as.factor(DomSpeciesSuperSimp))) +
    geom_boxplot() +
  geom_jitter(size = 3)  +
    scale_fill_viridis(discrete = TRUE, begin = 0.5, end = 0.8, option = "A",
                      name="",
...
Mx <- glmmTMB(Stems ~ Estuary + Species + DomSpeciesSuperSimp + sp_prop + rest_status + (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "nbinom2", data = stem_dens_unique_less)
check_collinearity(Mx) # all low
```
