...
-   relationship between YSR and Elevation (numeric predictors)
    -   need to include YSR as a polynomial variable as the relationship doesn't look linear and the residuals graph has a trend without the polynomial variable
    -   model this relationship and test the model
        -   residuals plot doesn't look great with untransformed data, so also try log and sqrt transformed data (offset as they can't take negatives)
            -   transformed data don't look good. stick with original but need to try troubleshooting further
...
# TEST MODEL
M <- glmmTMB(Elevation ~ DomSpeciesSuperSimp + poly(YSR,2) + (1|Site), family = "gaussian", data = salt_comm_rest_H)
M0 <- glmmTMB(Elevation ~ DomSpeciesSuperSimp + (1|Site), family = "gaussian", data = salt_comm_rest_H)

anova(M,M0)

# also test whether it is a polynomial or linear effect
M0.5 <- glmmTMB(Elevation ~ DomSpeciesSuperSimp + YSR + (1|Site), family = "gaussian", data = salt_comm_rest_H)
anova(M,M0.5)
...
S_Elev_rest <- ggplot(salt_comm_rest, aes(x=YSR, y=Elevation, group = YSR, fill = Estuary)) +
      geom_hline(yintercept = mean(salt_comm_rest$Elevation), linetype = "dashed", linewidth = 1) +
  annotate(
    "rect",
    xmin = -Inf, xmax = Inf,  # Extend the rectangle across the entire x-axis
    ymin = mean(salt_comm_rest$Elevation) - 0.14, 
    ymax = mean(salt_comm_rest$Elevation) + 0.14,  # Set the y-range for the rectangle as the 95% confidence interval bounds
...
S_Elev_nat <- ggplot(salt_comm_nat, aes(x=Site, y=Elevation, fill=Estuary, group = Site)) +
  geom_hline(yintercept = mean(salt_comm_nat$Elevation), linetype = "dashed", linewidth = 1) +
  annotate(
    "rect",
    xmin = -Inf, xmax = Inf,  # Extend the rectangle across the entire x-axis
    ymin = mean(salt_comm_nat$Elevation) - 0.16, 
    ymax = mean(salt_comm_nat$Elevation) + 0.16,  # Set the y-range for the rectangle as the 95% confidence interval bounds
    alpha = 0.4,  # Set the transparency level
    fill = "grey"  # Set the fill color
  ) +
  ...
  guides(fill = guide_legend(position = "inside")) +
    theme(legend.position.inside = c(0.5,0.2),
          ...
          legend.title = element_text(size = 16)) + # was 36
  xlab("Natural site") +
  ylab("")  +
  ylim(-0.5,1)
S_Elev_nat
...
ggarrange(S_Elev_rest,S_Elev_nat,ncol =2,nrow=1,widths = c(2,1))
# print at 1500 x 800 for paper, 2400 x 800 for preso

#ggsave("C:/Users/z5204897/OneDrive - UNSW/Desktop/PhD/Data_analysis/Project/Functional_traits/Output/Saltmarsh_elev_doc.jpg", width = 2400, height = 1500, units = "px")

emmip(M, ~YSR, at = list(YSR = c(5:33)), type = "response", CIs = TRUE) +
  xlim(0,35)
```
...
```{r Model_building}
M <- glmmTMB(cover_prop_no01 ~ Species:Elevation + Species:YSR + (1|Site) + rr(Species+0|Combo, d = 1), data = comm_comp_long, family = beta_family(link = "logit"))
...
M1 <- glmmTMB(cover_prop_no1 ~ Species:Elevation + Species:YSR + (1|Site/Combo) + rr(Species+0|Combo, d = 1), data = comm_comp_long, family = beta_family(link = "logit"), ziformula = ~1)
...
-   test model using anova and comparing it to a model with no interaction with DomSpecies (M1) and with no YSR term (M0)

-   if no significant interaction, compare model with a model without the polynomial YSR (M) to determine whether the trend is polynomial or linear (comparing AIC values)
...
#M2 <- glmmTMB(total_cover_prop_no1 ~ poly(Elevation,2) + DomSpeciesSuperSimp * poly(YSR,2) + (1|Site), data = salt_comm_rest_H, family = "beta_family", contrasts = list(DomSpeciesSuperSimp = "contr.sum"))
```
...
S_Cov_nat <- ggplot(salt_comm_nat, aes(x=Site, y=`Total_cover_%`, fill=Estuary, group = Site)) +
      geom_hline(yintercept = mean(salt_comm_nat$`Total_cover_%`, na.rm = TRUE), linetype = "dashed", linewidth = 1) +
  annotate(
    "rect",
    xmin = -Inf, xmax = Inf,  # Extend the rectangle across the entire x-axis
    ymin = 78, 
    ymax = 92,  # Set the y-range for the rectangle as the 95% confidence interval bounds
    alpha = 0.4,  # Set the transparency level
    fill = "grey"  # Set the fill color
  ) +
    geom_boxplot(width=0.4) +
...
emmip(M2, ~ YSR | DomSpeciesSuperSimp | Elevation, at = list(YSR = c(6,10,15,20,25,30,33), Elevation = c(-0.2,0,0.2,0.4,0.6,0.8)), type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Predicted proportion cover") 

# fit a model not including elevation - for plotting
M2.1 <- glmmTMB(total_cover_prop_no1 ~ DomSpeciesSuperSimp * poly(YSR,2) + (1|Site), data = salt_comm_rest_H, family = "beta_family")
...
# plot overall predicted cover with only sporobolus and sarcocornia data and no elevation in model (given it confuses results by assuning elevation needs to stay constant over time, even though it naturally increases)

M2.2 <- glmmTMB(total_cover_prop_no1 ~ DomSpeciesSuperSimp * poly(YSR,2) + (1|Site), data = salt_comm_rest_H %>% subset(DomSpecies %in% c("Sporobolus", "Sarcocornia")), family = "beta_family")

em3 <- emmip(M2.2, ~ YSR, at = list(YSR = c(6,10,15,20,25,30,33)), type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Predicted proportion cover") +
  geom_jitter(aes(x = YSR, y = total_cover_prop_no1), data = salt_comm_rest_H) +
  ylim(0,1) +
  theme_bw()
...
Make a modified figure for a poster presentation.

```{r Poster_figure}
t.test(salt_comm_nat_H$`Total_cover_%`)$conf.int # calculate natural total cover confidence interval

S_Cov_rest2 <- ggplot(salt_comm_rest_H, aes(x=YSR, y=total_cover_prop*100, group = YSR, fill = Estuary)) +
  geom_hline(yintercept = mean(salt_comm_nat_H$`Total_cover_%`, na.rm = TRUE), linetype = "dashed", linewidth = 1) + # add horizontal line for natural mean cover
  annotate( # add grey box for natural 95% confidence interval
    "rect",
    xmin = -Inf, xmax = Inf,  # Extend the rectangle across the entire x-axis
    ymin = 86, 
    ymax = 97,  # Set the y-range for the rectangle as the 95% confidence interval bounds
    alpha = 0.4,  # Set the transparency level
    fill = "grey"  # Set the fill color
  ) +
  geom_boxplot(width = 2) +
  ...
  xlab("Years since restoration") +
  ylab("Saltmarsh cover (%)") +
  ggtitle("Saltmarsh cover") +
  xlim(0,35) + 
  ylim(0,100) 
S_Cov_rest2

ggsave("C:/Users/z5204897/OneDrive - UNSW/Desktop/PhD/Data_analysis/Project/Functional_traits/Output/Saltmarsh_cover_poster.jpg", width = 2400, height = 2400, units = "px")
```
...
#MODEL LINES
em <- emmeans(M_WB, ~ YSR|DomSpeciesSuperSimp* site_within_elev*site_mean_elev, at = list(YSR = c(6,11,12,16,30,33),site_within_elev = 0,site_mean_elev= 0.32 ), type= "response")

# Also try with polynomial YSR term
M_WB2 <- glmmTMB(total_cover_prop_no1 ~ site_mean_elev + site_within_elev + DomSpeciesSuperSimp + poly(YSR,2) + DomSpeciesSuperSimp:YSR + (1|Site), data = salt_comm_rest_H, family = "beta_family")

anova(M_WB,M_WB2) # non significant so therefore use simpler model with linear YSR term

# model assumptions
resid <- residuals(simulateResiduals(M_WB), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M_WB,re.form = NA), resid) 
plotQQunif(M_WB)
# all good
...
# predicted cover using within-between model

em1 <- emmip(M_WB, ~ YSR, at = list(YSR = c(6,10,15,20,25,30,33)), type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Predicted proportion cover") +
  geom_jitter(aes(x = YSR, y = total_cover_prop_no1), data = salt_comm_rest_H) +
  theme_bw()
...
# create a zero-inflated poisson model, consistent with the restoration status model
M3 <- glmmTMB(Burrows_0.5x0.5 ~ poly(Elevation,2) + DomSpeciesSuperSimp + poly(YSR,2) + (1|Site), data = salt_comm_rest_H_burrows, family = "poisson", ziformula =~1)

resid <- residuals(simulateResiduals(M3), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M3,re.form = NA), resid) # good

#check collinearity on a model without polynomial terms
M0 <- glmmTMB(Burrows_0.5x0.5 ~ Elevation + DomSpeciesSuperSimp + YSR +  (1|Site), data = salt_comm_rest_H_burrows, family = "poisson", ziformula =~1)
check_collinearity(M0)

# compare to null models
M <- glmmTMB(Burrows_0.5x0.5 ~ poly(Elevation,2) + DomSpeciesSuperSimp + YSR +  (1|Site), data = salt_comm_rest_H_burrows, family = "poisson", ziformula =~1)
M0 <- glmmTMB(Burrows_0.5x0.5 ~ poly(Elevation,2) + DomSpeciesSuperSimp +  (1|Site), data = salt_comm_rest_H_burrows, family = "poisson", ziformula =~1)
...
S_Bur_rest <- ggplot(salt_comm_rest_H, aes(x=YSR, y=(Burrows_0.5x0.5+0.01), group = YSR, fill = Estuary)) +
  geom_hline(yintercept = mean(salt_comm_rest_H$Burrows_0.5x0.5, na.rm = TRUE), linetype = "dashed", linewidth = 1) +
  annotate(
    "rect",
    xmin = -Inf, xmax = Inf,  # Extend the rectangle across the entire x-axis
    ymin = 2, 
    ymax = 21,  # Set the y-range for the rectangle as the 95% confidence interval bounds
    alpha = 0.4,  # Set the transparency level
    fill = "grey"  # Set the fill color
  ) +
    geom_boxplot(width = 2) +
...
S_Bur_nat <- ggplot(salt_comm_nat, aes(x=Site, y=Burrows_0.5x0.5, fill=Estuary, group = Site)) +
    geom_hline(yintercept = mean(salt_comm_nat$Burrows_0.5x0.5, na.rm = TRUE), linetype = "dashed", linewidth = 1) +
  annotate(
    "rect",
    xmin = -Inf, xmax = Inf,  # Extend the rectangle across the entire x-axis
    ymin = 4, 
    ymax = 56,  # Set the y-range for the rectangle as the 95% confidence interval bounds
    alpha = 0.4,  # Set the transparency level
    fill = "grey"  # Set the fill color
  ) +
    geom_boxplot(width=0.4) +
  geom_jitter(width = 0.1, height = 0)  +
    scale_fill_viridis(discrete = TRUE, begin = 0.5, end = 0.8, option = "A") +
    theme_classic() +
    theme(legend.position = c(0.5, 50),
          axis.text = element_text(size = 18),
          axis.text.y = element_blank(),# remove y axis labels when combining plot with restored
          legend.text = element_text(size = 16),
...
ggarrange(S_Bur_rest,S_Bur_nat,ncol =2,nrow=1,widths = c(2,1))

ggsave("C:/Users/z5204897/OneDrive - UNSW/Desktop/PhD/Data_analysis/Project/Functional_traits/Output/Saltmarsh_burrows_grey.jpg", width = 2400, height = 1500, units = "px")

emmip(M3, ~ YSR, at = list(YSR = c(6:33)), type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Number of burrows") +
...
```{r Poster-figure}
t.test(salt_comm_nat_H$Burrows_0.5x0.5)$conf.int # calculate natural total cover confidence interval

S_Bur_rest2 <- ggplot(salt_comm_rest_H, aes(x=YSR, y=(Burrows_0.5x0.5+0.01), group = YSR, fill = Estuary)) +
  geom_hline(yintercept = mean(salt_comm_nat_H$Burrows_0.5x0.5, na.rm = TRUE), linetype = "dashed", linewidth = 1) +
  annotate(
    "rect",
    xmin = -Inf, xmax = Inf,  # Extend the rectangle across the entire x-axis
    ymin = 2.1, 
    ymax = 13.3,  # Set the y-range for the rectangle as the 95% confidence interval bounds
    alpha = 0.4,  # Set the transparency level
    fill = "grey"  # Set the fill color
  ) +
    geom_boxplot(width = 2) +
  ...
  xlab("Years since restoration") +
  ylab("Number of burrows") +
  ggtitle("Crab burrows") +
  xlim(0,35) 
S_Bur_rest2

ggsave("C:/Users/z5204897/OneDrive - UNSW/Desktop/PhD/Data_analysis/Project/Functional_traits/Output/Saltmarsh_burrows_poster.jpg", width = 2400, height = 2400, units = "px")
```
...
-   subset for only Sporobolus, Sarcocornia and Triglochin because these are the only species seen consistently across quadrats at sites of different ages
...
```{r Model_building}
M <- glmmTMB(Stems ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:YSR  +  offset(log(Quadrat_area)) + rr(Species+0|Combo, d = 2) + (1|Site), family = "poisson", data = stem_dens_unique_H_res)

resid <- residuals(simulateResiduals(M), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M, re.form = NA), resid) # has a trend, not great but ok
plotQQunif(M) # ok

M5 <- glmmTMB(Stems ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:YSR + offset(log(Quadrat_area)) + rr(Species+0|Combo, d = 2) + (1|Site), family = "nbinom2", data = stem_dens_unique_H_res)
...
M0 <- glmmTMB(Stems ~ DomSpeciesSuperSimp + Species + Elevation + sp_prop + YSR + offset(log(Quadrat_area)) + rr(Species+0|Combo, d = 2) + (1|Site), family = "nbinom2", data = stem_dens_unique_H_res)
...
-   anova for model testing - compare with a model that lacks the Species \* YSR interaction (M) and with a model that lacks YSR terms altogether (M0)
...
M0 <- glmmTMB(Stems ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + offset(log(Quadrat_area)) + rr(Species+0|Combo, d = 2) + (1|Site), family = "nbinom2", data = stem_dens_unique_H_res)
...
-   Create a stems dataset (we'll use this for diameter and height) with the three species that have consistent stem data
...
```{r Model_building}
M <- glmmTMB(Stem_dia_mm ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:YSR + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_dia)

plot(predict(M, re.form = NA), residuals(M)) # ok but clumped
plotQQunif(M) # ok but not great

M6.0 <- glmmTMB(sqrt(Stem_dia_mm) ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:YSR + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_dia)

plot(predict(M6.0, re.form = NA), residuals(M6.0)) # looks ok
plotQQunif(M6.0) # not great but better than M - but these models are robust to deviations from normality for this sample size so it's totally fine.
...
anova(M6.0, M) # no difference, but M6.0 looks better



### create a new model with log normal distribution, consistent with restoration status model

M6 <- glmmTMB(log(Stem_dia_mm) ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:YSR + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_dia)

plot(predict(M6, re.form = NA), residuals(M6)) # looks ok
plotQQunif(M6)

# check collinearity
M0 <- glmmTMB(log(Stem_dia_mm) ~ DomSpeciesSuperSimp + Species + Elevation + sp_prop + YSR + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_dia)
...
M0 <- glmmTMB(sqrt(Stem_dia_mm) ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop  + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_dia)

anova(M0, M6.0)

summary(M6.0)

em <- emtrends(M6.0, var = "YSR", specs = ~Species)
...
emmeans(M6.0, ~ YSR | Species, at = list(YSR = c(6,11,12,16,30,33)), type = "response")


## with restoration-status consistent model

M6 <- glmmTMB(log(Stem_dia_mm+0.2) ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:YSR + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_dia)

M0 <- glmmTMB(log(Stem_dia_mm+0.2) ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop  + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_dia)
...
emmip(M, ~ YSR, at = list(YSR = c(6:33)), type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Stem diameter (mm)") +
  geom_jitter(aes(x = YSR, y = Stem_dia_mm), data = salt_H_rest_stems_dia) +
  theme_bw()
...
-   Try log-transformed data as raw data are slightly right-skewed

    -   log-transformed explains more variance so use that
...
```{r Model_building}
M <- glmmTMB(Stem_ht_mm ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:YSR + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht)

plot(predict(M), residuals(M)) # clumped but ok
plotQQunif(M) # great

M7.0 <- glmmTMB(log(Stem_ht_mm) ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:YSR +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht)

plot(predict(M7.0), residuals(M7)) # clumped but ok
plotQQunif(M7.0) # great

anova(M,M7.0) # M7.0 is better

M0 <- glmmTMB(log(Stem_ht_mm) ~ DomSpeciesSuperSimp + Species + Elevation + sp_prop + YSR +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht) 

check_collinearity(M0) # all low


### create a model that's consistent with restoration status model - square root normal

M7 <- glmmTMB(sqrt(Stem_ht_mm) ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:YSR +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht)

plot(predict(M7), residuals(M7)) # good
plotQQunif(M7) # great
...
```{r Model_testing}
M7.0 <- glmmTMB(log(Stem_ht_mm) ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:YSR +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht)
M0 <- glmmTMB(log(Stem_ht_mm) ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht)

anova(M7.0, M0)
summary(M7.0)

em <- emtrends(M7.0, var = "YSR", specs = ~Species, type = "response")

em

plot(em)

emmeans(M7.0, ~ YSR | Species, at = list(YSR = c(6,11,12,16,30,33)), type = "response")


## with model that's consistent with restoration status model

M7 <- glmmTMB(sqrt(Stem_ht_mm) ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:YSR +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht)
M0 <- glmmTMB(sqrt(Stem_ht_mm) ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht)
...
```{r Model_building}
M8 <- glmmTMB(Reproduction ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:(reprod_type * YSR) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)
...
```{r Model_testing}
M8 <- glmmTMB(Reproduction ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:(reprod_type * YSR) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)
M <- glmmTMB(Reproduction ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:(reprod_type + YSR) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)
M0 <- glmmTMB(Reproduction ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:reprod_type + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)
...
em <- emtrends(M8, var = "YSR", specs = ~Species | reprod_type)
...
M8.5 <- glmmTMB(Elevation ~ Estuary + DomSpeciesSuperSimp + rest_status + (1|Site), family = "gaussian", data = salt_comm)
...
check_collinearity(M8.5)

M0 <- glmmTMB(Elevation ~ Estuary + DomSpeciesSuperSimp + (1|Site), family = "gaussian", data = salt_comm)

anova(M8.5,M0)

emmeans(M8.5, ~rest_status)
```
...
```{r Model_create}
M <- glmmTMB((Sp_cover_prop + 0.000001) ~ Estuary + Species:Elevation + Species:rest_status + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = beta_family(link = "logit"), data = salt_comm_comp_long_less)
...
M9 <- glmmTMB(Sp_cover_prop ~ Estuary + Species:Elevation + Species:rest_status + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = beta_family(link = "logit"), ziformula = ~ 1, data = salt_comm_comp_long_less)
...
hist(log(1 - salt_comm$total_cover_prop)) 
...
```{r Model_create}
M10 <- glmmTMB(total_cover_prop_offset ~ Estuary + DomSpeciesSuperSimp + Elevation + rest_status + (1|Site), family = "beta_family", data = salt_comm)
...
```{r Model_test}
M10 <- glmmTMB(total_cover_prop_offset ~ Estuary + DomSpeciesSuperSimp + Elevation + rest_status + (1|Site), family = "beta_family", data = salt_comm)
M0 <- glmmTMB(total_cover_prop_offset ~ Estuary + DomSpeciesSuperSimp + Elevation + (1|Site), family = "beta_family", data = salt_comm)
...
emmeans(M10, pairwise ~ rest_status + DomSpeciesSuperSimp, type = "response")
```
...
```{r Model_create}
M <- glmmTMB(Burrows_0.5x0.5 ~ Estuary + DomSpeciesSuperSimp + Elevation + rest_status + (1|Site), data = salt_comm_burrows, family = "poisson")
...
M11 <- glmmTMB(Burrows_0.5x0.5 ~ Estuary + DomSpeciesSuperSimp + Elevation + rest_status + (1|Site), data = salt_comm_burrows, family = "poisson", ziformula =~1)
...
M1 <- glmmTMB(Burrows_0.5x0.5 ~ Estuary + DomSpeciesSuperSimp + Elevation + rest_status + (1|Site), data = salt_comm_burrows, family = "nbinom2")

resid <- residuals(simulateResiduals(M1), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M1, re.form = NA), resid)
check_collinearity(M1) # low collinearity
plotQQunif(M1)
...
```{r Model_test}
M0 <- glmmTMB(Burrows_0.5x0.5 ~ Estuary + DomSpeciesSuperSimp + Elevation + (1|Site), data = salt_comm_burrows, family = "poisson", ziformula =~1)
...
```{r Plot}
ggplot(salt_comm, aes(x=Estuary, y=total_cover_prop*100, fill = as.factor(DomSpeciesSuperSimp))) +
    geom_boxplot() +
  geom_jitter(size = 3)  +
    scale_fill_viridis(discrete = TRUE, begin = 0.5, end = 0.8, option = "A",
                      name="",
...
-   Subset for only species with consistent data across sites and remove NAs
...
cor(stem_dens_unique_less$total_stems, stem_dens_unique_less$sp_prop)
```
...
```{r Model_create}
M <- glmmTMB(Stems ~ Estuary + Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:rest_status + (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "poisson", data = stem_dens_unique_less)
...
M12 <- glmmTMB(Stems ~ Estuary + Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:rest_status + (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "nbinom2", data = stem_dens_unique_less)
...
scatter.smooth(predict(M12, re.form = NA), abs(resid))
plotQQunif(M12)
## use this one

M0 <- glmmTMB(sqrt(Stems/Quadrat_area) ~ Estuary + Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:rest_status +  (1|Site/Combo) + rr(Species+0|Combo, d = 1), family = "gaussian", data = stem_dens_unique_less)
...
Mx <- glmmTMB(sqrt(Stems) ~ Estuary + Species + Elevation + sp_prop + rest_status +  (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "gaussian", data = stem_dens_unique_less)
...
```{r Model_test}
M12 <- glmmTMB(sqrt(Stems) ~ Estuary + Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:rest_status +  (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "gaussian", data = stem_dens_unique_less)

M <- glmmTMB(sqrt(Stems) ~ Estuary + Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "gaussian", data = stem_dens_unique_less)

anova(M12,M)


joint_tests(M12)
emmeans(M12, ~rest_status | Species, type = "response", offset = log(10000))
summary(M12)


## use negative bimomial model

M12 <- glmmTMB(Stems ~ Estuary + Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:rest_status + (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "nbinom2", data = stem_dens_unique_less)

M <- glmmTMB(Stems ~ Estuary + Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "nbinom2", data = stem_dens_unique_less)
...
-   subset for only Sporobolus, Sarcocornia, and Triglochin as these are the only species that exist in both natural and restored sites across multiple sites
...
```{r Model_create}
M13 <- glmmTMB(log(Stem_dia_mm) ~ Estuary + Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:rest_status +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
...
```{r Model_test}
M0 <- glmmTMB(log(Stem_dia_mm) ~ Estuary + Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
...
```{r Model_create}
M14 <- glmmTMB(sqrt(Stem_ht_mm) ~ Estuary + Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:rest_status +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
...
```{r Model_test}
M0 <- glmmTMB(sqrt(Stem_ht_mm) ~ Estuary + Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
...
-   subset for only Hunter estuary (not much Maroochy data, and the inclusion of the term Estuary is causing model convergence issues)
...
reprod_all_long_H <- reprod_all_long %>% subset(Estuary == "Hunter") 
...
```{r Model_create}
M15 <- glmmTMB(Reproduction ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:(reprod_type * rest_status) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long_H)
...
M <- glmmTMB(Reproduction ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:(reprod_type + rest_status) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long_H)

M0 <- glmmTMB(Reproduction ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:reprod_type + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long_H)
...
# confidence intervals etc don't mean much unless split up by reproduction type
...
```{r Plot}
reprod_proportions_H <- reprod_all_long_H %>% 
...
ggplot(aes(x = rest_status, y = avg, group = rest_status), data = reprod_proportions_H) +
  geom_boxplot() +
  facet_wrap(~reprod_type + Species)
```

P value adjustment

```{r}
salt_main_raw_p <- tibble(variable = c(),
                         test = c(),
                         pvalue = c())
```
