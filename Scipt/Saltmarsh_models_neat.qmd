...
M1 <- glmmTMB(cover_prop_no1 ~ Species:(Elevation + YSR) + (1|Site) + rr(Species+0|Combo, d = 1), data = comm_comp_long, family = beta_family(link = "logit"), ziformula = ~1)

# check assumptions
resid <- residuals(simulateResiduals(M1), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M1,re.form = NA), resid) # wonderful!
plotQQunif(M1) # wonderful
...
```{r Model_building}
M <- glmmTMB(Stems ~ DomSpeciesSuperSimp + Species:(Elevation + sp_prop + YSR) +  offset(log(Quadrat_area)) + rr(Species+0|Combo, d = 2) + (1|Site), family = "poisson", data = stem_dens_unique_H_res)

resid <- residuals(simulateResiduals(M), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M, re.form = NA), resid) # has a trend, not great but ok
plotQQunif(M) # ok

M5 <- glmmTMB(Stems ~ DomSpeciesSuperSimp + Species:(Elevation + sp_prop + YSR)+ offset(log(Quadrat_area)) + rr(Species+0|Combo, d = 2) + (1|Site), family = "nbinom2", data = stem_dens_unique_H_res)
...
M6 <- glmmTMB(sqrt(Stem_dia_mm) ~ DomSpeciesSuperSimp + Species:(Elevation + sp_prop + YSR) + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_dia)
...
```{r Model_building}
# M <- glmmTMB(Stem_ht_mm ~ Elevation + sp_prop + DomSpeciesSuperSimp + Species*YSR +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht)

M7 <- glmmTMB(log(Stem_ht_mm) ~ DomSpeciesSuperSimp + Species:(Elevation + sp_prop + YSR) +  (1|Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht)
...
-   collinearity of predictors using check_collinearity on a model without interactions (M0)

```{r Model_building}
M8 <- glmmTMB(Reproduction ~ Elevation +  Species:(reprod_type * YSR) + reprod_type * YSR + (1+reprod_type|Species) + (1|Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)
...
M0 <- glmmTMB(Reproduction ~ Elevation +  Species + reprod_type + YSR + reprod_type * YSR + (1+reprod_type|Species) + (1|Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)
...
-   include interactions between Estuary and restoration status, and between Elevation and restoration status, because we could expect these interactions may impact burrow numbers
...
```{r Model_create}
M <- glmmTMB(Burrows_0.5x0.5 ~ DomSpeciesSuperSimp + Estuary * rest_status + Elevation * rest_status + (1|Site), data = salt_comm_burrows, family = "poisson")
...
M11 <- glmmTMB(Burrows_0.5x0.5 ~ DomSpeciesSuperSimp + Estuary * rest_status + Elevation * rest_status + (1|Site), data = salt_comm_burrows, family = "poisson", ziformula =~1)
