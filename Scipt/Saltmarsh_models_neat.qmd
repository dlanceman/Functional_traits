...
M1 <- glmmTMB(cover_prop_no1 ~ Species + Species:Elevation + Species:YSR + YSR + (1|Site) + rr(Species+0|Combo, d = 1), data = comm_comp_long, family = beta_family(link = "logit"), ziformula = ~1)
...
Mx <- glmmTMB(cover_prop_no1 ~ Species:(Elevation + YSR) + (1|Site) + rr(Species+0|Combo, d = 1), data = comm_comp_long, family = beta_family(link = "logit"), ziformula = ~1)

resid <- residuals(simulateResiduals(Mx), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(Mx,re.form = NA), resid) # wonderful!
plotQQunif(Mx)
...
-   joint_tests to test effect of each variable (p values)
...
joint_tests(M1)
emmeans(M1, specs = pairwise ~ YSR | Species, type = "response")
...
em <- emmeans(M1, specs = pairwise ~ Species | YSR, adjust = "tukey")
summary(em, infer = TRUE)$contrasts
```
...
-   check covariance between predictors using check_collinearity
...
```{r Model}
M2 <- glmmTMB((total_cover_prop - 0.001) ~ poly(Elevation,2) + DomSpeciesSuperSimp + poly(YSR,2) + (1|Site), data = salt_comm_rest_H, family = "beta_family")
...
emmip(M2, ~ poly(YSR,2), at = list(YSR = c(6,10,15,20,25,30,33)), type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Predicted proportion cover") +
  geom_jitter(aes(x = YSR, y = cover_prop), data = comm_comp_long) +
  theme_bw()
```
...
anova(M3,M4) # no evidence of difference, so use M4
...
-   emmeans for parameter estimates. Include offset = log(10000) so we're looking at results in terms of stem density in a 100 x 100 cm quadrat
...
joint_tests(M5)
summary(M5)
emmeans(M5, ~ Species | YSR, type = "response", offset = log(10000))
...
```{r Post-hoc}
ggplot(data = stem_dens_unique_H_res, aes(x = YSR, y = total_stems, group = YSR)) + 
  geom_boxplot() +   
  facet_wrap(~Species, scales = "free")  
...
em <- emmeans(M5, specs = pairwise ~ Species | YSR, adjust = "tukey") 
summary(em, infer = TRUE)$contrasts
```
...
-   We are interested in how reproduction changes for each species considering all other factors, so create interactions with as many factors as possible (take factors outside of the brackets if the model doesn't run with them inside).
...
-   collinearity of predictors using check_collinearity on a model without interactions (M0) **(having issues with model convergence)**

```{r Model_building}
M8 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Species*(Elevation  + reprod_type * YSR) + (1+reprod_type|Species) + (1|Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)
...
joint_tests(M8)
summary(M8)
emmeans(M8, ~ Species | YSR | reprod_type, type = "response")
```
...
```{r Plot}
k <- emtrends(M9, var = "Elevation", ~Species, type = "response")
k
plot(k) 
...
joint_tests(M12)
emmeans(M12, pairwise~rest_status|Species)
emmeans(M12, pairwise~Elevation|Species)
summary(M12)
```
...
```{r Model_create}
M14 <- glmmTMB(sqrt(Stem_ht_mm) ~ Estuary + DomSpeciesSuperSimp + Species + Species*(Elevation + sp_prop + rest_status) +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
...
```{r Model_create}
M15 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + sp_prop + Estuary + Species*(Elevation + rest_status * reprod_type) + (1+reprod_type|Species) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long)
