...
```{r Model_building}
M16 <- glmmTMB(Stems ~ DomSpeciesSuperSimp + Elevation + YSR + offset(log(Quadrat_area)) + (1|Site), family = "nbinom2", data = stem_dens_rest_spor)
...
M16.0 <- glmmTMB(Stems ~ DomSpeciesSuperSimp + Elevation + poly(YSR,2) + offset(log(Quadrat_area)) + (1|Site), family = "nbinom2", data = stem_dens_rest_spor)
anova(M16,M16.0) # no better, stick with linear

M16.1 <- glmmTMB(Stems ~ DomSpeciesSuperSimp + Elevation + YSR + offset(log(Quadrat_area)) + (1|Site), family = "poisson", data = stem_dens_rest_spor)
