...
Now, let's run some post-hoc tests (multiple comparisons) to see where differences lie - which species are changing with YSR?

-   Plot differences using ggplot

-   **Need to code for multiple comparisons** (currently not working and/or not testing what we want to test)

```{r Post-hoc}
ggplot(data = comm_comp_long, aes(x = YSR, y = cover_prop_no1, group = YSR)) +
  geom_boxplot() +
...
emmip(M1, ~ YSR | Species, at = list(YSR = c(6,10,15,20,25,30,33)), CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Predicted proportion cover", type = "response") +
...
-   there are insufficient data across the matrix to model interactions between any of the predictors
...
em1 <- emmip(M2, ~ YSR | Elevation, at = list(YSR = c(6,10,15,20,25,30,33), Elevation = c(-0.2,0,0.2,0.4,0.6,0.8)), type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Predicted proportion cover") 
em1
...
```{r Model_building}
M8 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species*(reprod_type * YSR) + (1+reprod_type|Species) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)
...
M0 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Species + Elevation +   reprod_type + YSR  + (1+reprod_type|Species) + (1|Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)
...
```{r Model_testing}
M1 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species*(reprod_type + YSR) + (1+reprod_type|Species) + (1|Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)

M <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species*reprod_type + YSR + (1+reprod_type|Species) + (1|Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)

M0 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Species*reprod_type  + (1+reprod_type|Species) + (1|Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod.s_sf)
...
```{r Model_create}
M15 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Estuary + Species*(reprod_type * rest_status) + (1+reprod_type|Species) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long)

M16 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Estuary + Species*(rest_status) + (1|Site/Combo), family = "binomial", data = reprod_all_long)

resid <- residuals(simulateResiduals(M15), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M15)) 
plotQQunif(M15) 

resid <- residuals(simulateResiduals(M16), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M16)) 
plotQQunif(M16) 
```
...
```{r Model_test}
M1 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + sp_prop + Estuary + Species*(Elevation + rest_status + reprod_type) + (1+reprod_type|Species) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long)

M <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + sp_prop + Estuary + Species*(Elevation + rest_status) + reprod_type + (1+reprod_type|Species) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long)

M0 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + sp_prop + Estuary + Species*(Elevation + reprod_type) + (1+reprod_type|Species) + (1|Site/Combo), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long)

anova(M15,M1,M,M0)

#Mx <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Estuary + Species + rest_status + (1|Site/Combo), family = "binomial", data = reprod_all_long)
# this model is not working

Mx0 <- glmmTMB(Reproduction ~ DomSpeciesSuperSimp + Elevation + Estuary + Species + (1|Site/Combo), family = "binomial", data = reprod_all_long)

anova(M16,Mx0)

joint_tests(M16)

emmeans(M16, ~ rest_status, type = "response")
emmeans(M16, ~ rest_status | Species, type = "response")
```
