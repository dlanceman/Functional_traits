...
M6 <- glmmTMB(sqrt(Stem_dia_mm) ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:YSR + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_dia)
...
anova(M6, M) # no difference, but M6 looks better
```
...
M7 <- glmmTMB(log(Stem_ht_mm) ~ Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:YSR +  (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht)

plot(predict(M7), residuals(M7)) # clumped but ok
plotQQunif(M7) # great

anova(M,M7) # M7 is better
...
M0 <- glmmTMB(Stems ~ Estuary + Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:rest_status + (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "nbinom2", data = stem_dens_unique_less)
...
scatter.smooth(predict(M0, re.form = NA), abs(resid))
plotQQunif(M0)

M12 <- glmmTMB(sqrt(Stems/Quadrat_area) ~ Estuary + Species + DomSpeciesSuperSimp + Species:Elevation + Species:sp_prop + Species:rest_status +  (1|Site/Combo) + rr(Species+0|Combo, d = 1), family = "gaussian", data = stem_dens_unique_less)
