...
```{r Model_building}
# check collinearity on model without interactions
M0 <- glmmTMB(LMA ~ Estuary + Tree_sapling + YSR, family = "gaussian", data = manleaf_wide_grey_rest)
check_collinearity(M0)
...
## use beta distribution model, consistent with restoration status model

M16.4 <- glmmTMB(Herb_prop ~ Estuary + YSR + (1|Site/Combo/Tree), family = "beta_family", data = manleaf_wide_grey_rest_t) 

resid <- residuals(simulateResiduals(M16.4), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M16.4,re.form = NA), resid) 
scatter.smooth(predict(M16.4,re.form = NA), abs(resid)) 
plotQQunif(M16.4)


summary(M16.4)

summary(manleaf_wide_grey_rest_t$Herb_prop)
```
...
# beta model, consistent with restoration status model
M16.4 <- glmmTMB(Herb_prop ~ Estuary + YSR + (1|Site/Combo/Tree), family = "beta_family", data = manleaf_wide_grey_rest_t) 
M <- glmmTMB(Herb_prop ~ Estuary + (1|Site/Combo/Tree), family = "beta_family", data = manleaf_wide_grey_rest_t) 
...
emmeans(M16.4, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
emmeans(M16.4, ~ Estuary, type = "response")
```
...
M31.5 <- glmmTMB(Herb_prop ~ Estuary + Tree_sapling + Species * rest_status + Estuary:rest_status + (1|Site/Combo/Tree), family = "tweedie", data = manleafandherb_nored)
