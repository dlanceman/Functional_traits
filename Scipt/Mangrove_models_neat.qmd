...
m_Ht_rest <- ggplot(mantree_grey_rest, aes(x=YSR, y=Height_m, fill=Estuary, group = YSR)) +
  ...
  annotate(
    ...
    xmin = -Inf, xmax = Inf,  # Extend the rectangle across the entire x-axis
    ymin = 4.6, 
    ymax = 5.7,  # Set the y-range for the rectangle as the 95% confidence interval bounds
    alpha = 0.4,  # Set the transparency level
    fill = "grey"  # Set the fill color
  ) +
    geom_boxplot(width = 2) +
...
m_Ht_nat <- ggplot(mantree_grey_nat, aes(x=Site, y=Height_m, fill=Estuary, group = Site)) +
  ...
  annotate(
    ...
    xmin = -Inf, xmax = Inf,  # Extend the rectangle across the entire x-axis
    ymin = 5.4, 
    ymax = 6.9,  # Set the y-range for the rectangle as the 95% confidence interval bounds
    alpha = 0.4,  # Set the transparency level
    fill = "grey"  # Set the fill color
  ) +
    geom_boxplot(width = 0.4) +
...
```{r Model_testing}
M14.2 <- glmmTMB(LMA ~ YSR + (1|Site/Combo/Tree_ID), family = "gaussian", data = manleaf_wide_grey_rest)
M <- glmmTMB(LMA ~ (1|Site/Combo/Tree_ID), family = "gaussian", data = manleaf_wide_grey_rest)
...
emmeans(M14.2, ~YSR, at = list(YSR = c(5,9,12,16,30,33)))
```
...
# use normal model (non-transformed), consistent with restoration status model

M15.2 <- glmmTMB(Area ~ Estuary + Tree_sapling + YSR + (1|Site/Combo/Tree), family = "gaussian", data = manleaf_wide_grey_rest)

resid <- residuals(simulateResiduals(M15.2), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M15.2,re.form = NA), resid) 
plotQQunif(M15.2)
```
...
# normal model, consistent with restoration status model

M15.2 <- glmmTMB(Area ~ Estuary + Tree_sapling + YSR + (1|Site/Combo/Tree), family = "gaussian", data = manleaf_wide_grey_rest)
M <- glmmTMB(Area ~ Estuary + Tree_sapling + (1|Site/Combo/Tree), family = "gaussian", data = manleaf_wide_grey_rest)
...
emmeans(M15.2, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
```
...
emmeans(M21.7, ~ rest_status, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
emmeans(M21.7, ~ rest_status | Species, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
emmeans(M21.7, ~ rest_status | Estuary, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
emmeans(M21.7, ~ rest_status | Species | Estuary, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
```
