...
-   try including an interaction between estuary and elevation. This doesn't work for the nontransformed model, and the sqrt transformed model performs better without it, so use the sqrt model without the interaction
...
M11.3 <- glmmTMB(sqrt(Pneu_dia_mm) ~ Estuary * poly(Elevation,2) + poly(YSR, 2) + (1|Site/Combo/Subquadrat), family = "gaussian", data = manq_rest)
...
-   try an interaction between elevation and estuary, however there is only one site in the Maroochy so this likely does not mean much. The interaction did not improve the model, so don't use it.
...
M13.2 <- glmmTMB(dfratio ~ Estuary * poly(Elevation,2) + Tree_sapling + YSR + (1|Site/Combo/Tree), family = "gaussian", data = manleaf_wide_grey_rest)
...
-   try with estuary/elevation interaction - it improves the model, so keep it
...
M14.2 <- glmmTMB(LMA ~ Estuary * poly(Elevation,2) + Tree_sapling + YSR + (1|Site/Combo/Tree_ID), family = "gaussian", data = manleaf_wide_grey_rest)
...
# assumptions plots look good, warning message is ok
```
...
-   only using tree data because using all data and including "Tree_sapling" as a factor comes up with a model convergence problem
...
# looks fine
# try with elevation * estuary interaction

M21.2 <- glmmTMB(Cover_prop ~ Estuary * Elevation + rest_status + Estuary:rest_status + (1|Site), family = "beta_family", data = mantree_comm)

resid <- residuals(simulateResiduals(M21.2), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M21.2,re.form = NA), resid) 
plotQQunif(M21.2)

anova(M21,M21.2)

# no difference between these so stick with simpler model without interaction
```
...
# test whether adding species * elevation or removing est*elev interaction improves the model

M21.7 <- glmmTMB(Stems ~ Estuary * Elevation + Species + Species:Elevation + rest_status + Species : rest_status + Estuary:rest_status + (1|Site/Combo), family = "nbinom2", data = mantree_nored)
...
M21.8 <- glmmTMB(Stems ~ Estuary + Elevation + Species * rest_status + Estuary:rest_status + (1|Site/Combo), family = "nbinom2", data = mantree_nored)

resid <- residuals(simulateResiduals(M21.8), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M21.8,re.form = NA), resid) 
plotQQunif(M21.8)

anova(M21.7,M21.8,M21.6) # neither improves the model, stick with 21.6
```
...
# log is better
# check whether including elev*est interaction improves the model

M22.2 <- glmmTMB(log(DBH_cm) ~ Estuary * Elevation + Species * rest_status + Estuary: rest_status + (1|Site/Combo), family = "gaussian", data = mantree_nored)

resid <- residuals(simulateResiduals(M22.2), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M22.2,re.form = NA), resid) 
plotQQunif(M22.2)

anova(M22,M22.2)
# no difference, so use simpler model

# also check whether species * elevation improves the model

M22.3 <- glmmTMB(log(DBH_cm) ~ Estuary + Elevation * Species + rest_status + Species:rest_status + Estuary: rest_status + (1|Site/Combo), family = "gaussian", data = mantree_nored)

resid <- residuals(simulateResiduals(M22.3), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M22.3,re.form = NA), resid) 
plotQQunif(M22.3)

anova(M22,M22.3) # no difference, so use simpler model
```
...
# test whether Elev * Est interaction improves the model

M23.3 <- glmmTMB(sqrt(Height_m) ~ Estuary * Elevation + Species * rest_status + Estuary : rest_status + (1|Site/Combo), family = "gaussian", data = mantree_nored)
...
anova(M23.3,M23.2)
# no improvement, therefore don't use it

# also test whether Elev * Species interaction improves the model

M23.4 <- glmmTMB(sqrt(Height_m) ~ Estuary + Elevation * Species + rest_status + Species:rest_status + Estuary : rest_status + (1|Site/Combo), family = "gaussian", data = mantree_nored)

resid <- residuals(simulateResiduals(M23.4), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M23.4,re.form = NA), resid) 
plotQQunif(M23.4)

anova(M23.4,M23.2)
# improves the model, therefore use it
```
...
emmeans(M23.4, ~ rest_status | Species, type = "response")  
emmeans(M23.4, ~ rest_status | Estuary, type = "response") 
```
...
# also check whether the elev * est interaction improves the model
...
anova(M24.4,M24.3)
# marginal difference, but residuals of the interaction plot look slightly better so use it
```
...
# see if elev * est interaction improves the model

M25.3 <- glmmTMB(sqrt(Pneu_ht_mm) ~ Estuary * poly(Elevation,2) + rest_status + Estuary:rest_status + (1|Site/Combo/Subquadrat), family = "gaussian", data = manq)
...
anova(M25.3,M25.2)
# no difference, stick with simpler model
```
...
# check if adding est * elev interaction improves the model

M28.1 <- glmmTMB(dfratio ~ Estuary * poly(Elevation,2) + Tree_sapling + rest_status + (1|Site/Combo/Tree), family = "gaussian", data = manleaf_wide_grey)
...
anova(M28,M28.1)
# no difference so stick with original
```
...
```{r Model_building}
M30.1 <- glmmTMB(Area ~ Estuary + Species*Elevation + Estuary:Elevation + Tree_sapling + rest_status + Species:rest_status + (1|Site/Combo/Tree), family = "gaussian", data = manleafandherb_nored)
...
M30.2 <- glmmTMB(log(Area) ~ Estuary + Species*Elevation + Estuary:Elevation + Tree_sapling + rest_status + Species:rest_status + (1|Site/Combo/Tree), family = "gaussian", data = manleafandherb_nored)
...
# check if adding a rest_status * estuary interaction improves the model

M30.3 <- glmmTMB(log(Area) ~ Estuary + Species*Elevation + Estuary:Elevation + Tree_sapling + rest_status + Species:rest_status + Estuary:rest_status + (1|Site/Combo/Tree), family = "gaussian", data = manleafandherb_nored)
...
anova(M30.3,M30.2)
# does not improve model, use without interaction

# also check if the Species*Elevation improved the model


M30.4 <- glmmTMB(log(Area) ~ Estuary + Species + Elevation + Estuary:Elevation + Tree_sapling + rest_status + Species:rest_status + (1|Site/Combo/Tree), family = "gaussian", data = manleafandherb_nored)
...
anova(M30.4,M30.2)
# it does improve the model so keep it
```
...
# check whether rest_status * estuary or elev * species are important

M31.3 <- glmmTMB(Herb_prop ~ Estuary + Species + Elevation + Estuary:Elevation + Tree_sapling + rest_status + Species:rest_status + (1|Site/Combo/Tree), family = "beta_family", data = manleafandherb_nored)
  
resid <- residuals(simulateResiduals(M31.3), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M31.3,re.form = NA), resid) 
plotQQunif(M31.3)
...
M31.5 <- glmmTMB(Herb_prop ~ Estuary + Estuary:Elevation + Tree_sapling + rest_status + Species:rest_status + Estuary:rest_status + (1|Site/Combo/Tree), family = "beta_family", data = manleafandherb_nored)
