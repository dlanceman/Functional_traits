...
ggsave("C:/Users/z5204897/OneDrive - UNSW/Desktop/PhD/Data_analysis/Project/Functional_traits/Output/Mangrove_saplings.jpg", width = 4000, height = 3000, units = "px")
...
-   test assumptions using resid and scatter.smooth, plotQQunif and check_collinearity on a model without interactions
    ...
    -   sqrt and log transformed models looks better than the origianl and have significantly lower AICs. still somewhat fan-shaped but hopefully ok. log has the lowest AIC (signifcantly) so use that
...
emmip(M8.2, ~ YSR, at = list(YSR = c(5:33)), type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Tree height") +
...
-   It creates a NaN error with Site and Tree_ID as a random effect, but that should be ok
    -   doesn't run with Tree_sapling as a factor either

```{r Model_building}
M14 <- glmmTMB(LMA ~ Estuary + poly(Elevation,2) + YSR + (1|Site/Combo/Tree_ID), family = "gaussian", data = manleaf_wide_grey_rest)
...
```{r Model_testing}
M14 <- glmmTMB(LMA ~ Estuary + poly(Elevation,2) + YSR + (1|Site/Combo/Tree_ID), family = "gaussian", data = manleaf_wide_grey_rest)
M <- glmmTMB(LMA ~ Estuary + poly(Elevation,2) + (1|Site/Combo/Tree_ID), family = "gaussian", data = manleaf_wide_grey_rest)
...
M16.2 <- glmmTMB(Herb_prop ~ Estuary * Elevation + YSR + (1|Site/Combo/Tree), family = "tweedie", data = manleaf_wide_grey_rest) 
...
# tweedie distribution is better than beta - qq plot still looks weird but the residuals plot is good at least
```
...
```{r Model_testing}
M16.2 <- glmmTMB(Herb_prop ~ Estuary * Elevation + YSR + (1|Site/Combo/Tree), family = "tweedie", data = manleaf_wide_grey_rest) 
M <- glmmTMB(Herb_prop ~ Estuary * Elevation + (1|Site/Combo/Tree), family = "tweedie", data = manleaf_wide_grey_rest) 
...
anova(M21.5,M21.6) #M21.6 is better, diagnostics plots are good enough, use this
```
...
```{r Model_building}
M29 <- glmmTMB(LMA ~ Estuary + poly(Elevation,2) + rest_status + (1|Site/Combo/Tree_ID), family = "gaussian", data = manleaf_wide_grey)
...
M0 <- glmmTMB(LMA ~ Estuary + poly(Elevation,2) + (1|Site/Combo/Tree_ID), family = "gaussian", data = manleaf_wide_grey)
...
```{r Prep/explore}
manleafandherb <- manleafandherb %>% 
  mutate(Herb_prop = case_when(
    `Herb_%` == 0 ~ 0.00001,
    .default = `Herb_%`/100
  ))

hist(manleafandherb$`Herb_%`) # right skewed
hist(sqrt(manleafandherb$`Herb_%`)) # still right skewed
hist(log(manleafandherb$`Herb_%`)) # still right skewed

ggplot(manleafandherb, aes(x = `Herb_%`)) +
  geom_histogram() +
  facet_wrap(~Species)

ggplot(manleafandherb, aes(x = rest_status, y = `Herb_%`, group = rest_status)) +
  geom_boxplot() +
  facet_wrap(~Species)

plot(manleafandherb$`Herb_%` ~ manleafandherb$Elevation)

ggplot(manleafandherb, aes(x = Elevation, y = `Herb_%`)) +
  geom_point() +
  geom_smooth(method = "lm") +
  facet_wrap(~Species) # look like different trends, so include a species interaction, but just keep as a linear (not polynomial) term
```
...
-   Note, currently using a dataset that has red mangroves, but these are only at natural sites, so we can't test for an effect of restoration status, so it might be pointless to include them

```{r Model_building}
M31.1 <- glmmTMB(Herb_prop ~ Estuary + Species*Elevation + Estuary:Elevation + rest_status + Species:rest_status + (1|Site/Combo/Tree), family = "beta_family", data = manleafandherb)
...
M31.2 <- glmmTMB(`Herb_%` ~ Estuary + Species*Elevation + Estuary:Elevation + rest_status + Species:rest_status + (1|Site/Combo/Tree), family = "tweedie", data = manleafandherb)
...
```{r Model_testing}
M31.1 <- glmmTMB(Herb_prop ~ Estuary + Species*Elevation + Estuary:Elevation + rest_status + Species:rest_status + (1|Site/Combo/Tree), family = "beta_family", data = manleafandherb)
M <- glmmTMB(Herb_prop ~ Estuary + Species*Elevation + Estuary:Elevation + rest_status + (1|Site/Combo/Tree), family = "beta_family", data = manleafandherb)
M0 <- glmmTMB(Herb_prop ~ Estuary + Species*Elevation + Estuary:Elevation + (1|Site/Combo/Tree), family = "beta_family", data = manleafandherb)
...
emmeans(M31.1, ~ rest_status | Species, type = "response")
```
...
emmip(M31.1, ~ rest_status | Species, type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Leaf herbivory (proportion)")
```
