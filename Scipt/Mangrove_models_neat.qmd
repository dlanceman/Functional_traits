...
## use a negative binomial model, consistent with restoration status model
...
emmeans(M6.3, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")

## use negative binomial model, consistent with restoration status model
...
emmeans(M8.31, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")



## model consistent with restoration status model
...
emmeans(M11.3, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")


# log normal model, consistent with restoration status model
...
Don't include Estuary as a factor, because the Maroochy estuary only had one site where we could measure leaf weights (so therefore can't compare restored and natural sites there).
...
emmip(M15.2, ~ YSR, at = list(YSR = c(6:33)), type = "response", CIs = TRUE)
...
emmip(M16.4, ~ YSR, at = list(YSR = c(6:33)), type = "response", CIs = TRUE)
...
emmeans(M19, ~ rest_status | Estuary, type = "response", offset = log(100))  
...
emmeans(M19.3, ~ rest_status | Estuary, type = "response")  
...
## use Log because without, there are model convergence issues
...
# beta model looks better but still weird (not sure if this is true...)
...
emmeans(M31.5, ~ rest_status | Species, type = "response")
emmeans(M31.5, ~ rest_status | Species | Estuary, type = "response")
emmeans(M31.5, ~ rest_status | Estuary, type = "response")
...
Given that we've done a lot of testing, our likelihood of falsely finding significant results is inflated. Therefore, we need to adjust our p-values to account for this.

```{r p_value_adjust_Stage_1}
man_int_raw_p <- tibble(variable = c("Trees", "Saplings", "Seedling_sm", "Seedling_med", "Hollows", "Canopy_cover", "Stems", "DBH", "Ht", "Stems", "DBH", "Ht", "Pneu", "Pneu_ht", "Pneu_dia", "Leaf_area", "Leaf_herb", "Leaf_area", "Leaf_herb"), 
                        interaction = c("Est", "Est", "Est", "Est", "Est", "Est", "Est", "Est", "Est", "Species", "Species", "Species", "Est", "Est", "Est", "Est", "Est", "Species", "Species"), 
                        pvalue = c(0.1047, 0.46677, 0.2369, 0.698, 0.1817, 0.01876, 0.31365, 0.04268, 0.02092, 0.09102, 0.869651, 0.0439, 0.5139, 0.52632, 0.8882, 0.5823, 0.00263, 0.4395, 0.905927))

man_int_raw_p$pvalueadj <- p.adjust(man_int_raw_p$pvalue, method = "fdr")
```

In this approach, only the interaction between restoration status and estuary for leaf herbivory remains significant after FDR adjustment. Therefore, we will use main effects for all other variables in Stage 2

```{r p_value_adjust_Stage_2}
man_main_raw_p <- tibble(variable = c("Elevation", "Trees", "Saplings", "Seedling_sm", "Seedling_med", "Hollows", "Canopy_cover", "Stems", "DBH", "Ht", "Pneu", "Pneu_ht", "Pneu_dia", "LDMC", "LMA", "Leaf_area", "Leaf_herb", "Elevation", "Trees", "Saplings", "Seedling_sm", "Seedling_med", "Hollows", "Canopy_cover", "Stems", "DBH", "Ht", "Pneu", "Pneu_ht", "Pneu_dia", "LDMC", "LMA", "Leaf_area", "Leaf_herb"),
                         test = c("YSR","YSR","YSR","YSR","YSR","YSR","YSR","YSR","YSR","YSR","YSR","YSR","YSR","YSR","YSR","YSR","YSR","rest_status","rest_status","rest_status","rest_status","rest_status","rest_status","rest_status","rest_status","rest_status","rest_status","rest_status","rest_status","rest_status","rest_status","rest_status","rest_status","rest_status"),
                         pvalue = c(0.009303, 0.2218, 0.004884, 0.3345, 0.7984, 0.7577, 0.1163, 0.8856, 0.01991, 0.01944, 0.4027, 0.03345, 0.07077, 0.6848, 0.04668, 0.1471, 0.00951, 0.03374, 0.3272, 0.02448, 0.2055, 0.8098, 0.006295, 0.08696, 0.20857, 0.005847, 0.18645, 0.7166, 0.00611, 0.7726, 0.01391, 0.1219, 0.2437, 0.00263))

man_main_raw_p$pvalueadj <- p.adjust(man_main_raw_p$pvalue, method = "fdr")
```
