...
This code is for analysis of mangrove functional traits data, analysing for associations between functional traits and time since restoration and between restored and natural sites.
...
mantree <- read_excel(here("Data/Functional_traits_combined_data.xlsx"), sheet = "Mangrove_tree") %>% 
  arrange(Date, Time) %>% 
...
manq <- read_excel(here("Data/Functional_traits_combined_data.xlsx"), sheet = "Mangrove_quadrat") %>% 
  arrange(Date, Time) %>% 
...
manherb <- read_excel(here("Data/Functional_traits_combined_data.xlsx"), sheet = "Mangrove_herb") 
...
manleaf <- read_excel(here("Data/Functional_traits_combined_data.xlsx"), sheet = "Mangrove_leaf") %>% 
  rowwise() %>% 
...
manleafAI <- read_excel(here("Data/leafarea_machinelearning.xlsx"), sheet = "refined") %>%
  mutate(YSR = case_when(
           Site == "HS" ~ as.numeric("12"),
           Site == "CC" ~ as.numeric("33"),
           Site == "CF" ~ as.numeric("30"),
...
# fill out missing tree/sapling label data 
manleafandherb[166:170,6] <- "Sapling"
manleafandherb[c(696:703,709,710),6] <- "Tree"
...
manleaf_wide_grey_rest_t <- manleaf_wide_grey_rest %>% subset(manleaf_wide_grey_rest$Tree_sapling == "Tree") 
```
...
-   Subquadrat variables
    ...
    -   Number of small seedlings

    -   Number of medium seedlings
...
-   All manq variables have low to moderate correlations

-   Both dry weight and wet weight are strongly correlated with leaf area, so don't include those

    -   All other manleaf variables have low to moderate correlations except LMA and dfratio which have a correlation of 0.71. This is somewhat high but ok to retain both.
...
u <- cor(manq[,c(8,9,11,16,17)], use = "pairwise.complete.obs")
u

manq_numeric <- manq[,c(8,9,11,16,17)]
plot(manq_numeric)

v <- cor(manleafandherb[,c(11,12,13,14,15,17)], use = "pairwise.complete.obs")
v

manleaf_numeric <- manleafandherb[,c(11,12,13,14,15,17)]
...
-   relationship between YSR and Elevation (numeric predictors)
    ...
    -   However, given there is variability in their relationship, we can still include both terms in models.
        -   Given they are strongly correlated, we decided to exclude Elevation from YSR models.
...
-   Test the importance of YSR by comparing models without without YSR using anova

-   compare model with a model without the polynomial YSR (M) to determine whether the trend is polynomial or linear (comparing AIC values)
...
m_elev_rest <- ggplot(mantree_comm_rest, aes(x=YSR, y=Elevation, fill=Estuary, group = YSR)) +
  geom_hline(yintercept = mean(mantree_comm_rest$Elevation, na.rm = TRUE), linetype = "dashed", linewidth = 1) +
  annotate(
    "rect",
    xmin = -Inf, xmax = Inf,  # Extend the rectangle across the entire x-axis
    ymin = 0.16, 
    ymax = 0.41,  # Set the y-range for the rectangle as the 95% confidence interval bounds
    alpha = 0.4,  # Set the transparency level
    fill = "grey"  # Set the fill color
  ) +
...
m_elev_nat <- ggplot(mantree_comm_nat, aes(x=Site, y=Elevation, fill=Estuary, group = Site)) +
    geom_hline(yintercept = mean(mantree_comm_nat$Elevation, na.rm = TRUE), linetype = "dashed", linewidth = 1) +
  annotate(
    "rect",
    xmin = -Inf, xmax = Inf,  # Extend the rectangle across the entire x-axis
    ymin = 0.37, 
    ymax = 0.67,  # Set the y-range for the rectangle as the 95% confidence interval bounds
    alpha = 0.4,  # Set the transparency level
    fill = "grey"  # Set the fill color
  ) +
...
ggsave(here("Output/Mangrove_elev_doc.jpg"), width = 2400, height = 1500, units = "px")

em1 <- emmip(M1, ~ YSR, at = list(YSR = c(6,10,15,20,25,30,33)), type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Predicted elevation") +
  geom_jitter(aes(x = YSR, y = Elevation), data = mantree_comm_rest) +
  xlim(0,35) +
  theme_bw()
...
Make a poster presentation figure for mangrove elevation.

```{r Poster-figure-elevation}
t.test(mantree_comm_nat$Elevation)$conf.int # calculate natural total cover confidence interval

m_elev_rest2 <- ggplot(mantree_comm_rest, aes(x=YSR, y=Elevation, fill=Estuary, group = YSR)) +
  geom_hline(yintercept = mean(mantree_comm_nat$Elevation, na.rm = TRUE), linetype = "dashed", linewidth = 1) +
  annotate(
    "rect",
    xmin = -Inf, xmax = Inf,  # Extend the rectangle across the entire x-axis
    ymin = 0.42, 
    ymax = 0.61,  # Set the y-range for the rectangle as the 95% confidence interval bounds
    alpha = 0.4,  # Set the transparency level
    fill = "grey"  # Set the fill color
  ) +
    geom_boxplot(width = 2) +
  ...
  xlab("Years since restoration") +
  ylab("Elevation (m)") +
  ggtitle("Surface elevation") +
  xlim(0,35) +
  scale_y_continuous(limits = c(-0.45,0.7), breaks = scales::pretty_breaks(n = 6))
m_elev_rest2

ggsave(here("Output/Mangrove_elevation_poster.jpg"), width = 2400, height = 2400, units = "px")
```
...
-   It's count data, so try negative binomial and poisson distributions

    -   negative binomial is slightly better (and consistent with restoration status model) so use that

-   Include Estuary as a fixed predictor variable and Site as a random effect
...
em <- emmeans(M2.3, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
...
-   Count data so use a poisson or negative binomial model
    ...
    -   use NB as it looks slightly better and is consistent with the restoration status model

-   Include YSR as a polynomial term

-   Include Estuary as a fixed predictor
...
M3.2 <- glmmTMB(total_saplings ~ Estuary + poly(YSR, 2) + (1|Site) + offset(log(Quadrat_size_m2)), family = "poisson",data = mantree_comm_rest)  
...
check_collinearity(M3.3)
```

Next, test the model.

-   Use an anova to test it against null model that lacks the YSR term
-   compare model with a model without the polynomial YSR (M) to determine whether the trend is polynomial or linear (comparing AIC values)
...
em <- emmeans(M3.3, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response", offset = log(100))  
...
```{r Plot}
emmip(M3.3, ~ YSR, at = list(YSR = c(5:33)), type = "response", CIs = TRUE) +
...
m_sap_rest <- ggplot(mantree_comm_rest, aes(x=YSR, y=total_saplings, fill=Estuary, group = YSR)) +
  geom_hline(yintercept = mean(mantree_comm_rest$total_saplings, na.rm = TRUE), linetype = "dashed", linewidth = 1) +
  annotate(
    "rect",
    xmin = -Inf, xmax = Inf,  # Extend the rectangle across the entire x-axis
    ymin = 61, 
    ymax = 225,  # Set the y-range for the rectangle as the 95% confidence interval bounds
    alpha = 0.4,  # Set the transparency level
    fill = "grey"  # Set the fill color
  ) +
    geom_boxplot(width = 2) +
...
m_sap_nat <- ggplot(mantree_comm_nat, aes(x=Site, y=total_saplings, fill=Estuary, group = Site)) +
  geom_hline(yintercept = mean(mantree_comm_nat$total_saplings, na.rm = TRUE), linetype = "dashed", linewidth = 1) +
  annotate(
    "rect",
    xmin = -Inf, xmax = Inf,  # Extend the rectangle across the entire x-axis
    ymin = 12, 
    ymax = 58,  # Set the y-range for the rectangle as the 95% confidence interval bounds
    alpha = 0.4,  # Set the transparency level
    fill = "grey"  # Set the fill color
  ) +
    geom_boxplot(width = 0.4) +
...
# plot natural and restored together
ggarrange(m_sap_rest,m_sap_nat,ncol =2,nrow=1,widths = c(1.3,1))

ggsave(here("Output/Mangrove_saplings.jpg"), width = 2400, height = 1500, units = "px")

ggarrange(m_sap_rest,m_sap_nat,ncol =2,nrow=1,widths = c(3,2.2))

ggsave(here("Output/Mangrove_saplings_preso.jpg"), width = 4000, height = 2500, units = "px")
```

Make a plot for a poster presentation.

```{r Poster-figure}
t.test(mantree_comm_nat$total_saplings)$conf.int # calculate natural total cover confidence interval

m_sap_rest2 <- ggplot(mantree_comm_rest, aes(x=YSR, y=total_saplings, fill=Estuary, group = YSR)) +
  geom_hline(yintercept = mean(mantree_comm_nat$total_saplings, na.rm = TRUE), linetype = "dashed", linewidth = 1) +
  annotate(
    "rect",
    xmin = -Inf, xmax = Inf,  # Extend the rectangle across the entire x-axis
    ymin = 17, 
    ymax = 49,  # Set the y-range for the rectangle as the 95% confidence interval bounds
    alpha = 0.4,  # Set the transparency level
    fill = "grey"  # Set the fill color
  ) +
    geom_boxplot(width = 2) +
  ...
  xlab("Years since restoration") +
  ylab("Number of saplings per 100m2") +
  ggtitle("Mangrove saplings") +
  xlim(0,35) +
  ylim(0,450)
m_sap_rest2

ggsave(here("Output/Mangrove_saplings_poster.jpg"), width = 2400, height = 2400, units = "px")
```
...
-   Count data so use a poisson or negative binomial model
    ...
    -   both similar but use negative binomial to be consistent with restoration status model
...
M3.4 <- glmmTMB(`Seedlings_<30cm_1x1` ~ Estuary + YSR + (1|Site/Combo), data = manq_comm_rest, family = "poisson") 
...
em <- emmeans(M3.5, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")  
...
```{r Plot}
emmip(M3.5, ~ YSR, at = list(YSR = c(5:33)), type = "response", CIs = TRUE) +    
...
-   Count data so use a poisson or negative binomial model
    ...
    -   similar so use negative binomial to be consistent with restoration status model
...
```{r Model_building}
M3.6 <- glmmTMB(`Seedlings_30-100cm_1x1` ~ Estuary + YSR + (1|Site/Combo), data = manq_comm_rest, family = "poisson") 
...
em <- emmeans(M3.7, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")  
...
-   count data so use poisson or negative binomial mode, or use a zero-inflated model if these can't account for excessive zeroes

    -   use negative binomial to be consistent with restoration status model, and looks fine without zero inflated model, so use without

-   Include additional fixed predictor Estuary and random factor Site
...
M4.1 <- glmmTMB(Hollows ~ Estuary + YSR + (1|Site) + offset(log(Quadrat_size_m2)), family = "poisson", data = mantree_comm_rest)
...
check_collinearity(M4.21)

M4.3 <- glmmTMB(Hollows ~ Estuary + YSR + (1|Site) + offset(log(Quadrat_size_m2)), family = "nbinom2", data = mantree_comm_rest, ziformula = ~1)
...
```{r Plot}
emmip(M4.21, ~ YSR, at = list(YSR = c(5:33)), type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Number of hollows per 10m2", offset = log(100)) +
...
-   Include fixed predictor Estuary aand random factor Site
...
-   compare the model against a null model without the YSR term to test the importance of YSR
-   compare model with a model without the polynomial YSR (M) to determine whether the trend is polynomial or linear (comparing AIC values)
...
emmeans(M5.2, ~ YSR | Estuary, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
emmeans(M5.2, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
```

Finally, plot the results.

```{r Plot}
emmip(M5.2, ~ YSR, at = list(YSR = c(5:33)), type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Canopy cover (proportion)")

emmip(M5.2, ~ YSR | Estuary, at = list(YSR = c(5:33)), type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Canopy cover  (proportion)")
...
-   It's count data so use a poisson or negative binomial distribution
    ...
    -   they both look similar, so use negative binomial to be consistent with restoration status model

-   Include additional fixed predictor Estuary
...
M6.4 <- glmmTMB(Stems ~ Estuary + YSR + (1|Site/Combo), family = "nbinom2", data = mantree_grey_rest)

resid <- residuals(simulateResiduals(M6.4), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M6.4,re.form = NA), resid) 
plotQQunif(M6.4)

# check collinearity on model without interactions
M0 <- glmmTMB(Stems ~ Estuary + YSR + (1|Site/Combo), family = "nbinom2", data = mantree_grey_rest)
...
M6.4 <- glmmTMB(Stems ~ Estuary + YSR + (1|Site/Combo), family = "nbinom2", data = mantree_grey_rest)
M0 <- glmmTMB(Stems ~ Estuary + (1|Site/Combo), family = "nbinom2", data = mantree_grey_rest)

anova(M6.4, M0)

joint_tests(M6.4)

emmeans(M6.4, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
```

Finally, plot the results.

```{r Plot}
emmip(M6.4, ~ YSR, at = list(YSR = c(5:33)), type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Number of stems")
```
...
```{r Model_building}
M7 <- glmmTMB(DBH_cm ~ Estuary + poly(YSR,2) + (1|Site/Combo), family = "gaussian", data = mantree_grey_rest)
...
check_collinearity(M7.1)
```

Next, test the model:

-   Use anova to compare model with a null model that lacks the YSR term (M0)
-   compare model with a model without the polynomial YSR (M) to determine whether the trend is polynomial or linear (comparing AIC values)
...
emmeans(M7.1, ~YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
...
```{r Plot}
emmip(M7.1, ~ YSR, at = list(YSR = c(5:33)), type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "DBH (cm)")

# restoring sites YSR
m_DBH_rest <- ggplot(mantree_grey_rest, aes(x=YSR, y=DBH_cm, fill=Estuary, group = YSR)) +
    geom_boxplot(width = 2) +
  geom_jitter(height = 0) +
    scale_fill_viridis(discrete = TRUE, begin = 0.5, end = 0.8, option = "A") +
    theme_classic() +
...
# natural sites
m_DBH_nat <- ggplot(mantree_grey_nat, aes(x=Site, y=DBH_cm, fill=Estuary, group = Site)) +
    geom_boxplot(width = 0.4) +
  geom_jitter(height = 0, width = 0.1) +
    scale_fill_viridis(discrete = TRUE, begin = 0.5, end = 0.8, option = "A") +
    theme_classic() +
...
# plot natural and restored together
ggarrange(m_DBH_rest,m_DBH_nat,ncol =2,nrow=1,widths = c(1.2,1))

ggsave(here("Output/Mangrove_DBH.jpg"), width = 4000, height = 2500, units = "px")
```
...
-   additional fixed predictor Estuary

-   test assumptions using resid and scatter.smooth, plotQQunif and check_collinearity on a model without interactions

    -   sqrt and log transformed models looks better than the original
    -   use sqrt transformation to be consistent with restoration status model

```{r Model_building}
M8.2 <- glmmTMB(Height_m ~ Estuary + poly(YSR,2) + (1|Site/Combo), family = "gaussian", data = mantree_grey_rest)
...
## use square root normal model, consistent with restoration status model

M8.41 <- glmmTMB(sqrt(Height_m) ~ Estuary + poly(YSR,2) + (1|Site/Combo), family = "gaussian", data = mantree_grey_rest)

resid <- residuals(simulateResiduals(M8.41), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M8.41,re.form = NA), resid) 
scatter.smooth(predict(M8.41,re.form = NA), abs(resid)) 
plotQQunif(M8.41)

check_collinearity(M4.1)
```

Next, test the model.

-   test model against a null model (M0) that lacks the YSR term

-   compare AIC values of the model and a model (M) without polynomial YSR to determine if the trend is linear or polynomial

```{r Model_testing}

M8.41 <- glmmTMB(sqrt(Height_m) ~ Estuary + poly(YSR,2) + (1|Site/Combo), family = "gaussian", data = mantree_grey_rest)
M <- glmmTMB(sqrt(Height_m) ~ Estuary + YSR + (1|Site/Combo), family = "gaussian", data = mantree_grey_rest)
M0 <- glmmTMB(sqrt(Height_m) ~ Estuary + (1|Site/Combo), family = "gaussian", data = mantree_grey_rest)
...
em <- emtrends(M8.41, var = "YSR")

em

plot(em)

emmeans(M8.41, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
```

Finally, plot the results.

```{r Plot}
emmip(M8.41, ~ YSR, at = list(YSR = c(5:33)), type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Tree height") +
  #geom_boxplot(data = mantree_grey_rest, aes(x = YSR, y = Height_m, group = YSR)) +
  ylim(0,12) +
  xlim(0,35)

# restoring sites YSR
m_Ht_rest <- ggplot(mantree_grey_rest, aes(x=YSR, y=Height_m, fill=Estuary, group = YSR)) +
  geom_hline(yintercept = mean(mantree_grey_rest$Height_m, na.rm = TRUE), linetype = "dashed", linewidth = 1) +
  annotate(
    "rect",
    xmin = -Inf, xmax = Inf,  # Extend the rectangle across the entire x-axis
    ymin = 4.6, 
    ymax = 5.74,  # Set the y-range for the rectangle as the 95% confidence interval bounds
    alpha = 0.4,  # Set the transparency level
    fill = "grey"  # Set the fill color
  ) +
    geom_boxplot(width = 2) +
...
# natural sites
m_Ht_nat <- ggplot(mantree_grey_nat, aes(x=Site, y=Height_m, fill=Estuary, group = Site)) +
    geom_hline(yintercept = mean(mantree_grey_nat$Height_m, na.rm = TRUE), linetype = "dashed", linewidth = 1) +
  annotate(
    "rect",
    xmin = -Inf, xmax = Inf,  # Extend the rectangle across the entire x-axis
    ymin = 5.37, 
    ymax = 6.87,  # Set the y-range for the rectangle as the 95% confidence interval bounds
    alpha = 0.4,  # Set the transparency level
    fill = "grey"  # Set the fill color
  ) +
    geom_boxplot(width = 0.4) +
...
# plot natural and restored together
ggarrange(m_Ht_rest,m_Ht_nat,ncol =2,nrow=1,widths = c(1.2,1))

ggsave(here("Output/Mangrove_Height.jpg"), width = 2400, height = 1500, units = "px")
```
...
-   It's count data, so could use a poisson or negative binomial distribution, but it's also close to normally distributed, so could use a gaussian distribution
    -   gaussian distribution looks best and is consistent with restoration status model
...
M9.21 <- glmmTMB(Pneu_0.5x0.5 ~ Estuary + YSR + (1|Site/Combo), data = manq_comm_rest, family = "poisson")

resid <- residuals(simulateResiduals(M9.21), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M9.21,re.form = NA), resid) 
plotQQunif(M9.21) # residuals plots don't look great

M9.31 <- glmmTMB(Pneu_0.5x0.5 ~ Estuary + YSR + (1|Site/Combo), data = manq_comm_rest, family = "nbinom2")

resid <- residuals(simulateResiduals(M9.31), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M9.31,re.form = NA), resid) 
plotQQunif(M9.31) # residuals plots don't look great
```
...
emmeans(M9.11, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
```
...
emmip(M9.11, ~ YSR, at = list(YSR = c(5:33)), type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Number of pneumatophores per 0.5 x 0.5 m2") +
...
-   right skewed so try sqrt transformation
    ...
    -   sqrt transformed residuals plots look better

-   random effect of subquadrat nested within quadrat (Combo) nested within Site

```{r Model_building}
M10.2 <- glmmTMB(Pneu_ht_mm ~ Estuary + poly(YSR,2) + (1|Site/Combo/Subquadrat), data = manq_rest, family = "gaussian")
...
check_collinearity(M10.3)
```

Next, test the model.

-   Test model using anova, comparing the chosen model to a model without YSR (M0)
...
emmeans(M10.3, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
emmeans(M10.3, ~ Estuary, type = "response")
```
...
# natural sites
m_pneu_ht_nat <- ggplot(manq_nat, aes(x=Site, y=Pneu_ht_mm/10, fill=Estuary, group = Site)) +
    geom_boxplot(width = 0.4) +
  geom_jitter(width = 0.1) +
    scale_fill_viridis(discrete = TRUE, begin = 0.5, end = 0.8, option = "A") +
    theme_classic() +
...
ggsave(here("Output/Pneumatophore_height.jpg"), width = 4000, height = 2500, units = "px")
```
...
-   continuous data so use gaussian distribution
-   try sqrt and log transformations if residuals aren't great
    -   log and sqrt transform look similar - use log transformation, consistent with restoration status model

```{r Model_building}
M11.2 <- glmmTMB(Pneu_dia_mm ~ Estuary + poly(YSR, 2) + (1|Site/Combo/Subquadrat), family = "gaussian", data = manq_rest)
...
## make a log normal model, consistent with restoration status model

M11.4 <- glmmTMB(log(Pneu_dia_mm) ~ Estuary + poly(YSR, 2) + (1|Site/Combo/Subquadrat), family = "gaussian", data = manq_rest)

resid <- residuals(simulateResiduals(M11.4), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M11.4,re.form = NA), resid) 
plotQQunif(M11.4)

check_collinearity(M11.4)
```
...
M11.4 <- glmmTMB(log(Pneu_dia_mm) ~ Estuary + poly(YSR, 2) + (1|Site/Combo/Subquadrat), family = "gaussian", data = manq_rest)
M <- glmmTMB(log(Pneu_dia_mm) ~ Estuary + YSR + (1|Site/Combo/Subquadrat), family = "gaussian", data = manq_rest)
M0 <- glmmTMB(log(Pneu_dia_mm) ~ Estuary + (1|Site/Combo/Subquadrat), family = "gaussian", data = manq_rest)
...
emmeans(M11.4, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
```
...
emmip(M11.4, ~ YSR, at = list(YSR = c(5:33)), type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Pneumatophore diameter (mm)") +
  geom_boxplot(data = manq_rest, aes(x = YSR, y = Pneu_dia_mm, group = YSR))
```

## 2m. Leaf dry matter content
...
em <- emmeans(M13.2, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)))
...
emmip(M13.2, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)))
# misleading plot - very slight change in values but on this scale looks large
```
...
Don't include Estuary as a factor, because the Maroochy estuary only had one site where we could measure leaf weights.
...
M0 <- glmmTMB(LMA ~ Tree_sapling + YSR, family = "gaussian", data = manleaf_wide_grey_rest)
check_collinearity(M0)
```

Next, test the model.

-   Model convergence issues - scale continuous factors LMA and YSR -\> fixed

```{r Model_testing}
M14.2 <- glmmTMB(scale(LMA) ~ Tree_sapling + scale(YSR) + (1|Site/Combo/Tree_ID), family = "gaussian", data = manleaf_wide_grey_rest)
M <- glmmTMB(scale(LMA) ~ Tree_sapling + (1|Site/Combo/Tree_ID), family = "gaussian", data = manleaf_wide_grey_rest)
...
emmeans(M14.2, ~YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
```

Finally, plot the results.

```{r Plot}
emmip(M14.2, ~YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response") +
...
-   If non-transformed data don't meet assumptions, try log transformation
    -   both are fine, but use log to be consistent with restoration status model

```{r Model_building}
M15 <- glmmTMB(Area ~ Estuary + Tree_sapling + YSR + (1|Site/Combo/Tree), family = "gaussian", data = manleaf_wide_grey_rest)
...
emmeans(M15.1, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
...
-   Use a beta or tweedie distribution as it's proportion data

    -   tweedie distribution looks better

        -   however it has a model convergence problem - needed to remove the random effect level of Tree for it to run properly

-   There are no sapling data, so therefore don't need to include the Tree_sapling term

```{r Model_building}
M16.2 <- glmmTMB(Herb_prop ~ Estuary + YSR + (1|Site/Combo/Tree), family = "beta_family", data = manleaf_wide_grey_rest_t) 
...
M16.3 <- glmmTMB(Herb_prop ~ Estuary + YSR + (1|Site/Combo), family = "tweedie", data = manleaf_wide_grey_rest_t) 
...
M16.3 <- glmmTMB(Herb_prop ~ Estuary + YSR + (1|Site/Combo), family = "tweedie", data = manleaf_wide_grey_rest_t) 
M <- glmmTMB(Herb_prop ~ Estuary + (1|Site/Combo), family = "tweedie", data = manleaf_wide_grey_rest_t) 
...
emmeans(M16.3, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
emmeans(M16.3, ~ Estuary, type = "response")
...
ggsave(here("Output/Mangrove_herbivory.jpg"), width = 4000, height = 2500, units = "px")
```
...
-   relationship between rest_status and Elevation
    ...
    -   We have decided to exclude elevation as a predictor from all mangrove models, given it has a strong correlation with YSR and restoration status
...
```{r Model_building}
M18 <- glmmTMB(Trees ~ Estuary*rest_status + (1|Site) + offset(log(Quadrat_size_m2)), family = "poisson", data = mantree_comm)
...
emmeans(M18.1, ~ rest_status | Estuary, type = "response")  
emmeans(M18.1, ~ rest_status, type = "response")  
```
...
```{r Model_building}
M19 <- glmmTMB(Saplings ~ Estuary * rest_status + (1|Site) + offset(log(Quadrat_size_m2)), family = "poisson", data = mantree_comm)
...
emmeans(M19.1, ~ rest_status | Estuary, type = "response", offset = log(100))  
emmeans(M19.1, ~ rest_status, type = "response", offset = log(100))  
```
...
-   Distribution type: it's count data so poisson or negative binomial
    ...
    -   negative binomial looks better
...
```{r Model_building}
M19.2 <- glmmTMB(`Seedlings_<30cm_1x1` ~ Estuary * rest_status + (1|Site), family = "poisson", data = manq_comm)
...
emmeans(M19.31, ~ rest_status | Estuary, type = "response")  
emmeans(M19.31, ~ rest_status, type = "response")  
```
...
```{r Model_building}
M19.41 <- glmmTMB(`Seedlings_30-100cm_1x1` ~ Estuary * rest_status + (1|Site), family = "poisson", data = manq_comm)

resid <- residuals(simulateResiduals(M19.41), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M19.41,re.form = NA), resid) 
plotQQunif(M19.41)
...
emmeans(M19.51, ~ rest_status | Estuary, type = "response") 
emmeans(M19.51, ~ rest_status, type = "response") 
```
...
-   Distribution type: zero inflated count data so poisson, negative binomial or a zero-inflated version of one of these
    ...
    -   looks fine without zero inflation, negative binomial model looks best
...
```{r Model_building}
M20.2 <- glmmTMB(Hollows ~ Estuary * rest_status + offset(log(Quadrat_size_m2)) + (1|Site), family = "poisson", data = mantree_comm)
...
emmeans(M20.3, ~ rest_status | Estuary, type = "response", offset = log(100))  
emmeans(M20.3, ~ rest_status, type = "response", offset = log(100)) 
```
...
#check collinearity on model without interactions
M0 <- glmmTMB(Cover_prop ~ Estuary + rest_status + (1|Site), family = "beta_family", data = mantree_comm)
check_collinearity(M0)
```
...
emmeans(M21.1, ~ rest_status | Estuary, type = "response")  
emmeans(M21.1, ~ rest_status, type = "response")  
```
...
-   Distribution type: poisson or negative binomial as it's count data
    ...
    -   NB is slightly better so use that
...
```{r Model_building}
M21.6 <- glmmTMB(Stems ~ Estuary + Species * rest_status + Estuary:rest_status + (1|Site/Combo), family = "poisson", data = mantree_nored)
...
emmeans(M21.7, ~ rest_status, type = "response")
emmeans(M21.7, ~ rest_status | Species, type = "response")
emmeans(M21.7, ~ rest_status | Estuary, type = "response")
emmeans(M21.7, ~ rest_status | Species | Estuary, type = "response")
```
...
-   Distribution type: it's continous data so use a gaussian distribution, and try sqrt or log transformed data if assumptions are not met without a transformation

    -   log transformed is best

-   This variable is measured by species so need to include species interacting with restoration status

-   Include Estuary as a fixed predictor variable and Quadrat (Combo) within Site as a random effect

-   Check assumptions using resid and scatter.smooth, plotQQunif and check_collinearity with a model without interactions

```{r Model_building}
M22.0 <- glmmTMB(DBH_cm ~ Estuary + Species * rest_status + Estuary: rest_status + (1|Site/Combo), family = "gaussian", data = mantree_nored)

resid <- residuals(simulateResiduals(M22.0), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M22.0,re.form = NA), resid) 
plotQQunif(M22.0)

M22 <- glmmTMB(sqrt(DBH_cm) ~ Estuary + Species * rest_status + Estuary: rest_status + (1|Site/Combo), family = "gaussian", data = mantree_nored)
...
emmeans(M22.1, ~ rest_status, type = "response")
emmeans(M22.1, ~ rest_status | Species, type = "response")  
emmeans(M22.1, ~ rest_status | Estuary, type = "response") 
emmeans(M22.1, ~ rest_status | Estuary | Species, type = "response")  
```
...
ggsave(here("Output/Mangrove_DBH_nat_vs_rest.jpg"), width = 2000, height = 1500, units = "px")
```
...
-   Distribution type: continuous so use a gaussian distribution, if it doesn't meet assumptions try the square root transformation
    ...
    -   sqrt is an improvement - use that
...
```{r Model_building}
M23.2 <- glmmTMB(Height_m ~ Estuary + Species * rest_status + Estuary : rest_status + (1|Site/Combo), family = "gaussian", data = mantree_nored)
...
emmeans(M23.3, ~ rest_status | Species, type = "response")  
emmeans(M23.3, ~ rest_status | Estuary | Species, type = "response") 
```
...
```{r Model_building}
M24.3 <- glmmTMB(Pneu_0.5x0.5 ~ Estuary * rest_status + (1|Site/Combo), family = "poisson", data = manq_comm)
...
M24.4 <- glmmTMB(Pneu_0.5x0.5 ~ Estuary * rest_status + (1|Site/Combo), family = "nbinom2", data = manq_comm)

resid <- residuals(simulateResiduals(M24.4), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M24.4,re.form = NA), resid) 
plotQQunif(M24.4)
...
M0 <-  glmmTMB(Pneu_0.5x0.5 ~ Estuary + rest_status + (1|Site/Combo), family = "gaussian", data = manq_comm)
...
emmeans(M24.5, ~ rest_status | Estuary, type = "response")  
emmeans(M24.5, ~ rest_status, type = "response")  
```
...
```{r Model_building}
M25.2 <- glmmTMB(Pneu_ht_mm ~ Estuary * rest_status + (1|Site/Combo/Subquadrat), family = "gaussian", data = manq)
...
# check collinearity on model without interactions
M0 <- glmmTMB(sqrt(Pneu_ht_mm) ~ Estuary + rest_status + (1|Site/Combo/Subquadrat), family = "gaussian", data = manq)
check_collinearity(M0)
```
...
emmeans(M25.3, ~ rest_status, type = "response")
emmeans(M25.3, ~ rest_status | Estuary, type = "response")
```
...
```{r Model_building}
M26.2 <- glmmTMB(Pneu_dia_mm ~ Estuary * rest_status + (1|Site/Combo/Subquadrat), family = "gaussian", data = manq)
...
M26.3 <- glmmTMB(sqrt(Pneu_dia_mm) ~ Estuary * rest_status + (1|Site/Combo/Subquadrat), family = "gaussian", data = manq)
...
# check collinearity on model without interactions
M0 <- glmmTMB(log(Pneu_dia_mm) ~ Estuary + rest_status + (1|Site/Combo/Subquadrat), family = "gaussian", data = manq)
check_collinearity(M0)
```
...
emmeans(M26.4, ~ rest_status, type = "response")
emmeans(M26.4, ~ rest_status | Estuary, type = "response")
```
...
-   Include Estuary as a fixed predictor variable and Tree nested within Quadrat (combo) nested within Site as a random effect
...
emmeans(M28.1, ~ rest_status, type = "response")
```

Finally, plot the results.

```{r Plot}
# natural vs restored sites
m_LDMC_nat <- ggplot(manleaf_wide_grey, aes(x=rest_status, y=dfratio, group = rest_status)) +
    geom_boxplot(width = 0.4) +
  geom_jitter(width = 0.1, height = 0, alpha = 0.1) +
    theme_classic() +
    theme(legend.text = element_text(size = 20),
...
check_collinearity(M29.1)
```

Next, test the model:

-   use anova to test the model against a null model without YSR to test the importance of YSR
-   Convergence issue - scale LMA -\> fixed model

```{r Model_testing}

M29.1 <- glmmTMB(scale(LMA) ~ Estuary + Tree_sapling + rest_status + (1|Site/Combo/Tree_ID), family = "gaussian", data = manleaf_wide_grey)
M0 <- glmmTMB(scale(LMA) ~ Estuary + Tree_sapling + (1|Site/Combo/Tree_ID), family = "gaussian", data = manleaf_wide_grey)
...
emmeans(M29.1, ~ rest_status, type = "response")
```
...
M30.4 <- glmmTMB(Area ~ Estuary + Tree_sapling + Species * rest_status + Estuary:rest_status + (1|Site/Combo/Tree), family = "gaussian", data = manleafandherb_nored)
  
resid <- residuals(simulateResiduals(M30.4), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M30.4,re.form = NA), resid) 
plotQQunif(M30.4)

## use Log because it looks better and without, there are model convergence issues
...
emmeans(M30.3, ~ rest_status, type = "response")
emmeans(M30.3, ~ rest_status | Species, type = "response")
emmeans(M30.3, ~ rest_status | Estuary | Species, type = "response")
```
...
M31.6 <- glmmTMB(Herb_prop ~ Estuary + Tree_sapling + Species * rest_status + Estuary:rest_status + (1|Site/Combo/Tree), family = "tweedie", data = manleafandherb_nored)
  
resid <- residuals(simulateResiduals(M31.6), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M31.6,re.form = NA), resid) 
plotQQunif(M31.6)

# check collinearity on a model without interactions
M0 <- glmmTMB(Herb_prop ~ Estuary + Species+ Tree_sapling + rest_status + (1|Site/Combo/Tree), family = "tweedie", data = manleafandherb_nored)
check_collinearity(M0)
```
...
M31.6 <- glmmTMB(Herb_prop ~ Estuary + Tree_sapling + Species * rest_status + Estuary:rest_status + (1|Site/Combo/Tree), family = "tweedie", data = manleafandherb_nored)
M1 <- glmmTMB(Herb_prop ~ Estuary + Species + Tree_sapling + rest_status + Species:rest_status + (1|Site/Combo/Tree), family = "tweedie", data = manleafandherb_nored)
M <- glmmTMB(Herb_prop ~ Estuary + Species + Tree_sapling + rest_status + (1|Site/Combo/Tree), family = "tweedie", data = manleafandherb_nored)
M0 <- glmmTMB(Herb_prop ~ Estuary + Species + Tree_sapling + (1|Site/Combo/Tree), family = "tweedie", data = manleafandherb_nored)
...
emmeans(M31.6, ~ rest_status | Species, type = "response")
emmeans(M31.6, ~ rest_status | Species | Estuary, type = "response")
emmeans(M31.6, ~ rest_status | Estuary, type = "response")
...
```{r Plot}
emmip(M31.5, ~ rest_status | Species, type = "response", CIs = TRUE, engine = "ggplot", xlab = "Restoration status", ylab = "Leaf herbivory (proportion)")
