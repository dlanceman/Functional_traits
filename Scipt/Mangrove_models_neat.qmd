...
manleafandherb <- merge(manleafandherb, mantree[,c("Estuary","Site","Combo","Elevation")], all.x = TRUE) 
...
mantree_comm_sapling <- mantree_comm %>% 
  dplyr::select(Estuary, Site, Elevation, rest_status, YSR, Combo, Saplings, Quadrat_size_m2) %>% 
  pivot_longer(cols = Saplings, names_to = "SizeClass", values_to = "Count")

seedlings <- rbind(manq_comm_seedling, mantree_comm_sapling) %>% 
  mutate(Total = Count * 100/Quadrat_size_m2,
         SizeClass = as.factor(SizeClass))

seedlings_rest <- seedlings %>% subset(rest_status == 1)
```
...
```{r Prep/explore}
ggplot(seedlings_rest, aes(x = Count)) +
  geom_histogram()
...
ggplot(seedlings_rest, aes(x = Total)) +
  geom_histogram()
...
ggplot(seedlings_rest, aes(x = YSR, y = Total)) +
  geom_point() +
...
ggplot(seedlings_rest, aes(x = Elevation, y = Total)) +
  geom_point() +
  stat_smooth(method = "lm", formula = y ~ x + I(x^2)) +
  geom_smooth(method = "lm") +
  facet_wrap(~SizeClass, scales = "free")
...
```{r Model_building}
M3.1 <- glmmTMB(Count ~ SizeClass * (Estuary * Elevation + poly(YSR, 2)) + (1|Site/Combo) + rr(SizeClass+0|Combo, d = 2) + offset(log(Quadrat_size_m2)), family = "poisson",data = seedlings_rest)
...
M3.2 <- glmmTMB(Count ~ SizeClass * (Estuary * Elevation + poly(YSR, 2)) + (1|Site/Combo) + rr(SizeClass+0|Combo, d = 2) + offset(log(Quadrat_size_m2)), family = "nbinom2",data = seedlings_rest)
...
M <- glmmTMB(Count ~ SizeClass * (Estuary * Elevation + YSR) + (1|Site/Combo) + rr(SizeClass+0|Combo, d = 2) + offset(log(Quadrat_size_m2)), family = "poisson",data = seedlings_rest)

M0 <- glmmTMB(Count ~ SizeClass * (Estuary * Elevation) + (1|Site/Combo) + rr(SizeClass+0|Combo, d = 2) + offset(log(Quadrat_size_m2)), family = "poisson",data = seedlings_rest)
...
em <- emmeans(M3.1, ~ YSR | SizeClass, at = list(YSR = c(5,9,12,16,30,33)), type = "response", offset = log(100))
...
emmip(M3.1, ~ YSR | SizeClass, at = list(YSR = c(5:33)), type = "response", CIs = TRUE, offset = log(100))

emmip(M3.1, ~ YSR | SizeClass, at = list(YSR = c(5:33)), type = "response", CIs = TRUE, offset = log(100)) + 
  geom_boxplot(data = seedlings_rest, aes(x = YSR, y = Total, group = YSR)) + ylim(0,4000)
```
...
# negative binomial models look best - check if there is evidence of a difference between these
anova(M4.2,M4.4)

# given there's no difference, use the simpler (non-zero-inflated) model - M4.2
...
emmeans(M8, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
```
...
emmeans(M9.3, ~ YSR | Estuary, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
```
...
hist(manq_comm_rest$Bare_prop) # highly left skewed
...
-   can't include any interaction terms with Estuary as there's only one site with leaf weight data in the Maroochy

-   random effect of tree within quadrat (Combo) within Site

```{r Model_building}

M13 <- glmmTMB(dfratio ~ Estuary + poly(Elevation,2) + YSR + (1|Site/Combo/Tree), family = "gaussian", data = manleaf_wide_grey_rest)
...
M <- glmmTMB(dfratio ~ Estuary + poly(Elevation,2) + (1|Site/Combo/Tree), family = "gaussian", data = manleaf_wide_grey_rest)
...
ggplot(manleaf_wide_grey_rest, aes(x = Elevation, y = LMA)) +
  geom_point() +
  geom_smooth(method = "lm") +
  stat_smooth(method = "lm", formula = y ~ x + I(x^2))
```
...
-   It wouldn't run with Site as a random effect (maybe because there are only 4 levels of site for this variable), so only include tree within quadrat (Combo) as random effects
    -   **ISSUE: IT WON'T RUN WITH SITE OR TREE AS RANDOM EFFECTS, AND WE NEED THESE..**

```{r Model_building}
M14 <- glmmTMB(LMA ~ Estuary + poly(Elevation,2) + YSR + (1|Combo), family = "gaussian", data = manleaf_wide_grey_rest)
...
```{r Model_testing}
M14 <- glmmTMB(LMA ~ Estuary + poly(Elevation,2) + YSR + (1|Combo), family = "gaussian", data = manleaf_wide_grey_rest)
M <- glmmTMB(LMA ~ Estuary + poly(Elevation,2) + (1|Combo), family = "gaussian", data = manleaf_wide_grey_rest)
...
# these values are very similar, so no difference.
```
...
M <- glmmTMB(Area ~ Estuary * Elevation + YSR + (1|Site/Combo/Tree), family = "gaussian", data = manleaf_wide_grey_rest)

resid <- residuals(simulateResiduals(M), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M,re.form = NA), resid) 
plotQQunif(M)

M15 <- glmmTMB(log(Area) ~ Estuary * Elevation + YSR + (1|Site/Combo/Tree), family = "gaussian", data = manleaf_wide_grey_rest)
...
# assumptions plots look great

anova(M,M15)

# log transformed model is better, use that
```
...
M15 <- glmmTMB(log(Area) ~ Estuary * Elevation + YSR + (1|Site/Combo/Tree), family = "gaussian", data = manleaf_wide_grey_rest)
M <- glmmTMB(log(Area) ~ Estuary * Elevation + (1|Site/Combo/Tree), family = "gaussian", data = manleaf_wide_grey_rest)
...
M16 <- glmmTMB(Herb_prop ~ Estuary * Elevation + YSR + (1|Site/Combo/Tree), family = "beta_family", data = manleaf_wide_grey_rest) 
...
-   **NEED TO FIX ABOVE MODEL ISSUES FIRST**

```{r Model_testing}
M16 <- glmmTMB(Herb_prop ~ Estuary * Elevation + YSR + (1|Site/Combo/Tree), family = "beta_family", data = manleaf_wide_grey_rest) 
M <- glmmTMB(Herb_prop ~ Estuary * Elevation + (1|Site/Combo/Tree), family = "beta_family", data = manleaf_wide_grey_rest) 
...
emmeans(M16, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")
```
...
## x2q. Leaf dry weight (exploratory, correlated with other vars)

First, prepare and explore the data.

```{r Prep/explore}
hist(manleaf_wide_grey_rest$Dry_weight) # slightly right skewed
hist(sqrt(manleaf_wide_grey_rest$Dry_weight)) # better, approx normal

plot(manleaf_wide_grey_rest$Dry_weight ~ manleaf_wide_grey_rest$YSR)

plot(manleaf_wide_grey_rest$Dry_weight ~ manleaf_wide_grey_rest$Elevation) # not obviously non-linear
```

Next, create a model.

-   Exclude random effect of site (error otherwise)

```{r Model_building}

M17 <- glmmTMB(Dry_weight ~ Estuary * Elevation + YSR + (1|Combo/Tree), family = "gaussian", data = manleaf_wide_grey_rest)

resid <- residuals(simulateResiduals(M17), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M17,re.form = NA), resid) 
plotQQunif(M17)
...
```{r Model_testing}
M17 <- glmmTMB(Dry_weight ~ Estuary * Elevation + YSR + (1|Combo/Tree), family = "gaussian", data = manleaf_wide_grey_rest)
M <- glmmTMB(Dry_weight ~ Estuary * Elevation + (1|Combo/Tree), family = "gaussian", data = manleaf_wide_grey_rest)
...
emmeans(M17, ~YSR, at = list(YSR = c(5,9,12,16,30,33)))
```

Finally, plot the results.

```{r Plot}
emmip(M17, ~YSR, at = list(YSR = c(6:33)), type = "response", CIs = TRUE)
```
...
## 3b. Number of saplings/seedlings
...
```{r Prep/explore}
hist(seedlings$Total)

ggplot(seedlings, aes(x = Total)) +
  geom_histogram() +
  facet_wrap(~SizeClass)

plot(seedlings$Total ~ seedlings$rest_status)
plot(seedlings$Total ~ seedlings$Elevation) # not obviously non-linear
```
...
```{r Model_building}
M19 <- glmmTMB(Total ~ Elevation + SizeClass * (Estuary + rest_status + Estuary:rest_status) + rr(SizeClass+0|Combo, d = 2) + (1|Site) + offset(log(Quadrat_size_m2)), family = "poisson", data = seedlings)
...
# looks terrible (has a trend), try negative binomial

M19 <- glmmTMB(Total ~ Elevation + SizeClass * (Estuary + rest_status + Estuary:rest_status) + rr(SizeClass+0|Combo, d = 2) + (1|Site) + offset(log(Quadrat_size_m2)), family = "nbinom2", data = seedlings)

resid <- residuals(simulateResiduals(M19), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M19,re.form = NA), resid) 
plotQQunif(M19)

# looks great! use this one
```
...
M <- glmmTMB(Total ~ Elevation + SizeClass * (Estuary + rest_status) + rr(SizeClass+0|Combo, d = 2) + (1|Site) + offset(log(Quadrat_size_m2)), family = "nbinom2", data = seedlings)

M0 <- glmmTMB(Total ~ Elevation + SizeClass * (Estuary + rest_status) + rr(SizeClass+0|Combo, d = 2) + (1|Site) + offset(log(Quadrat_size_m2)), family = "nbinom2", data = seedlings)
...
emmeans(M19, ~ rest_status | SizeClass, type = "response", offset = log(100))  
```
...
emmip(M19, ~ rest_status | SizeClass, type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Number of saplings per 10m2", offset = log(100))
```
...
```{r Model_testing}   anova(M0,M2.2)    joint_tests()  summary()  emmeans(M2.2, ~ YSR, at = list(YSR = c(5,9,12,16,30,33)), type = "response")}
```

Finally, plot the results.

```{r Plot}   emmip(M2, ~ YSR, at = list(YSR = c(5:33)), type = "response", CIs = TRUE, engine = "ggplot", xlab = "Years since restoration", ylab = "Number of trees per 10m2", offset = log(100))}
```
...
anova(M23.1,M23.2) # sqrt transformed version has lower AIC so use that
```
...
M1 <- glmmTMB(sqrt(Height_m) ~ Estuary + Elevation + Species * rest_status + (1|Site/Combo), family = "gaussian", data = mantree_nored)
...
Insufficient data... (this isn't even grouped by Species which would limit the data even further)

```{r}
mantree_grey_comm %>% group_by(Estuary) %>% count(Buds)
mantree_grey_comm %>% group_by(Estuary) %>% count(Flowers_live)
mantree_grey_comm %>% group_by(Estuary) %>% count(Flowers_dead)
mantree_grey_comm %>% group_by(Estuary) %>% count(Fruits)
```
...
emmeans(M24.3, ~ rest_status | Estuary, type = "response")  
```

Finally, plot the results.

```{r Plot}
emmip(M24.3, ~ rest_status | Estuary, type = "response", CIs = TRUE, engine = "ggplot", xlab = "Restoration status", ylab = "Number of pneumatophores per 0.25m2")
```
...
hist(manq_comm$Bare_prop) # highly left skewed

plot(manq_comm$Bare_prop ~ manq_comm$rest_status)

ggplot(manq_comm, aes(x = Elevation, y = Bare_prop)) +
  geom_point() +
  geom_smooth() +
  stat_smooth(method = "lm")
...
-   Distribution type: beta distribution as it's proportion data

-   Include Elevation \* Estuary and Estuary:rest_status as additional fixed predictor variables and quadrat (Combo) nested within Site as a random effect

-   Check assumptions using resid and scatter.smooth, plotQQunif and check_collinearity with a model without interactions

```{r Model_building}
M27 <- glmmTMB(Bare_prop ~ Estuary * Elevation + rest_status + Estuary:rest_status + (1|Site/Combo), family = beta_family(link = "logit"), data = manq_comm)

resid <- residuals(simulateResiduals(M27), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M27,re.form = NA), resid) 
plotQQunif(M27)
...
M <- glmmTMB(Bare_prop ~ Estuary * Elevation + rest_status + (1|Site/Combo), family = beta_family(link = "logit"), data = manq_comm)

M0 <- glmmTMB(Bare_prop ~ Estuary * Elevation + (1|Site/Combo), family = beta_family(link = "logit"), data = manq_comm)
...
emmeans(M27, ~ rest_status, type = "response")
emmeans(M27, ~ rest_status | Estuary, type = "response")
```

Finally, plot the results.

```{r Plot}
emmip(M27, ~ rest_status, type = "response", CIs = TRUE, engine = "ggplot", xlab = "Restoration status", ylab = "Percent bare groundcover (%)")
```
...
-   Include Elevation and Estuary as additional fixed predictor variables and Tree nested within Quadrat (combo) nested within Site as a random effect
    ...
    -   can't include Estuary:rest_status or Estuary:elevation interaction because there is only one Maroochy site with restored data for leaf weights

-   Check assumptions using resid and scatter.smooth, plotQQunif and check_collinearity with a model without interactions

```{r Model_building}
M28 <- glmmTMB(dfratio ~ Estuary + poly(Elevation,2) + rest_status + (1|Site/Combo/Tree), family = "gaussian", data = manleaf_wide_grey)
...
M0 <- glmmTMB(dfratio ~ Estuary + poly(Elevation,2) + (1|Site/Combo/Tree), family = "gaussian", data = manleaf_wide_grey)
...
```{r Model_building}
M29 <- glmmTMB(LMA ~ Estuary + poly(Elevation,2) + rest_status + (1|Combo), family = "gaussian", data = manleaf_wide_grey)
...
-   use anova to test the model against a null model without YSR to test the importance of YSR
-   **NEED TO FIX THE MODEL BEFORE WE CAN TRUST THESE RESULTS**

```{r Model_testing}
M29 <- glmmTMB(LMA ~ Estuary + poly(Elevation,2) + rest_status + (1|Combo), family = "gaussian", data = manleaf_wide_grey)

M0 <- glmmTMB(LMA ~ Estuary + poly(Elevation,2) + (1|Combo), family = "gaussian", data = manleaf_wide_grey)
...
```{r Model_building}
M30.1 <- glmmTMB(Area ~ Estuary + Species*Elevation + Estuary:Elevation + rest_status + Species:rest_status + (1|Site/Combo/Tree), family = "gaussian", data = manleafandherb_nored)
...
M30.2 <- glmmTMB(log(Area) ~ Estuary + Species*Elevation + Estuary:Elevation + rest_status + Species:rest_status + (1|Site/Combo/Tree), family = "gaussian", data = manleafandherb_nored)
...
M <- glmmTMB(log(Area) ~ Estuary + Species*Elevation + Estuary:Elevation + rest_status + (1|Site/Combo/Tree), family = "gaussian", data = manleafandherb_nored)

M0 <- glmmTMB(log(Area) ~ Estuary + Species*Elevation + Estuary:Elevation + (1|Site/Combo/Tree), family = "gaussian", data = manleafandherb_nored)
...
```{r Plot}
emmip(M30, ~ rest_status | Species, type = "response", CIs = TRUE, engine = "ggplot", xlab = "Restoration status", ylab = "Leaf area (mm^2)")
```
...
## x3q. Leaf dry weight (exploratory analysis only)
