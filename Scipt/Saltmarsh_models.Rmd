...
summary(M14)
joint_tests(M14)
emmeans(M14, pairwise~YSR)
emmeans(M14, pairwise~DomSpeciesSuperSimp)
emmeans(M14, pairwise~YSR|DomSpeciesSuperSimp)


M15 <- glmmTMB(log(1-total_cover_prop) ~ poly(YSR,2)* DomSpeciesSuperSimp + poly(Elevation,2)   + (1|Site), data = salt_comm_rest_H, family = "gaussian")
...
joint_tests(M15)
emmeans(M15, pairwise~YSR)
emmeans(M15, pairwise~DomSpeciesSuperSimp)
emmeans(M15, pairwise~YSR|DomSpeciesSuperSimp)
...
joint_tests(M3)
emmeans(M3, pairwise~YSR|Species)
...
M5 <- glmmTMB(Sp_cover_prop ~ Species*(rest_status + Elevation) + Estuary + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = beta_family(link = "logit"), ziformula = ~ 1, data = salt_comm_comp_long_less)
...
scatter.smooth(predict(M5, re.form = NA), abs(resid)) # not a trend with absolute values, therefore ok

emmeans(M5, pairwise~rest_status|Species) # test research question
joint_tests(M5) # test effects of covariates
k <- emtrends(M5, var = "Elevation", ~Species) # confidence intervals for slopes of elevation for each species
plot(k) # plot these confidence intervals

summary(M5) # can't interpret these p values for my research question but can give an idea of the importance of random effects
```
...
M7 <- lmer(log(1-(total_cover_prop - 0.0001)) ~ rest_status * Estuary + Elevation +  DomSpeciesSuperSimp + (1|Site), data = salt_comm)
plot(M7) # looks great
plotQQunif(M7) # ok

joint_tests(M7) # test effects of covariates
...
M5 <- glmmTMB(sqrt(Stems) ~ Species * rest_status + Species * Elevation + DomSpeciesSuperSimp + (1|Estuary/Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "gaussian", data = stem_dens_unique_less)

M6 <- glmmTMB(sqrt(Stems) ~ Species * rest_status + Species * Elevation + DomSpeciesSuperSimp + Estuary + (1|Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "gaussian", data = stem_dens_unique_less)
...
resid <- residuals(simulateResiduals(M6), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M6, re.form = NA), resid) # ok 1 outlier
plotQQunif(M6)



summary(M5)
emmeans(M5, pairwise~rest_status|Species)
joint_tests(M5)
k <- emtrends(M5, var = "Elevation", ~Species)
plot(k)

ranef(M5)


emmeans(M6, pairwise~rest_status|Species)
```
...
scatter.smooth(predict(M10, re.form = NA), abs(resid)) # pretty smooth
