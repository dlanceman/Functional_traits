...
fit_cover4 <- manyany(SaltComm ~ YSR + Elevation + Site, "glm", data = salt_comm_comp_H_res, family=tweedie(link = 0, var.power=1.1), var.power = 1.1)

plot(fit_cover4) # residuals look ok - assumptions ok

# test importance of YSR by comparing it to a model without YSR 
fit_cover_null <- manyany(SaltComm ~ Elevation + Site, "glm", data = salt_comm_comp_H_res, family=tweedie(link = 0, var.power=1.1), var.power = 1.1) # only works at max var.power = 1.4
...
M3 <- glmmTMB(cover_prop ~ Species:(YSR + Elevation) + (1|Site) + rr(Species+0|Combo, d = 1), data = comm_comp_long, family = tweedie(link = "log"))
...
M1 <- glmmTMB(Sp_cover_prop ~ rest_status * Species * Elevation + Estuary + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = beta_family(link = "logit"), ziformula = ~ 1, data = salt_comm_comp_long_less)

resid <- residuals(simulateResiduals(M1), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M1, re.form = NA), resid) 
scatter.smooth(predict(M1, re.form = NA), abs(resid))

predicted <- predict(M1, re.form = NA)

with_predict <- bind_cols(salt_comm_comp_long_less, predicted = predicted)
```
...
glmer(cbind(Sporobolus, Sarcocornia, Pneumatophores, Triglochin) ~ rest_status * Elevation + (1|Estuary/Site), data = salt_comm_comp_less, family = beta_family(link = "logit"))
...
M2 <- glmmTMB(sqrt(Stems) ~ Species:(rest_status * Elevation) + (1|Estuary/Site/Combo) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "gaussian", data = stem_dens_unique_less)
resid <- residuals(simulateResiduals(M2), quantilefunction = qnorm, outlierValues = c(-7,7))
...
M2 <- glmmTMB(Stem_dia_mm ~ Species:(rest_status * Elevation + sp_prop) + (1|Estuary/Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
...
# log
M3 <- glmmTMB(log(Stem_dia_mm) ~ Species:(rest_status * Elevation + sp_prop) + (1|Estuary/Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
resid <- residuals(simulateResiduals(M3), quantilefunction = qnorm, outlierValues = c(-7,7))
...
# sqrt
M4 <- glmmTMB(sqrt(Stem_dia_mm) ~ Species:(rest_status * Elevation + sp_prop) + (1|Estuary/Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
# model convergence problem

M5 <- glmmTMB(sqrt(Stem_dia_mm) ~ Species:(rest_status * Elevation) + (1|Estuary/Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
# still model convergence problem without sp_prop

M6 <- glmmTMB(sqrt(Stem_dia_mm) ~ Species:(rest_status + Elevation) + (1|Estuary/Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
...
M7 <- glmmTMB(Stem_dia_mm ~ Species:(rest_status * Elevation + sp_prop) + (1|Estuary/Site/Combo) + rr(Species+0|Combo, d = 2), family = Gamma(link = "inverse"), data = salt_less)
# 44 warnings
resid <- residuals(simulateResiduals(M7), quantilefunction = qnorm, outlierValues = c(-7,7))
...
M8 <- glmmTMB(Stem_dia_mm ~ Species:(rest_status * Elevation + sp_prop) + rest_status * Estuary + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
# doesn't work when estuary is in the bracket with species
resid <- residuals(simulateResiduals(M8), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M8)) # not good
plotQQunif(M8) # not good

M9 <- glmmTMB(log(Stem_dia_mm) ~ Species:(rest_status * Elevation + sp_prop) + rest_status * Estuary + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
# doesn't work when estuary is in the bracket with species
resid <- residuals(simulateResiduals(M9), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M9)) # pretty decent
plotQQunif(M9) # not good

M10 <- glmmTMB(log(Stem_dia_mm) ~ Species:(rest_status + Elevation + sp_prop) + rest_status * Estuary + (1|Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
resid <- residuals(simulateResiduals(M10), quantilefunction = qnorm, outlierValues = c(-7,7))
...
M2 <- glmmTMB(Stem_ht_mm ~ Species:(rest_status * poly(Elevation,2) + sp_prop) + (1|Estuary/Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
# model convergence problems

M3 <- glmmTMB(Stem_ht_mm ~ Species:(rest_status * poly(Elevation,2)) + (1|Estuary/Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
# model convergence problem

M4 <- glmmTMB(Stem_ht_mm ~ Species:(rest_status * Elevation + sp_prop) + (1|Estuary/Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
# model convergence problem

M5 <- glmmTMB(Stem_ht_mm ~ Species:(rest_status + Elevation + sp_prop) + (1|Estuary/Site/Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
# model convergence problem
