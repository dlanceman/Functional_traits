...
# however, this isn't looking for YSR on its own which is what I'm actually interested in
M13 <- lm(log(1-salt_comm_rest_H$total_cover_prop) ~ poly(cbind(YSR,Elevation),2) + YSR + DomSpeciesSuperSimp, data = salt_comm_rest_H, na.action = na.exclude)
plot(M13) # plots look good enough - the data are ~1 inflated so that's going to show up regardless but the spread looks way better now.
summary(M13) # comes up with NAs for YSR?
```
...
M10 <- glmmTMB(Burrows_0.5x0.5 ~ poly(YSR,2)* Elevation + DomSpeciesSuperSimp, data = salt_comm_rest_H, family = "nbinom2")

M11 <- glmmTMB(Burrows_0.5x0.5 ~ poly(YSR,2)* Elevation + DomSpeciesSuperSimp, data = salt_comm_rest_H, family = "nbinom2", ziformula =~1)
...
# I think the count part of the model models the likelihood of burrows given that burrows may exist on site whereas the zero-inflated part might model the likelihood that there are just no burrows on site?

# interpreting model with coefficients
expCoef <- exp(coef((M9)))
expCoef <- matrix(expCoef, ncol = 2)

rownames(expCoef) <- c("(intercept)","poly(YSR,2)1","poly(YSR,2)2","Elevation", "DomSpeciesSuperSimpSarcocornia", "DomSpeciesSuperSimpSporobolus", "poly(YSR,2)1:Elev", "poly(YSR,2)2:Elev")
colnames(expCoef) <- c("Count_model","Zero_inflation_model")
expCoef

# trying to interpret the count model. 
# - the baseline number of crabs is 0.075.
# - one unit increase in the first "arm" of YSR polynomial corresponds to 500279700 x more crabs (???)
# - one unit increase in the second "arm" of YSR polynomial corresponds to 3898 x more crabs (???)
# - one unit increase in elevation corresponds to 0.006 less crabs
```
...
fit_cover <- manyany(SaltComm ~ YSR + Elevation, "glm", data = salt_comm_comp_H_res, family=tweedie(var.power=1.1, link.power = 0), var.power = 1.1)
residuals(fit_cover)
predict(fit_cover)
summary(fit_cover)

# check residuals vs fitted values plot
plot.manyglm(fit_cover)
...
fit_cover2 <- manyany(SaltComm ~ YSR * Elevation, "glm", data = salt_comm_comp_H_res, family=tweedie(link = 0, var.power=1.1), var.power = 1.1)
...
# testing if it works for an individual species
fit_cover3 <- glm(Sporobolus ~ YSR * Elevation, data = salt_comm_comp_H_res, family=tweedie(link = 0, var.power=1.1))
...
# in this case can we do a generalised linear model?

# try negative binomial as that's what worked above

# need to include an offset for quadrat size.

stem_mod5 <- glm.nb(Stems ~ Species * YSR * Elevation + DomSpeciesSimp + sp_prop +  offset(log(Quadrat_area)), data = stem_dens_unique_H_res, control=glm.control(maxit=50))

plot(stem_mod5) # plots don't look good - not straight lines...

stem_mod6 <- glm.nb(Stems ~ Species * YSR * Elevation + DomSpeciesSuperSimp + sp_prop +  offset(log(Quadrat_area)), data = stem_dens_unique_H_res, control=glm.control(maxit=50))

plot(stem_mod6) # almost identical (bad) plots to the above
```
...
# try recoding character vectors as factors
reprod_wide$reprod_type <- as.factor(reprod_wide$reprod_type)
reprod_wide$DomSpeciesSuperSimp <- as.factor(reprod_wide$DomSpeciesSuperSimp)
reprod_wide$DomSpeciesSimp <- as.factor(reprod_wide$DomSpeciesSimp)

#reprod_mod1 <- manyglm(reprod_mvabund ~ YSR * Elevation + DomSpeciesSuperSimp + reprod_type, data = reprod_wide, family = "binomial", na.rm = TRUE)
# coming up with a fatal error!
...
salt_H_rest_stems <- salt_H_rest %>% subset(Species %in% c("Aster", "Bolboschoenus", "Phragmites", "Pneumatophores", "Sarcocornia", "Sporobolus", "Triglochin"))
...
hist(salt_H_rest_stems$Stem_ht_mm) # highly right skewed

# explore relationship between dependent and predictor variables
plot(salt_H_rest_stems$Stem_ht_mm ~ salt_H_rest_stems$YSR) # not clearly non-linear
plot(salt_H_rest_stems$Stem_ht_mm ~ salt_H_rest_stems$Elevation) # not clearly non-linear

# model stem height
M1 <- lmer(Stem_ht_mm ~ Species * YSR * Elevation + DomSpeciesSimp + sp_prop + (1|Quadrat.f), data = salt_H_rest_stems, na.action = na.exclude)

# residuals plots
plot(M1) # not ideal variance plot
qqnorm(resid(M1)) 
qqline(resid(M1)) # not good normality plot

# try log-transformation
M2 <- lmer(log(Stem_ht_mm) ~ Species * YSR * Elevation + DomSpeciesSimp + sp_prop + (1|Quadrat.f), data = salt_H_rest_stems, na.action = na.exclude)

# residuals plots
plot(M2) # variances look ok
qqnorm(resid(M2)) 
qqline(resid(M2)) #normality is better but still not good - lower values are off

# try sqrt-transformation
M3 <- lmer(sqrt(Stem_ht_mm) ~ Species * YSR * Elevation + DomSpeciesSimp + sp_prop + (1|Quadrat.f), data = salt_H_rest_stems, na.action = na.exclude)

# residuals plots
plot(M3) # variances not great
qqnorm(resid(M3)) 
qqline(resid(M3)) #normality not good

# try DomSpeciesSuperSimp with log transform
M4 <- lmer(log(Stem_ht_mm) ~ Species * YSR * Elevation + DomSpeciesSuperSimp + sp_prop + (1|Quadrat.f), data = salt_H_rest_stems, na.action = na.exclude)

# residuals plots
plot(M4) # variances look ok not amazing
qqnorm(resid(M4)) 
qqline(resid(M4)) # not any better than M2

# try gamma distribution
M5 <- glmer(Stem_ht_mm ~ Species * YSR * Elevation + DomSpeciesSimp + sp_prop + (1|Quadrat.f), data = salt_H_rest_stems, family = Gamma(), na.action = na.exclude)
# errors
```
...
# explore distribution
hist(salt_H_rest_stems$Stem_dia_mm) # highly right skewed

# explore relationship between dependent and predictor variables
plot(salt_H_rest_stems$Stem_dia_mm ~ salt_H_rest_stems$YSR) # not clearly non-linear
plot(salt_H_rest_stems$Stem_dia_mm ~ salt_H_rest_stems$Elevation) # not clearly non-linear

# model stem height
M1 <- lmer(Stem_dia_mm ~ Species * YSR * Elevation + DomSpeciesSimp + sp_prop + (1|Quadrat.f), data = salt_H_rest_stems, na.action = na.exclude)

# residuals plots
plot(M1) # not amazing but might be ok
qqnorm(resid(M1)) 
qqline(resid(M1)) # not good normality plot

# try log-transformation
M2 <- lmer(log(Stem_dia_mm) ~ Species * YSR * Elevation + DomSpeciesSimp + sp_prop + (1|Quadrat.f), data = salt_H_rest_stems, na.action = na.exclude)

# residuals plots
plot(M2) # not great but ok
qqnorm(resid(M2)) 
qqline(resid(M2)) # normality not ok

# try sqrt-transformation
M3 <- lmer(sqrt(Stem_dia_mm) ~ Species * YSR * Elevation + DomSpeciesSimp + sp_prop + (1|Quadrat.f), data = salt_H_rest_stems, na.action = na.exclude)

# residuals plots
plot(M3) # pretty good
qqnorm(resid(M3)) 
qqline(resid(M3)) # pretty good

#therefore use M3
summary(M3)

# results are very different when elevation is rescaled vs not....
```
