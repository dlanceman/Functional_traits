...
plotting_single_elevation$predict_cover <- predict(M14, newdata = plotting_single_elevation, type = "response")

# backtransform data 
plotting_single_elevation <- plotting_single_elevation %>% 
  mutate(Cover = (1 - exp(predict_cover))*100)

ggplot(data = plotting_single_elevation, aes(x = YSR, y = Cover)) +
  geom_point() +
  facet_wrap(~DomSpeciesSuperSimp) +
  ylim(0,100)


# try at multiple levels of elevation
...
plotting_space$predict_cover <- predict(M14, newdata = plotting_space, type = "response")

# backtransform data 
plotting_space <- plotting_space %>% 
  mutate(Cover = (1 - exp(predict_cover))*100)

ggplot(data = plotting_space, aes(x = YSR, y = Cover, color = Elevation)) +
  geom_point() +
  facet_wrap(~DomSpeciesSuperSimp) +
  ylim(0,100)
```
...
fit_cover3 <- manyany(SaltComm ~ YSR + Elevation, "glm", data = salt_comm_comp_H_res, family=tweedie(link = 0, var.power=1.1), var.power = 1.1)
...
fit_cover4 <- manyany(SaltComm ~ YSR + Elevation, "glm", data = salt_comm_comp_H_res, family=tweedie(link = 0, var.power=1.4), var.power = 1.4)

plot(fit_cover4) # residuals look ok - assumptions ok

# test importance of YSR by comparing it to a model without YSR 
fit_cover_null <- manyany(SaltComm ~ Elevation, "glm", data = salt_comm_comp_H_res, family=tweedie(link = 0, var.power=1.4), var.power = 1.4) # only works at max var.power = 1.4
...
reprod_proportions <- reprod_long %>% 
  ...
  summarise(avg = mean(Reproduction), se = sd(Reproduction)/sqrt(n()), YSR = mean(YSR)) %>% 
  ungroup()
...
# set up with Species properly - Doesn't work...
...
plot(predict(M4), residuals(M4)) # not that even
plotQQunif(M4)



# play around with variables with the sqrt model
M5 <- glmmTMB(sqrt(Stem_dia_mm) ~ Species:(YSR * Elevation + sp_prop) + DomSpeciesSuperSimp + (1|Combo/Site) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_dia)
...
plot(predict(M6), residuals(M6)) # looks weird with two distinct clumps. but maybe ok
...
plot(predict(M7), residuals(M7)) # looks weird with two distinct clumps. but maybe ok
...
plotQQunif(M8) # not good

# play around - no combos are making normality look ok.
M9 <- glmmTMB(sqrt(Stem_dia_mm) ~ Species * YSR + Elevation  + sp_prop + DomSpeciesSuperSimp + (1|Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_dia)

plot(predict(M9), residuals(M9)) # looks good
plotQQunif(M9)
```
...
# sqrt model
M5 <- glmmTMB(sqrt(Stem_ht_mm) ~ Species:(YSR * Elevation + sp_prop) + DomSpeciesSuperSimp + (1|Combo/Site) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht)
# have to exclude DomSpecies
...
M6 <- glmmTMB(log(Stem_ht_mm) ~ Species:(YSR + Elevation + sp_prop) + DomSpeciesSuperSimp + (1|Combo/Site) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht)

plot(predict(M6), residuals(M6)) # a few outliers but probably ok?
plotQQunif(M6) # looks good 

M7 <- glmmTMB(sqrt(Stem_ht_mm) ~ Species:(YSR + Elevation + sp_prop) + DomSpeciesSuperSimp + (1|Combo/Site) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht)
...
M1 <- glmmTMB(Sp_cover_prop ~ rest_status * Species * Elevation + (1|Combo/Site/Estuary) + rr(Species+0|Combo, d = 1), family = beta_family(link = "logit"), ziformula = ~ 1, data = salt_comm_comp_long_less)

resid <- residuals(simulateResiduals(M1), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M1, REform = NA, type = "response"), resid) # not sure how/where to include REform?
scatter.smooth(predict(M1, type = "response"), resid) # not good! trend and clumping and a couple outliers
 
```


```{r Community composition - Multivariate mixed model}
# can I try a mixed model with a cbind multivariate response variable?

salt_comm_comp_less <- salt_comm_comp %>% 
  dplyr::select(-c("Aster", "Phragmites", "Bolboschoenus", "Juncus_kraussii")) %>% 
  mutate(Sporobolus = case_when(Sporobolus == 0 ~ 0.0000001,
                                .default = Sporobolus/100 - 0.0000001),
         Sarcocornia = case_when(Sarcocornia == 0 ~ 0.0000001,
                                .default = Sarcocornia/100 - 0.0000001),
         Triglochin = case_when(Triglochin == 0 ~ 0.0000001,
                                .default = Triglochin/100 - 0.0000001),
         Pneumatophores = case_when(Pneumatophores == 0 ~ 0.0000001,
                                .default = Pneumatophores/100 - 0.0000001))

glmer(cbind(Sporobolus, Sarcocornia, Pneumatophores, Triglochin) ~ rest_status * Elevation + (1|Site/Estuary), data = salt_comm_comp_less, family = beta_family(link = "logit"))
# no, this is not correct syntax


# instead, follow instructions here: https://mac-theobio.github.io/QMEE/lectures/MultivariateMixed.notes.html#a-trick-to-do-multivariate-mixed-models-using-lme4

# use salt_comm_comp_long_less variable computed in the chunk above

# try with lme4

glmer((Sp_cover_prop + 0.000001) ~ Species:(rest_status*Elevation) - 1 + (Species - 1|Combo) + (Species - 1|Site/Estuary), data = salt_comm_comp_long_less, family = beta_family(link = "logit"),
...
# fit fixed effects part of model
fmla.MMLM1  <- cbind(Sporobolus, Sarcocornia, Triglochin, Pneumatophores) ~
    Species:(rest_status*Elevation) - 1 
# set distribution of response variables
fam.test <- rep("multinomial", 4 ) # multinomial is a generalised version of the binomial distribution
# MCMCglmm is a fundamentally bayesian approach and requires priors. use default priors since I don't understand how these work
prior.model.1 <- list( R = list(V=diag(4)/4, nu=0.004),  
                       G = list(G1=list(V=diag(4)/4, nu=0.004)))
# 4 x 4 matrix as we have 4 species
# fit the model
MCMCglmm(fmla.MMLM1,
                          random=~ us(Species):Site/Estuary, # us = unstructured covariance matrix
                          rcov=~ us(Species):Combo,
                          prior=  prior.model.1,
                          data= salt_comm_comp_less, 
                          family = fam.test, 
                          nitt= 10000, burnin= 2000, thin=10)
# this is not working, and I don't quite understand it, so it might be too complex...
```

Could try mvabund with Estuary as a fixed variable and no Site random effect?

```{r Community composition - try mvabund}
SaltCommNR <- mvabund(salt_comm_comp[,1:8])

# check out spread of data
par(mar = c(2, 10, 2, 2)) # adjusts the margins
boxplot(salt_comm_comp[,1:8], horizontal = TRUE, las = 2, main = "Percent cover")
# large differences in variance, therefore check out mean-variance relationship

# check out mean-variance relationship
meanvar.plot(SaltCommNR)
# strong mean-variance relationship (close to linear but slightly curved like a negative quadratic function), therefore use a model that appropriately accounts for this

# roughly visualise data
plot(SaltCommNR ~ as.factor(salt_comm_comp$rest_status))



# model with2 no interaction between Elevation and YSR

fit_cover <- manyany(SaltCommNR ~ rest_status + Elevation + Estuary + Site, "glm", data = salt_comm_comp, family=tweedie(link = 0, var.power=1.1), var.power = 1.1) # doesn't work



# above doesn't work so likely delete all this
sum(fit_cover$logL) # 
...
LogL_vp <- rep(NA, length(vpwrs))
for(i in 1:length(vpwrs)){ 
  fit_vp <- manyany(SaltCommNR ~ rest_status + Elevation, "glm", data = salt_comm_comp, family=tweedie(link = 0, var.power=vpwrs[i]), var.power=vpwrs[i])
  LogL_vp[i] <- sum(fit_vp$logL)
}
...
# should create new model with var.power = 1.6. However, later at the null model stage, this doesn't work. Try lower values - 1.2 is the highest value that works
fit_cover2 <- manyany(SaltCommNR ~ rest_status + Elevation, "glm", data = salt_comm_comp, family=tweedie(link = 0, var.power=1.2), var.power = 1.2)

plot(fit_cover2) # residuals look ok - assumptions ok

# test importance of rest_status by comparing it to a model without rest_status 
fit_cover_null <- manyany(SaltCommNR ~ Elevation, "glm", data = salt_comm_comp, family=tweedie(link = 0, var.power=1.2), var.power = 1.2) # only works at max var.power = 1.4

# test for differences between the models
anova(fit_cover_null, fit_cover2) 
# the two models have different numbers of terms - why?
```
...
M6 <- lmer(log(1-salt_comm$total_cover_prop) ~ rest_status * Elevation +  DomSpeciesSuperSimp + (1|Site/Estuary), data = salt_comm, na.action = na.exclude)
plot(M6) # looks great
plotQQunif(M6) # ok - works with estuary as a random and not fixed effect (but aren't we interested in the fixed effect?)
...
# try negative binomial in glmmTMB - might be ok without zero inflation
M1 <- glmmTMB(Burrows_0.5x0.5 ~ rest_status * Elevation + DomSpeciesSuperSimp + (1|Site/Estuary), data = salt_comm_burrows, family = "nbinom2")
resid <- residuals(simulateResiduals(M1), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M1, type = "response"), resid) # not ok
...
resid <- residuals(simulateResiduals(M2), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M2, type = "response"), resid) # not ok
...
scatter.smooth(predict(M3, type = "response"), resid) # not ok!
# didn't work with estuary as part of the random factor

M4 <- glmmTMB(Burrows_0.5x0.5 ~ rest_status * Elevation + DomSpeciesSuperSimp + (1|Site/Estuary), data = salt_comm_burrows, family = "poisson", ziformula =~1)
resid <- residuals(simulateResiduals(M4), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M4, type = "response"), resid) # not ok
...
resid <- residuals(simulateResiduals(M5), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M5, type = "response"), resid) # not ok
...
scatter.smooth(predict(M6, type = "response"), resid) # not ok
...
scatter.smooth(predict(M7, type = "response"), resid) # not ok
...
scatter.smooth(predict(M8, type = "response"), resid) # not ok



# without elevation interaction?
M9 <- glmmTMB(Burrows_0.5x0.5 ~ rest_status + Elevation +  DomSpeciesSuperSimp + Estuary * rest_status + (1|Site), data = salt_comm_burrows, family = "nbinom2")
resid <- residuals(simulateResiduals(M9), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M9, type = "response"), resid) # not ok

# try zero inflated negative binomial model in glmmTMB
M10 <- glmmTMB(Burrows_0.5x0.5 ~ rest_status + Elevation + DomSpeciesSuperSimp + Estuary * rest_status + (1|Site), data = salt_comm_burrows, family = "nbinom2", ziformula =~1)
resid <- residuals(simulateResiduals(M10), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M10, type = "response"), resid) # not ok

# try poisson?
M11 <- glmmTMB(Burrows_0.5x0.5 ~ rest_status + Elevation + DomSpeciesSuperSimp + Estuary * rest_status + (1|Site), data = salt_comm_burrows, family = "poisson")
resid <- residuals(simulateResiduals(M11), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M11, type = "response"), resid) # not ok

M12 <- glmmTMB(Burrows_0.5x0.5 ~ rest_status + Elevation + DomSpeciesSuperSimp + Estuary * rest_status + (1|Site), data = salt_comm_burrows, family = "poisson", ziformula =~1)
resid <- residuals(simulateResiduals(M12), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M12, type = "response"), resid) # not ok


# try log-transformed and gaussian distribution
M13 <- glmmTMB(log(Burrows_0.5x0.5) ~ rest_status * Elevation +  DomSpeciesSuperSimp + Estuary * rest_status + (1|Site), data = salt_comm_burrows, family = "gaussian")
resid <- residuals(simulateResiduals(M13), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M13, type = "response"), resid)
```
...
M1 <- glmmTMB(Stems ~ Species:(rest_status * Elevation) + (1|Combo/Site/Estuary) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "nbinom2", data = stem_dens_unique_less)
resid <- residuals(simulateResiduals(M1), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M1))
# this has a trend
plotQQunif(M1) # good

# try a gaussian distribution with sqrt transformed data?
M2 <- glmmTMB(sqrt(Stems) ~ Species:(rest_status * Elevation) + (1|Combo/Site/Estuary) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "gaussian", data = stem_dens_unique_less)
resid <- residuals(simulateResiduals(M2), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M2)) # this has a trend too
plotQQunif(M2) # not great but ok..?

# try a gaussian distribution with log transformed data?
M3 <- glmmTMB(log(Stems) ~ Species:(rest_status * Elevation) + (1|Combo/Site/Estuary) + rr(Species+0|Combo, d = 1) + offset(log(Quadrat_area)), family = "gaussian", data = stem_dens_unique_less)
resid <- residuals(simulateResiduals(M3), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M3)) # bit of a trend but pretty good. one outlier. maybe good enough?
...
M1 <- glmmTMB(Stem_dia_mm ~ Species:(rest_status * Elevation + DomSpeciesSuperSimp + sp_prop) + (1|Combo/Site/Estuary) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
# rank deficient and model convergence problem

M2 <- glmmTMB(Stem_dia_mm ~ Species:(rest_status * Elevation + sp_prop) + (1|Combo/Site/Estuary) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
# works without DomSpeciesSuperSimp
resid <- residuals(simulateResiduals(M2), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M2)) # terrible
plotQQunif(M2) # terrible

# log
M3 <- glmmTMB(log(Stem_dia_mm) ~ Species:(rest_status * Elevation + sp_prop) + (1|Combo/Site/Estuary) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
resid <- residuals(simulateResiduals(M3), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M3)) # very clumped (likely due to the nature of the data) and not great but a lot better than the previous
plotQQunif(M3) # terrible

# sqrt
M4 <- glmmTMB(sqrt(Stem_dia_mm) ~ Species:(rest_status * Elevation + sp_prop) + (1|Combo/Site/Estuary) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
# model convergence problem

M5 <- glmmTMB(sqrt(Stem_dia_mm) ~ Species:(rest_status * Elevation) + (1|Combo/Site/Estuary) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
# still model convergence problem without sp_prop

M6 <- glmmTMB(sqrt(Stem_dia_mm) ~ Species:(rest_status + Elevation) + (1|Combo/Site/Estuary) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
# works without sp_prop and without interaction
resid <- residuals(simulateResiduals(M6), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M6)) # not great
plotQQunif(M6) # not good

# try different distribution type?
M7 <- glmmTMB(Stem_dia_mm ~ Species:(rest_status * Elevation + sp_prop) + (1|Combo/Site/Estuary) + rr(Species+0|Combo, d = 2), family = Gamma(link = "inverse"), data = salt_less)
# 44 warnings
resid <- residuals(simulateResiduals(M7), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M7)) # not great
plotQQunif(M7)
# apparently infinite residuals values have been created

# try estuary as a fixed value
M8 <- glmmTMB(Stem_dia_mm ~ Species:(rest_status * Elevation + sp_prop) + rest_status * Estuary + (1|Combo/Site) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
# doesn't work when estuary is in the bracket with species
resid <- residuals(simulateResiduals(M8), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M8)) # not good
plotQQunif(M8) # not good

M9 <- glmmTMB(log(Stem_dia_mm) ~ Species:(rest_status * Elevation + sp_prop) + rest_status * Estuary + (1|Combo/Site) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
# doesn't work when estuary is in the bracket with species
resid <- residuals(simulateResiduals(M9), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M9)) # pretty decent
plotQQunif(M9) # not good

M10 <- glmmTMB(log(Stem_dia_mm) ~ Species:(rest_status + Elevation + sp_prop) + rest_status * Estuary + (1|Combo/Site) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
resid <- residuals(simulateResiduals(M10), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M10)) # pretty decent
plotQQunif(M10) # not good

M11 <- glmmTMB(log(Stem_dia_mm+0.2) ~ Species:(rest_status + Elevation + sp_prop) + rest_status * Estuary + (1|Combo/Site) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
resid <- residuals(simulateResiduals(M11), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M11)) # pretty decent
...
M1 <- glmmTMB(Stem_ht_mm ~ Species:(rest_status * poly(Elevation,2) + DomSpeciesSuperSimp + sp_prop) + (1|Combo/Site/Estuary) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
# rank deficient and model convergence problems

M2 <- glmmTMB(Stem_ht_mm ~ Species:(rest_status * poly(Elevation,2) + sp_prop) + (1|Combo/Site/Estuary) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
# model convergence problems

M3 <- glmmTMB(Stem_ht_mm ~ Species:(rest_status * poly(Elevation,2)) + (1|Combo/Site/Estuary) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
# model convergence problem

M4 <- glmmTMB(Stem_ht_mm ~ Species:(rest_status * Elevation + sp_prop) + (1|Combo/Site/Estuary) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
# model convergence problem

M5 <- glmmTMB(Stem_ht_mm ~ Species:(rest_status + Elevation + sp_prop) + (1|Combo/Site/Estuary) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
# model convergence problem

M6 <- glmmTMB(Stem_ht_mm ~ Species:(rest_status + Elevation + sp_prop) + Estuary * rest_status + (1|Combo/Site) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
...
# try sqrt
M7 <- glmmTMB(sqrt(Stem_ht_mm) ~ Species:(rest_status + Elevation + sp_prop) + Estuary * rest_status + (1|Combo/Site) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
resid <- residuals(simulateResiduals(M7), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M7)) # ok spread but has a trend
plotQQunif(M7) # perfect

# try other sqrt models that have a better residuals plot?
M8 <- glmmTMB(sqrt(Stem_ht_mm) ~ Species:(rest_status + poly(Elevation,2) + sp_prop) + Estuary * rest_status + (1|Combo/Site) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
resid <- residuals(simulateResiduals(M8), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M8)) # still has a trend
plotQQunif(M8) # perfect

# log model
M9 <- glmmTMB(log(Stem_ht_mm) ~ Species:(rest_status + poly(Elevation,2) + sp_prop) + Estuary * rest_status + (1|Combo/Site) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_less)
resid <- residuals(simulateResiduals(M9), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M9)) # trend + uneven spread
plotQQunif(M9) # not good

# therefore M8 is closest to ok
...
M1 <- glmmTMB(Reproduction ~ Species:(rest_status * reprod_type + Elevation) + (1+reprod_type|Species) + (1|Combo/Site/Estuary), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_long)
...
M2 <- glmmTMB(Reproduction ~ Species:(rest_status * reprod_type + Elevation) + (1|Combo/Site/Estuary), contrasts = list(reprod_type = "contr.sum"), family = "binomial", data = reprod_all_nosarc)
