...
stem_dens_wide_H_res[,9:29] %>% colSums(stem_dens_wide_H_res != 0)
# add another row of code here to subset for only the columns with at least 5 observations
...
# do I want to do a simplified DomSpecies variable?
# interactions??
```
...
# add code here for also removing columns with <5 observations of that species

# set up mvabund with reproduction columns - these are 10-16 (?)
...
# transform this to wide format with a column for each species' stem height 
...
# remove columns that only have NAs / sum to zero (i.e. no heights)
stem_ht_wide_H_res[,9:33] %>% colSums() # the last four columns are all zeroes
stem_ht_wide_H_res <- stem_ht_wide_H_res[,1:29]

# also remove columns with <5 values (can't do stats on rare species as insufficient data)
stem_ht_wide_H_res[,9:29] %>% colSums(stem_ht_wide_H_res != 0)
# add another row of code here to subset for only the columns with at least 5 observations

# set up mvabund with stem height columns - these are 9-29
...
# continuous data so model with poisson or negative binomial distribution
