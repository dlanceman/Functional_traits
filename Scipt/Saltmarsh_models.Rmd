...
```{r Total cover - model}
# is there a benefit of using betareg instead of glm? does glm take beta_family properly?
# this is currently not working
M <- betareg(total_cover_prop ~ YSR + DomSpecies + Elevation, data = salt_comm_rest_H, na.action = na.exclude, link = "logit")
summary(M)

#with both linear
M <- glm(total_cover_prop ~ YSR + DomSpecies + Elevation, data = salt_comm_rest_H, family = quasibinomial(link = "logit"), na.action = na.exclude)
summary(M)

# with polynomial YSR and Elevation
M1 <- glm(total_cover_prop ~ poly(YSR,2) + DomSpecies + poly(Elevation,2), data = salt_comm_rest_H, family = beta_family(link = "logit"), na.action = na.exclude)
summary(M1)
# poly YSR are significant
# unvegetated is very significant which makes sense - should this be excluded?
# elevation is significant, but only the first order, therefore run again without polynomial

M2 <- glm(total_cover_prop ~ poly(YSR,2) + DomSpecies + Elevation, data = salt_comm_rest_H, family = beta_family(link = "logit"), na.action = na.exclude)
summary(M2)

# try YSR as a logarithmic variable
M3 <- glm(total_cover_prop ~ log(YSR) + DomSpecies + poly(Elevation,2), data = salt_comm_rest_H, family = beta_family(link = "logit"), na.action = na.exclude)
summary(M3)
```
...
# plot predicted values with ggeffects ggemmeans - linear
...
# plot predicted values with ggeffects ggemmeans - polynomial
...
# plot predicted values with ggeffects ggemmeans with YSR as a log variable
...
plot(predict_YSR, ci = T,add.data = TRUE, dot.alpha = 0.5,  jitter = 0.2)+
  ...
  labs(y = "Total cover (proportion)", x = "Years since restoration") +
  theme_classic()
# this probably looks the most realistic, but not sure if that's the basis of choosing a technique?
...
# 6.71 > 1 - evidence of overdispersion
...
# 1.06e+18 >>> 1 therefore highly overdispersed
...
# 1.93 therefore closer to even..

# compare the different models
AICtab(M1, M2, M3)
# why doesn't this work anymore?
# model was better as a mixed model but site was confounded with YSR. model has worse dispersion when YSR is non-polynomial
...
fit_cover <- manyany(SaltComm ~ YSR + Elevation, "glm", data = salt_comm_comp_H_res, family=tweedie(link = 0), var.power=1.1)
# errors
```
