...
M14 <- lmer(log(1-salt_comm_rest_H$total_cover_prop) ~ poly(YSR,2) + poly(Elevation,2) + DomSpeciesSuperSimp + (1|Site), data = salt_comm_rest_H, na.action = na.exclude)
...
plotting_space$predict_burrows <- predict(M11, newdata = plotting_space, type = "response")
...
plotting_single_elevation$predict_burrows <- predict(M11, newdata = plotting_single_elevation, type = "response")
...
# insert quantile 1 and quantile 3 values as Elevation values to use
emmip(M11, Elevation~YSR, at = list(YSR = c(6,9,11,12,19,30,33), Elevation = c(0.143, 0.469), CIs = TRUE, type = "response"))
...
emmip(M11, Elevation~YSR, at = list(YSR = c(6,9,11,12,19,30,33), Elevation = c(0.2,0.4,0.6,0.8)), type = "response")
...
```{r Try relative elevation}
salt_comm_rest_H_burrows <-
  salt_comm_rest_H_burrows %>% group_by(Site) %>% 
  mutate(relative_elevation = scale_this(Elevation)) %>% ungroup()

# visualise scaled distribution for each site
ggplot(data = salt_comm_rest_H_burrows, aes(y = relative_elevation)) +
  geom_histogram() +
  facet_wrap(~Site)

# check that this worked and the mean of each site is very close to zero
salt_comm_rest_H_burrows %>% group_by(Site) %>% summarise(mean(relative_elevation))

# visualise relationship between relative_elevation and burrows
plot(salt_comm_rest_H_burrows$Burrows_0.5x0.5 ~ salt_comm_rest_H_burrows$relative_elevation) # no obvious trend, include as a linear factor
plot(salt_comm_rest_H_burrows$relative_elevation ~ salt_comm_rest_H_burrows$YSR)

# try negative binomial in glmmTMB - might be ok without zero inflation
M12 <- glmmTMB(Burrows_0.5x0.5 ~ poly(YSR,2)* relative_elevation + DomSpeciesSuperSimp + (1|Site), data = salt_comm_rest_H_burrows, family = "nbinom2")

# try zero inflated negative binomial model in glmmTMB
M13 <- glmmTMB(Burrows_0.5x0.5 ~ poly(YSR,2)* relative_elevation + DomSpeciesSuperSimp + (1|Site), data = salt_comm_rest_H_burrows, family = "nbinom2", ziformula =~1)
...
resid <- residuals(simulateResiduals(M13), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M13,re.form = NA), resid) # looks a bit odd
hist(resid) # check distribution of residuals - should be normal
scatter.smooth(abs(resid), predict(M13,re.form = NA)) # test homogeneity of variance - looks fine with absolute values so should be ok

anova(M12,M13)
# no significance here means the zero inflated part of the model does not explain more of the variation, so therefore we should use the non zero inflated model - M12

summary(M12)

# for plotting, explore data space for each DomSpecies

# also need to know how this is for each DomSpeciesSuperSimp.
t <- salt_comm_rest_H_burrows %>% subset(DomSpeciesSuperSimp == "Sporobolus") 
plot(t$relative_elevation ~ t$YSR) # can only predict high relative elevations (0 to 1.5) for 6,11 and 12 years and larger range (-1 to 1.5) for 16,30,33 YSR
t <- salt_comm_rest_H_burrows %>% subset(DomSpeciesSuperSimp == "Sarcocornia") 
plot(t$relative_elevation ~ t$YSR) # only one data point at YSR 30 (relative elevation ~1), most of the rest of data are YSR 6-12 & elevation -0.7 to 0.5. Also YSR and elevation are correlated for this variable
t <- salt_comm_rest_H_burrows %>% subset(DomSpeciesSuperSimp == "Other") 
plot(t$relative_elevation ~ t$YSR) # only one data point at YSR 33 (relative elevation ~-0.5), rest of data are YSR 11-16 & relative elevation ~-1.5 to 1

plotting_space1 <- tibble(relative_elevation = c(0,0.5,1,1.5,
                                      0,0.5,1,1.5,
                                      0,0.5,1,1.5,
                                      -1,-0.5,0,0.5,1,1.5,
                                      -1,-0.5,0,0.5,1,1.5,
...
plotting_space2 <- tibble(relative_elevation = c(-1,0,1,
                                        -1,0,1,
                                        -1,0,1,
                                        1),
                          YSR = c(6,6,6,
...
plotting_space3 <- tibble(relative_elevation = c(-1.5,-1,-0.5,0,0.5,1,
                                        -1.5,-1,-0.5,0,0.5,1,
                                        -0.5),
                          YSR = c(11,11,11,11,11,11,
                                  16,16,16,16,16,16,
                                  33),
                          Site = NA,
                          DomSpeciesSuperSimp = "Other")

plotting_space_new <- rbind(plotting_space1, plotting_space2, plotting_space3)

plotting_space_new$predict_burrows <- predict(M12, newdata = plotting_space_new, type = "response")

ggplot(data = plotting_space_new, aes(x = YSR, y = predict_burrows, color = relative_elevation, group = relative_elevation)) +
  geom_point() +
  geom_line() +
  facet_wrap(~DomSpeciesSuperSimp) 
...
```{r try with elevation as an offset}
# try negative binomial in glmmTMB - might be ok without zero inflation
M14 <- glmmTMB(Burrows_0.5x0.5 ~ poly(YSR,2) + DomSpeciesSuperSimp + (1|Site) + offset(log(Elevation + 0.3)), data = salt_comm_rest_H_burrows, family = "nbinom2")

# try zero inflated negative binomial model in glmmTMB
M15 <- glmmTMB(Burrows_0.5x0.5 ~ poly(YSR,2) + DomSpeciesSuperSimp + (1|Site) + offset(log(Elevation + 0.3)), data = salt_comm_rest_H_burrows, family = "nbinom2", ziformula =~1)
# model convergence problem

# plot residuals vs predicted values with adjustments for easier interpretation - should see a relatively flat, even spread of residuals with no clear trend
resid <- residuals(simulateResiduals(M14), quantileFunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(predict(M14,re.form = NA), resid) # good
hist(resid) # check distribution of residuals - should be normal
scatter.smooth(abs(resid), predict(M14,re.form = NA)) # test homogeneity of variance - not ok!
```
...
plot(stem_dens_unique_H_res$sp_prop ~ stem_dens_unique_H_res$total_stems) # not correlated
...
resid <- residuals(simulateResiduals(M1), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M1)) # not good!! trend + very extreme values. This is the case whether Estuary is included as a random, fixed and or both factor
...
resid <- residuals(simulateResiduals(M1), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M1)) # I don't think this is ok - trend
...
resid <- residuals(simulateResiduals(M2), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M2)) # also has a trend...
...
scatter.smooth(resid, predict(M3)) # not ok!
# didn't work with estuary as part of the random factor
...
scatter.smooth(resid, predict(M4)) # not ok
...
resid <- residuals(simulateResiduals(M5), quantilefunction = qnorm, outlierValues = c(-7,7))
scatter.smooth(resid, predict(M5)) # not ok
...
scatter.smooth(resid, predict(M6)) # not ok
...
scatter.smooth(resid, predict(M7)) # not ok
...
scatter.smooth(resid, predict(M8))
# this one is ok
