...
First assumption: linear relationship between predictor and response variable
...
(mm_plot <- ggplot(salt_comm, aes(x = YSR, y = ln.canopy.ht, colour = Site)) +   
      geom_point(alpha = 0.5) +
      theme_classic()) 
```
...
(mm_plot <- ggplot(salt_comm, aes(x = YSR, y = total_cover_prop, colour = Site)) +   
      geom_point(alpha = 0.5) +
      theme_classic()) 
```
