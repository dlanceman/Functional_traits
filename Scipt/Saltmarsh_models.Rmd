...
# run code for wide stem density first 
...
stem_dens_long_zeroes <- stem_dens_wide_H_res %>% 
  pivot_longer(cols = c(Sarcocornia, Sporobolus, Triglochin, Pneumatophores, Bolboschoenus, Aster, Phragmites), names_to = "Species", values_to = "Stems") %>% 
  mutate(unique = paste0(Combo,":",Species))

# this dataset does not have species proportion or offset data - we will need to add these

id_data <- stem_dens_unique_H_res %>% dplyr::select(Quadrat_area, sp_prop, unique)
...
nquadrat = nrow(stem_dens_long_zeroes) #number of rows/ quadrats
nBoot = 999 #desired number of bootstraps
...
#In this data, the row names are not the row indexes, otherwise you could skip this step and join on the row names. To be safe we join on row indexes in the next step. 
row_names_row_index_lookup_table = data.frame(Combo = stem_dens_long_zeroes$Combo) %>% mutate(row.index = 1:nquadrat)
row_names_row_index_lookup_table
...
#Now we can run the model with an offset - which could vary by species as the data is long format - using a bootID matrix to make sure sites are resampled together. 
fit_null = manyglm(Stems ~ Elevation * Species + 
                     DomSpeciesSuperSimp + sp_prop + offset(log(Quadrat_area)),
                   data=stem_dens_long_zeroes) # without YSR
fit_null2 = manyglm(Stems ~ YSR * Species + Elevation * Species + 
                     DomSpeciesSuperSimp + sp_prop + offset(log(Quadrat_area)),
                   data=stem_dens_long_zeroes) # without interaction with Elevation
#The alternative model contains the treatment variable/ gradient and it's interaction with species (using : syntax)
fit_alt = manyglm(Stems ~ YSR * Elevation + YSR * Species + Elevation * Species + 
                     DomSpeciesSuperSimp + sp_prop + offset(log(Quadrat_area)),
                   data=stem_dens_long_zeroes)


anova(fit_null, fit_null2, bootID = bootID_long) # testing for importance of YSR
anova(fit_null2, ft_alt, bootID = bootID_long) # testing for importance of YSR * Elevation interaction
```
...
# sqrt model
M3 <- glmmTMB(sqrt(Stem_ht_mm) ~ Species * YSR * Elevation + DomSpeciesSuperSimp * Species + sp_prop * Species + (1|Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht)

M3 <- glmmTMB(sqrt(Stem_ht_mm) ~ Species * YSR * Elevation + DomSpeciesSuperSimp * Species  + (1|Combo) + rr(Species+0|Combo, d = 2), family = "gaussian", data = salt_H_rest_stems_ht)
