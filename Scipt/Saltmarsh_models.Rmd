...
spor_comm <- spor_comm[,-c(19,20,33,34)] # remove irrelevant rows
...
M <- lmer(Canopy_ht_cm ~ YSR + Elevation + Estuary + (1|Site), data = salt_comm, na.action = na.exclude)
summary(M)
...
M2 <- lmer(sq.stem.dens ~ YSR + Elevation + Estuary + (1|Site), data = spor_comm, na.action = na.exclude)
# check whether this model makes sense and whether rest_status:rest_year is the correct way to add rest_year as a fixed factor nested within rest_status
# this model gives a warning message about dropping a variable unless we assign "restoration ages" to the natural sites
summary(M2)
...
```{r Sporobolus stem density - check residuals assumptions}
# check for collinearity between predictors
check_collinearity(M) # VIF < 5 (low collinearity) - good.
...
# 3a again - Sporobolus new models

```{r Sporobolus modelling - data prep}
# new dataframe for Sporobolus Hunter only
spor_H <- spor %>% subset(Estuary == "Hunter")
spor_comm_H <- spor_comm  %>% subset(Estuary == "Hunter")
```


1. Sporobolus stem density, height and diameter

1a. Natural vs restored

Explanatory variables:
- restoration status (binary)
...
```{r Sporobolus stem height - univariate model}
...
plot(spor_H$Stem_ht_mm ~ spor_H$sp_prop) # not obviously non-linear, but clearly stem height is 1-inflated. Need to figure out if this is an issue and if there's anything I can do about it.
plot(spor_H$Stem_ht_mm ~ spor_H$Elevation) # looks non-linear
ggplot(spor_H, aes(x = Elevation, y = Stem_ht_mm)) +
  geom_point() +
  geom_smooth(span = 0.5)
# therefore use a generalised linear model with a non-linear method or include elevation as a non-linear predictor variable. Maybe a cubic function?


# create model
M_spor_H_stem_ht <- lmer(Stem_ht_mm ~ rest_status + poly(Elevation,3) + DomSpecies + sp_prop + (1|Site) + (1|Quadrat.f), data = spor_H, na.action = na.exclude)
...
M_spor_H_stem_ht <- lmer(Stem_ht_mm ~ rest_status + poly(Elevation,2) + DomSpecies + sp_prop + (1|Site) + (1|Quadrat.f), data = spor_H, na.action = na.exclude)
summary(M_spor_H_stem_ht)

# looks like elevation and Juncus dominance influence Sporobolus stem height, but not restoration status (consistent with exploratory plots about restoration status)

# create a simplified model with only the significant factors
M_spor_H_stem_ht <- lmer(Stem_ht_mm ~ poly(Elevation,2) + DomSpecies + (1|Site) + (1|Quadrat.f), data = spor_H, na.action = na.exclude)
summary(M_spor_H_stem_ht)
...
```{r Sporoblus stem density - univariate model}
# test assumptions - linear relationship between predictors and response
# numeric predictors are species proportional cover and elevation
...
ggplot(spor_comm_H, aes(x = sp_prop, y = total_stems)) +
  geom_point() +
  geom_smooth(span = 1) +
  geom_smooth(method = lm) # close enough to linear
...
# might not make sense to include species proportion in the model as it is logically correlated with stem density... rerun without?
```

how to interpret random factors??
...
# create model
M_spor_H_stem_dens <- lmer(total_stems ~ YSR + Elevation + DomSpecies + sp_prop + (1|Site), data = spor_comm_H, na.action = na.exclude)
# do I need to specify nesting here or will the model figure out due to the unique labelling of factors?
summary(M_spor_H_stem_dens)

# YSR is non-significant
