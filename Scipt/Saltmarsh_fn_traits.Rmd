...
plot(spor$total_stems/spor$`Sp_cover_%` ~ spor$YSR)
```
...
First, it's useful to rescale explanatory variables so they have a constant mean of 0 and standard deviation of 1.
...
For us, restoration year is a fixed factor, nested within the fixed factor restoration status. Elevation is also fixed. Site and quadrat are random factors. Estuary is fixed because it only has two levels.

Quadrat is not a variable for quadrat-level measurements though, as there is only one measurement per quadrat!

```{r Model - saltmarsh log canopy height}
M <- lmer(ln.canopy.ht ~ rest_status + rest_status:rest_year + Elevation + Estuary + (1|Site), data = salt, na.action = na.exclude)
...
```{r potential part of assumptions testing or delete it}
# Calculate correlations
cor(salt[, c("rest_status", "rest_year", "Elevation")])
...
```{r Model - log sporobolus stem height}
M <- lmer(ln.stem.ht ~ rest_status + rest_status:rest_year + Elevation + Estuary + (1|Site) + (1|Quadrat.f), data = spor, na.action = na.exclude)
# check whether this model makes sense and whether rest_status:rest_year is the correct way to add rest_year as a fixed factor nested within rest_status
summary(M)
```
...
```{r Model - ln sporobolus stem diameter}
M <- lmer(ln.stem.dia ~ rest_status + rest_status:rest_year + Elevation + Estuary + (1|Site) + (1|Quadrat.f), data = spor, na.action = na.exclude)
# check whether this model makes sense and whether rest_status:rest_year is the correct way to add rest_year as a fixed factor nested within rest_status
summary(M)
```
