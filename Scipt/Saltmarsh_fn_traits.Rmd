...
For us, restoration year is a fixed factor (although not all points have this - need to investigate). Elevation is also fixed. Site and quadrat are random factors. 
...
M <- lmer(ln.canopy.ht ~ rest_year + Elevation + (1|Estuary) + (1|Site), data = salt, na.action = na.exclude) # Error
M <- lmer(ln.canopy.ht ~ Elevation + (1|Estuary) + (1|Site) + (1|Quadrat.f), data = salt, na.action = na.exclude) # same Error
M <- lmer(ln.canopy.ht ~ Elevation + (1|Quadrat.f), data = salt, na.action = na.exclude) #  different Error
M <- lmer(ln.canopy.ht ~ Elevation + (1|Quadrat.f), data = salt, na.action = na.exclude, control = lmerControl(optCtrl = list(maxIter = 10000))) # different error remains
```

Given the constant errors here, let's try another model..
```{r Model - sporobolus stem height}
