...
manleaf <- read_excel("Data/Functional_traits_combined_data.xlsx", sheet = "Mangrove_leaf") %>% 
  ...
  mutate(Value_av = mean(c_across(Leaf_1:Leaf_5), na.rm = TRUE),
         Estuary = case_when( # make an estuary variable
           Site == "YCW" ~ "Maroochy",
           .default = "Hunter"
         ),
         YSR = case_when(
...
manleaf_wide <- manleaf %>% pivot_wider(id_cols = c(Site, Combo, Quadrat, Tree, Tree_sapling, Species, Estuary, YSR, rest_status), names_from = "Measure", values_from = "Value_av") # column for each dry and fresh weight
manleaf_wide <- manleaf_wide %>% mutate(dfratio = Dry_weight/Fresh_weight)
```
...
# rescale/standardise elevation
mantree$Elevation <- scale(mantree$Elevation, center = TRUE, scale = TRUE)
manq$Elevation <- scale(manq$Elevation, center = TRUE, scale = TRUE)
...
manleaf_wide_H <- manleaf_wide %>% subset(manleaf_wide$Estuary == "Hunter")
...
manleaf_wide_rest <- manleaf_wide %>% subset(manleaf_wide$rest_status == 1)
...
manleaf_wide_nat <- manleaf_wide %>% subset(manleaf_wide$rest_status == 0)

# mangrove leaves - trees only
manleaf_wide_t <- manleaf_wide %>% subset(manleaf_wide$Tree_sapling == "Tree")
...
plot(mantree_comm_rest$total_trees ~ mantree_comm_rest$YSR) # not obviously non-linear
plot(mantree_comm_rest$total_trees ~ mantree_comm_rest$Elevation) # not obviously non-linear

M <- lmer(total_trees ~ YSR + Elevation + Estuary + (1|Site), data = mantree_comm_rest, na.action = na.exclude)

summary(M)

# need to include an offset...

M1 <- glm(total_trees ~ YSR + Elevation + Estuary, data = mantree_comm_rest, na.action = na.exclude)
...
M <- lmer(total_saplings ~ poly(YSR,2) + Elevation + Estuary + (1|Site), data = mantree_comm_rest, na.action = na.exclude)
summary(M)

# need to include offset!

# not a mixed model
M1 <- glm(total_saplings ~ poly(YSR,2) + Elevation + Estuary, data = mantree_comm_rest, na.action = na.exclude)
summary(M1)

# with interactions
M2 <- glm(total_saplings ~ poly(YSR,2) * Elevation * Estuary, data = mantree_comm_rest, na.action = na.exclude)
summary(M2) # no significant interactions so run model without interactions
...
plot(M) # fine
qqnorm(resid(M))
qqline(resid(M)) # roughly fine, one slightly extreme value
```
