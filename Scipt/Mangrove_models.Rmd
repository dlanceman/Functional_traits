...
manleafall_long <- manleafall %>% 
  ...
  dplyr::select(-Value_av) # remove average value column
...
# CC1-2 has missing data (area and fresh weight) but all other columns are complete (not including many of the Maroochy leaves, which only have areas)
```
...
# therefore, investigate quadrat with large number of Trees
# it seems realistic - this is the number on the paper datasheet and photos show a high tree density. therefore retain
...
# therefore, don't model propagules, vege % or algae % and investigate large pneumatophore diameter values
...
# mangrove leaf data
cleveland_plot(manleafall_wide$Area, "Area") # right skewed, some large values - should be realistic but check
cleveland_plot(manleafall_wide$Dry_weight, "Dry_weight") # fairly even distribution, some large values but realistic
cleveland_plot(manleafall_wide$Fresh_weight, "Fresh_weight") # fairly even distribution, some large values but realistic
cleveland_plot(manleafall_wide$dfratio, "dfratio") # even spread, with a few outliers on each side - check these out in case there were any mistakes in the order of weight measurements
cleveland_plot(manleafall_wide$LMA, "LMA") # even spread but there are outliers - check these out
cleveland_plot(manherb_long$`Herb_%`, "Herb_%") # right skewed, some large values but realistic

# check leaf areas - largest areas are all for orange and red mangroves, which have bigger leaves, so makes sense.
# remeasured the 15 leaves with the largest leaf areas. Replacing values of leaves where the remeasured value was very different from the original measurement: 
#- CCW4 Orange 1 Leaf 1
manleafall_wide$Area[591] <- 7952
#- CCW3 Orange1 Leaf 3, 4, 5
manleafall_wide$Area[583] <- 5679
manleafall_wide$Area[584] <- 6447
manleafall_wide$Area[585] <- 5459
#- VSR 5 Orange 3 Leaf 4
manleafall_wide$Area[954] <- 5198
#- CCW 2 Orange 1 Leaf 2, 4
manleafall_wide$Area[577] <- 5334
manleafall_wide$Area[579] <- 6039
#- VSR 1 Orange 2 Leaf 3, 4
manleafall_wide$Area[853] <- 4925
manleafall_wide$Area[854] <- 5940

# check dfratios
# no reason to suggest any large values are unrealistic/incorrect except 2 leaf dry weights were in the wrong order - HS2 Tree 7 Leaf 3 and 5
manleafall_wide$Dry_weight[323] # dry weight 0.311
manleafall_wide$Dry_weight[325] # dry weight 0.484
# swap these
manleafall_wide$Dry_weight[323] <- 0.484
manleafall_wide$Dry_weight[325] <- 0.311
# recalculate dfratio
manleafall_wide <- manleafall_wide %>% mutate(dfratio = Dry_weight/Fresh_weight)


# investigate extreme LMA values
# identified some mistakes and fixed those in the original data.
...
mantree_sp <- mantree %>% distinct(Combo, Species, .keep_all = TRUE) 

# hunter estuary
mantree_sp_H <- mantree_sp %>% subset(Estuary == "Hunter")
mantree_sp_H %>% count(Species)

# maroochy estuary
mantree_sp_M <- mantree_sp %>% subset(Estuary == "Maroochy")
mantree_sp_M %>% count(Species)
```
...
```{r Relationships between predictor variables}
# YSR vs elevation
plot(mantree_comm_rest$Elevation ~ mantree_comm_rest$YSR)
# these variables are correlated (polynomial)! Therefore cannot model their interaction. and should check with Eve whether we can use both in models (there is variability within each YSR so it would be nice to..)
boxplot(mantree_comm_rest$Elevation ~ mantree_comm_rest$YSR)

# YSR vs estuary
boxplot(mantree_comm_rest$YSR ~ mantree_comm_rest$Estuary) # not that overlapping, therefore don't model interaction

# Elevation vs estuary
boxplot(mantree_comm_rest$Elevation ~ mantree_comm_rest$Estuary) # quite overlapping, so can include an interaction if we think it's relevant. The effect of elevation could depend on estuary so I think leave interaction in where the model runs.
```

```{r Elevation - YSR - model}
M1 <- lm(Elevation ~ poly(YSR,2) + Estuary, data = mantree_comm_rest)

plot(M1) # looks roughly ok given it's a relatively small dataset

summary(M1)

# ideally we'd include Site as an additional random effect, but this has a problem with singular fits.

M2 <- lme4::lmer(Elevation ~ poly(YSR,2) + Estuary + (1|Site), data = mantree_comm_rest)

summary(M2)
```

```{r Elevation - YSR - plot}
# restoring sites YSR
...
M0 <- glm(Saplings ~ poly(YSR,2) + Elevation + Estuary + offset(log(Quadrat_size_m2)), family = "poisson", data = mantree_comm_rest) # testing collinearity model
...
# plot natural and restored together
ggarrange(m_ht_rest,m_ht_nat,ncol =2,nrow=1,widths = c(1,1))
